...
Git::Git(const Context &cx, QString const &repodir, const QString &submodpath, const QString &sshkey)
	: m(new Private)
{
    qDebug()<<"Git::Git(const Context &cx, QString const &repodir, const QString &submodpath, const QString &sshkey) :"<<cx.git_command<<cx.ssh_command<<repodir<<submodpath;
...
{
    qDebug()<<"setGitCommand:"<<gitcmd<<sshcmd;
...
{
    qDebug() << "git command 1 : " << arg;
    qDebug() << "git command 11: " << gitCommand();
	QFileInfo info(gitCommand());
    qDebug() << "git command 2: " << gitCommand();
...
{
	out->clear();
	qDebug()<<"getRemoteURLs1";
	git("remote -v");
	qDebug()<<"getRemoteURLs2";
	QStringList lines = misc::splitLines(resultText());
	for (QString const &line : lines) {
		int i = line.indexOf('\t');
		int j = line.indexOf(" (");
