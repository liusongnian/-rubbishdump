...
{
    ui->setupUi(this);
    qDebug()<<"MainWindow ui->frame_repository_wrapper->bind";
    ...
    //connect(ui->treeWidget_repos_1, &RepositoriesTreeWidget::dropped, this, &MainWindow::onRepositoriesTreeDropped);
...
{
    qDebug()<<"MainWindow::onStartEvent()";
...
{
    qDebug()<<"MainWindow::customEvent(QEvent *e)";
...
{
    qDebug()<<"MainWindow::execWelcomeWizardDialog()";
...
void MainWindow::execRepositoryPropertyDialog(const RepositoryItem &repo, bool open_repository_menu)
{
...
{
    qDebug()<<"MainWindow::setGitCommand"<<path;
...
void MainWindow::setCurrentRepository(const RepositoryItem &repo, bool clear_authentication)
{
...
void MainWindow::openTerminal(const RepositoryItem *repo)
{
...
void MainWindow::openExplorer(const RepositoryItem *repo)
{
    runOnRepositoryDir([](QString dir, QString ssh_key){
        (void)ssh_key;
#ifdef Q_OS_MAC
...
bool MainWindow::runOnRepositoryDir(const std::function<void (QString, QString)> &callback, const RepositoryItem *repo)
{
    if (!repo) {
        repo = &m->current_repo;
    }
...
{
    QString text;
    qDebug()<<"MainWindow updateStatusBarText";
    ...
    qDebug()<<"MainWindow updateStatusBarText"<<text;
    setStatusBarText(text);
    qDebug()<<"MainWindow updateStatusBarText end"<<text;
}
...
void MainWindow::on_treeWidget_repos_currentItemChanged(QTreeWidgetItem * /*current*/, QTreeWidgetItem * /*previous*/)
{
    ...
    updateStatusBarText(frame());
    qDebug()<<"on_treeWidget_repos_currentItemChanged end";
}
...
void MainWindow::on_tableWidget_log_customContextMenuRequested(const QPoint &pos)
{
    ...
    if (commit) {
        ...
        qDebug()<<"on_tableWidget_log_customContextMenuRequested1"<<row;
...
void MainWindow::showSubmoduleMainWindow(const QString submodpath)
{
    if (!Git::isValidWorkingCopy(submodpath)) {
        if (QFileInfo(submodpath).isDir()) {
            QString text;
            text += tr("The folder is not a valid git repository.") + '\n';
            text += '\n';
            text += submodpath + '\n';
            text += '\n';
            text += tr("please check your repository!") + '\n';
            int r = QMessageBox::information(this, tr("Initialize Repository") , text, QMessageBox::Yes, QMessageBox::No);
            if (r == QMessageBox::Yes) {
                createRepository(submodpath);
            }
        }
        return;
    }
    GitPtr g = git(submodpath, {}, {});
    SubmoduleMainWindow *w = new SubmoduleMainWindow();
    w->show();
}
...
{
    qDebug()<<"MainWindow::selectGitCommand(bool save)";
...
const RepositoryItem &MainWindow::currentRepository() const
{
...
void MainWindow::on_action_open_existing_working_copy_1_triggered()
{
    QString dir = defaultWorkingDir();
    dir = QFileDialog::getExistingDirectory(this, tr("Add existing working copy"), dir);
    qDebug()<<"on_action_open_existing_working_copy_1_triggered"<<dir;
    showSubmoduleMainWindow(dir);
    //addWorkingCopyDir(dir, false);
}

void MainWindow::openPatchRepository()
{
    QString dir = defaultWorkingDir();
    dir = QFileDialog::getExistingDirectory(this, tr("Add existing working copy"), dir);
    qDebug()<<"on_action_open_existing_working_copy_1_triggered"<<dir;
...
{
    qDebug()<<"MainWindow::on_listWidget_unstaged_currentRowChanged";
    updateUnstagedFileCurrentItem(frame());
    qDebug()<<"MainWindow::on_listWidget_unstaged_currentRowChanged end";
}
...
{
    qDebug()<<"MainWindow::on_action_edit_settings_triggered()";
