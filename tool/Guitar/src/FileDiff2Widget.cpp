...
struct FileDiff2Widget::Private {
    SubmoduleMainWindow *mainwindow = nullptr;
	FileDiff2Widget::InitParam_ init_param_;
...
FileDiff2Widget::FileDiff2Widget(QWidget *parent)
	: QWidget(parent)
	, ui(new Ui::FileDiff2Widget)
	, m(new Private)
{
	ui->setupUi(this);
	...
	connect(ui->widget_diff_slider, &FileDiffSliderWidget::valueChanged, this, &FileDiff2Widget::scrollTo);
	connect(ui->widget_diff_left->texteditor(), &TextEditorWidget::moved, this, &FileDiff2Widget::onMoved);
	connect(ui->widget_diff_right->texteditor(), &TextEditorWidget::moved, this, &FileDiff2Widget::onMoved);
	...
	setViewType(FileViewType1::None);
}

FileDiff2Widget::~FileDiff2Widget()
{
	delete m;
	delete ui;
}

void FileDiff2Widget::setViewType(FileViewType1 type)
{
	ui->widget_diff_left->setViewType(type);
	ui->widget_diff_right->setViewType(type);
}

void FileDiff2Widget::setMaximizeButtonEnabled(bool f)
{
	ui->toolButton_fullscreen->setVisible(f);
	ui->toolButton_fullscreen->setEnabled(f);
}

FileDiff2Widget::ViewStyle FileDiff2Widget::viewstyle() const
{
...
void FileDiff2Widget::bind(SubmoduleMainWindow *mw)
{
	Q_ASSERT(mw);
	m->mainwindow = mw;
	...
	connect(ui->verticalScrollBar, &QAbstractSlider::valueChanged, this, &FileDiff2Widget::onVerticalScrollValueChanged);
	connect(ui->horizontalScrollBar, &QAbstractSlider::valueChanged, this, &FileDiff2Widget::onHorizontalScrollValueChanged);
}

SubmoduleMainWindow *FileDiff2Widget::mainwindow()
{
	return m->mainwindow;
}

GitPtr FileDiff2Widget::git()
{
	if (!mainwindow()) {
		qDebug() << "Maybe, you forgot to call FileDiff2Widget::bind()?";
		return GitPtr();
	}
	return mainwindow()->git();
}

Git::Object FileDiff2Widget::cat_file(GitPtr g, QString const &id)
{
    (void)g;
    qDebug() << "FileDiff2Widget::cat_file"<<id;
    assert(mainwindow()->frame());
    return mainwindow()->cat_file(mainwindow()->frame(), id);
    qDebug() << "FileDiff2Widget::cat_file end";
}

int FileDiff2Widget::totalTextLines() const
{
	return m->engine_left->document.lines.size();
}

void FileDiff2Widget::clearDiffView()
{
	ui->widget_diff_slider->clear(false);
}

int FileDiff2Widget::fileviewHeight() const
{
	return ui->widget_diff_left->height();
}

void FileDiff2Widget::resetScrollBarValue()
{
	ui->verticalScrollBar->setValue(0);
	ui->horizontalScrollBar->setValue(0);
}

void FileDiff2Widget::scrollToBottom()
{
	QScrollBar *sb = ui->verticalScrollBar;
	sb->setValue(sb->maximum());
}

void FileDiff2Widget::updateSliderCursor()
{
...
void FileDiff2Widget::updateControls()
{
	updateSliderCursor();
}

void FileDiff2Widget::makeSideBySideDiffData(Git::Diff const &diff, std::vector<std::string> const &original_lines, TextDiffLineList *left_lines, TextDiffLineList *right_lines)
{
	left_lines->clear();
	right_lines->clear();
...
void FileDiff2Widget::setDiffText(Git::Diff const &diff, TextDiffLineList const &left, TextDiffLineList const &right)
{
	m->max_line_length = 0;
...
FileViewType1 FileDiff2Widget::setupPreviewWidget()
{
	clearDiffView();
...
//	qDebug() << mimetype_l << mimetype_r;
	...
	if (misc::isImage(mimetype_l) || misc::isImage(mimetype_r)) { // image
		...
		return FileViewType1::Text;
	}
}

void FileDiff2Widget::setSingleFile(QByteArray const &ba, QString const &id, QString const &path)
{
	m->init_param_ = InitParam_();
	m->init_param_.view_style = FileDiff2Widget::ViewStyle::SingleFile;
...
void FileDiff2Widget::setOriginalLines_(QByteArray const &ba, Git::SubmoduleItem const *submodule, Git::CommitItem const *submodule_commit)
{
    (void)submodule;
...
void FileDiff2Widget::setLeftOnly(QByteArray const &ba, Git::Diff const &diff)
{
	m->init_param_ = InitParam_();
	m->init_param_.view_style = FileDiff2Widget::ViewStyle::LeftOnly;
	m->init_param_.bytes_a = ba;
	m->init_param_.diff = diff;

	setOriginalLines_(ba, &diff.a_submodule.item, &diff.a_submodule.commit);

	if (setupPreviewWidget() == FileViewType1::Text) {
...
bool FileDiff2Widget::setSubmodule(Git::Diff const &diff)
{
...
void FileDiff2Widget::setRightOnly(QByteArray const &ba, Git::Diff const &diff)
{
	m->init_param_ = InitParam_();
	m->init_param_.view_style = FileDiff2Widget::ViewStyle::RightOnly;
	...
	if (setSubmodule(diff)) {
		// ok
	} else if (setupPreviewWidget() == FileViewType1::Text) {
...
void FileDiff2Widget::setSideBySide(QByteArray const &ba, Git::Diff const &diff, bool uncommited, QString const &workingdir)
{
	m->init_param_ = InitParam_();
	m->init_param_.view_style = FileDiff2Widget::ViewStyle::SideBySideText;
	m->init_param_.bytes_a = ba;
	m->init_param_.diff = diff;
...
void FileDiff2Widget::setSideBySide_(QByteArray const &ba_a, QByteArray const &ba_b, QString const &workingdir)
{
	m->init_param_ = InitParam_();
	m->init_param_.view_style = FileDiff2Widget::ViewStyle::SideBySideImage;
...
QString FileDiff2Widget::diffObjects(GitPtr g, QString const &a_id, QString const &b_id)
{
	if (m->text_codec) {
...
bool FileDiff2Widget::isValidID_(QString const &id)
{
...
void FileDiff2Widget::updateDiffView(Git::Diff const &info, bool uncommited)
{
    qDebug()<<"FileDiff2Widget::updateDiffView";
	GitPtr g = git();
	if (!g) return;
	if (!g->isValidWorkingCopy()) return;
    qDebug()<<"FileDiff2Widget::updateDiffView 1";
    if (isValidID_(info.blob.a_id) && isValidID_(info.blob.b_id)) {
        qDebug()<<"FileDiff2Widget::updateDiffView 2"<<info.blob.a_id<<info.blob.b_id;
        Git::Object obj_a = cat_file(g, info.blob.a_id);
        qDebug()<<"FileDiff2Widget::updateDiffView 3";
		Git::Object obj_b = cat_file(g, info.blob.b_id);
        qDebug()<<"FileDiff2Widget::updateDiffView 4";
		QString mime_a = mainwindow()->determinFileType(obj_a.content);
        qDebug()<<"FileDiff2Widget::updateDiffView 5";
		QString mime_b = mainwindow()->determinFileType(obj_b.content);
        qDebug()<<"FileDiff2Widget::updateDiffView 6";
		if (misc::isImage(mime_a) && misc::isImage(mime_b)) {
            qDebug()<<"FileDiff2Widget::updateDiffView 7";
			setSideBySide_(obj_a.content, obj_b.content, g->workingDir());
            qDebug()<<"FileDiff2Widget::updateDiffView 8";
			return;
		}
	}
    ...
    qDebug()<<"FileDiff2Widget::updateDiffView end";
}

void FileDiff2Widget::updateDiffView(QString const &id_left, QString const &id_right, QString const &path)
{
	GitPtr g = git();
	if (!g) return;
	if (!g->isValidWorkingCopy()) return;
...
void FileDiff2Widget::resizeEvent(QResizeEvent *)
{
	refrectScrollBar();
}

void FileDiff2Widget::keyPressEvent(QKeyEvent *event)
{
...
void FileDiff2Widget::scrollTo(int value)
{
	ui->verticalScrollBar->setValue(value);
}

void FileDiff2Widget::onVerticalScrollValueChanged(int)
{
	refrectScrollBar();
}

void FileDiff2Widget::onHorizontalScrollValueChanged(int)
{
	refrectScrollBar();
}

void FileDiff2Widget::onDiffWidgetWheelScroll(int lines)
{
	while (lines > 0) {
...
void FileDiff2Widget::onScrollValueChanged2(int value)
{
	ui->verticalScrollBar->setValue(value);
}

void FileDiff2Widget::onDiffWidgetResized()
{
	updateControls();
}

void FileDiff2Widget::on_toolButton_fullscreen_clicked()
{
	if (m->init_param_.diff.blob.a_id.isEmpty() && m->init_param_.diff.blob.b_id.isEmpty()) {
		return;
	}

    BigDiff2Window win(mainwindow());
	win.setWindowState(Qt::WindowMaximized);
    win.init(mainwindow(), m->init_param_);
	win.exec();
}

void FileDiff2Widget::setFocusAcceptable(Qt::FocusPolicy focuspolicy)
{
	ui->widget_diff_left->setFocusPolicy(focuspolicy);
	ui->widget_diff_right->setFocusPolicy(focuspolicy);
}

void FileDiff2Widget::onUpdateSliderBar()
{
...
void FileDiff2Widget::refrectScrollBar()
{
...
QPixmap FileDiff2Widget::makeDiffPixmap(DiffPane pane, int width, int height)
{
...
void FileDiff2Widget::onMoved(int cur_row, int cur_col, int scr_row, int scr_col)
{
	(void)cur_col;
	(void)cur_row;
...
void FileDiff2Widget::setTextCodec(QTextCodec *codec)
{
	m->text_codec = codec;
...
void FileDiff2Widget::setTextCodec(char const *name)
{
	QTextCodec *codec = name ? QTextCodec::codecForName(name) : nullptr;
	setTextCodec(codec);
}

void FileDiff2Widget::on_toolButton_menu_clicked()
{
	QMenu menu;
