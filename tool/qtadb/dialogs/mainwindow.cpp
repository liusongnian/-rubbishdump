...
MainWindow::MainWindow(QWidget *parent) :QMainWindow(parent),ui(new Ui::MainWindow)
{
    ...
    this->readrinymix = NULL;
    this->asoundtrack = NULL;
    this->asoundinfo = NULL;
    this->asoundinfoWidget = NULL;
...
//    this->addButton(QIcon(":icons/contacts.png"), tr("Contacts"), SLOT(showPageContacts()), Action::Device);
    ...
    this->addButton(QIcon(":icons/logcat.png"), tr("tinymix", "logcat button"), "tinymix", SLOT(showTinymix()), Action::Device | Action::Recovery);
    this->addButton(QIcon(":icons/logcat.png"), tr("asoundtrack", "logcat button"), "asoundtrack", SLOT(showasoundtrack()), Action::Device | Action::Recovery);
    this->addButton(QIcon(":icons/logcat.png"), tr("asoundinfo", "logcat button"), "asoundinfo", SLOT(showasoundinfo()), Action::Device | Action::Recovery);
    this->addButton(QIcon(":icons/logcat.png"), tr("asoundinfo_1", "logcat button"), "asoundinfo_1", SLOT(showAsoundinfo()), Action::Device | Action::Recovery | Action::Disconnected | Action::Fastboot);
    this->changeToolBar();
...
void MainWindow::showTinymix()
{
    if (this->readrinymix != NULL)
    {
        this->readrinymix->close();
        this->readrinymix = NULL;
    }

    this->readrinymix = new ReadTinymix;
    this->readrinymix->show();
}

void MainWindow::showasoundtrack()
{
    if (this->asoundtrack != NULL)
    {
        this->asoundtrack->close();
        this->asoundtrack = NULL;
    }

    this->asoundtrack = new AsoundTrack;
    this->asoundtrack->show();
}

void MainWindow::showAsoundinfo()
{
    qDebug()<<__FUNCTION__;
    if (this->asoundinfoWidget == NULL)
    {
        this->asoundinfoWidget = new AsoundinfoWidget(this,this->fileWidget->phone);
        this->settingsWidget->changeFont();
        ui->stackedWidget->addWidget(this->asoundinfoWidget);
    }

    this->setButtonDown(9);
    qDebug()<<__FUNCTION__<<"setButtonDown 9";
    this->startAnimation(this->asoundinfoWidget);
}

void MainWindow::showasoundinfo()
{
qDebug() << "" << __FILE__<<  __FUNCTION__<< __LINE__;
    if (this->asoundinfo != NULL)
    {
qDebug() << "" << __FILE__<<  __FUNCTION__<< __LINE__;
        this->asoundinfo->close();
        this->asoundinfo = NULL;
    }
qDebug() << "" << __FILE__<<  __FUNCTION__<< __LINE__;
    this->asoundinfo = new AsoundInfo;
    this->asoundinfo->show();
}
