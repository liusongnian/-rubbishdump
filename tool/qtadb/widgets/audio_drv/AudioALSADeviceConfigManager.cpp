    ...
    mMixer(NULL)
{
    ...
    /*Audio Start for smartpa fw loading*/
#ifdef CIRRUS_SMART_PA
    for (size_t count = 0; count < mDeviceCtlSeq.mDeviceCltNameVector.size() ; count++) {
              String8 valname = mDeviceCtlSeq.mDeviceCltNameVector.itemAt(count);
              String8 valvalue = mDeviceCtlSeq.mDeviceCltValueVector.itemAt(count);
              setMixerCtl(String8(valname),String8(valvalue));
    }
#endif
    /*Audio End*/
...
status_t AudioALSADeviceConfigManager::GetVersion(XMLElement *root) {

    const char *VersionNumber = root->Attribute("value");  // get action
    if (!VersionNumber) {
        ALOGE("%s VersionNumber NULL", __func__);
        return BAD_VALUE;
    } else {
...
String8 AudioALSADeviceConfigManager::ParseDeviceSequence(XMLElement *root) {
    ...
    if (root != NULL) {
        ...
        if (devicename && action) { // both get data
            sequence = String8(devicename);
        } else {
            ALOGE("%s devicename[%s] action=[%s]", __func__, devicename, action);
            return sequence;
        }
        ...
        if (strcmp(action, AUDIO_DEVICE_TURNON) == 0) {
            ...
            XMLElement *child = root->FirstChildElement("kctl");
            while (child != NULL) {
...
status_t AudioALSADeviceConfigManager::ParseInitSequence(XMLElement *root) {
    ...
    while (child != NULL) {
        ...
        const char *valvalue  = child->Attribute("value");
        if (!valname || !valvalue) {
            ALOGE("valname = %s  valvalue = %s  ", valname, valvalue);
            return NO_INIT;
        } else {
...
int AudioALSADeviceConfigManager::parseCustDevicePathSeq(XMLElement *root) {
    XMLElement *outputDev;
    ...
    while(outputDev) {
        ...
        if (!devStr || !pathStr) {
            ALOGE("%s(), devStr=%s pathStr=%s", __FUNCTION__, devStr, pathStr);
        } else {
...
status_t AudioALSADeviceConfigManager::LoadAudioConfig(const char *path) {
    ...
    if (doc.LoadFile(path) == XML_SUCCESS) {
        ...
        if (declaration == NULL) {
            ALOGW("LoadAudioConfig declaration NULL");
            return NAME_NOT_FOUND;
        }
        ...
        if (root != NULL) {
            XMLElement *Version = root->FirstChildElement("versioncontrol"); // find with version contol
            if (Version != NULL) {
                GetVersion(Version);
            }
        }
        ...
        if (root) {
            ...
            XMLElement *child = root->FirstChildElement("path");
            while (child) {
        ...
        // load failed
        ALOGE("LoadAudioConfig fail, err(%d) %s", doc.ErrorID(), doc.ErrorStr());
        return INVALID_OPERATION;
    }
    return NO_ERROR;
}
