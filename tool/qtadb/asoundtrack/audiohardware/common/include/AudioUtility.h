...
#define AUDIO_FORMAT_16_BIT_STR "AUDIO_FORMAT_PCM_16_BIT"
#define AUDIO_FORMAT_24_BIT_STR "AUDIO_FORMAT_PCM_8_24_BIT"
#define AUDIO_CHANNEL_STEREO_STR "AUDIO_CHANNEL_OUT_STEREO"
#define AUDIO_CHANNEL_3CH_STR "AUDIO_CHANNEL_OUT_2POINT1"
#define AUDIO_CHANNEL_4CH_STR "AUDIO_CHANNEL_OUT_2POINT0POINT2"
#define AUDIO_CHANNEL_5CH_STR "AUDIO_CHANNEL_OUT_3POINT0POINT2"
#define AUDIO_CHANNEL_6CH_STR "AUDIO_CHANNEL_OUT_5POINT1"
#define AUDIO_CHANNEL_7CH_STR "AUDIO_CHANNEL_OUT_6POINT1"
#define AUDIO_CHANNEL_8CH_STR "AUDIO_CHANNEL_OUT_7POINT1"
#define AUDIO_SAMPLERATE_32K_STR "32000"
#define AUDIO_SAMPLERATE_44K_STR "44100"
#define AUDIO_SAMPLERATE_48K_STR "48000"
#define AUDIO_SAMPLERATE_96K_STR "96000"
#define AUDIO_SAMPLERATE_192K_STR "192000"
...
enum {
    AUD_TASK_DEFAULT_DISABLE,
    AUD_TASK_DEFAULT_ENABLE,
};
...
// enum for MTK_AUDIO_DYNAMIC_LOG
enum {
   AUD_LOG_DEBUG_EN         = 0x1 << 0,
   AUD_LATENCY_MONITOR      = 0x1 << 1,
   AUD_LATENCY_MONITOR_ALL  = 0x1 << 2,
   AUD_OUT_OP_LOG           = 0x1 << 3,
   AUD_OUT_WRITE_LOG        = 0x1 << 4,
   AUD_DSP_STREAM           = 0x1 << 5,
};
...
typedef enum {
    KERNEL_BUFFER_SIZE_NORMAL = 0,
    KERNEL_BUFFER_SIZE_DEEP,
    KERNEL_DSPBUFFER_SIZE_DEEP,
    KERNEL_DSPBUFFER_SIZE_HIFI_96K,
    KERNEL_DSPBUFFER_SIZE_HIFI_192K,
    NUM_PLATFORM_FO_TYPE_INT,
} audio_platform_fo_int_type_t;

typedef enum {
    DPCM_DEEP_BUFFER = 0,
    NUM_PLATFORM_FO_TYPE_STR,
} audio_platform_fo_str_type_t;

static const char* const platformInfoIntTypeStr[NUM_PLATFORM_FO_TYPE_INT] = {
    [KERNEL_BUFFER_SIZE_NORMAL] = "KERNEL_BUFFER_SIZE_NORMAL",
    [KERNEL_BUFFER_SIZE_DEEP]   = "KERNEL_BUFFER_SIZE_DEEP",
    [KERNEL_DSPBUFFER_SIZE_DEEP]      = "KERNEL_DSPBUFFER_SIZE_DEEP",
    [KERNEL_DSPBUFFER_SIZE_HIFI_96K]  = "KERNEL_DSPBUFFER_SIZE_HIFI_96K",
    [KERNEL_DSPBUFFER_SIZE_HIFI_192K] = "KERNEL_DSPBUFFER_SIZE_HIFI_192K",
};

static const char* const platformInfoStringTypeStr[NUM_PLATFORM_FO_TYPE_STR] = {
    [DPCM_DEEP_BUFFER] = "DPCM_DEEP_BUFFER",
};


class AudioPlatformInfo {
public:
    ~AudioPlatformInfo();
    static AudioPlatformInfo *getInstance();
    AudioPlatformInfo(const AudioPlatformInfo&) = delete;
    AudioPlatformInfo& operator=(const AudioPlatformInfo&) = delete;

    int getPlatformInfoValue(audio_platform_fo_int_type_t type);
    const char* getPlatformInfoStr(audio_platform_fo_str_type_t type);
private:
    AudioPlatformInfo();
    /**
     * singleton pattern
     */
    static AudioPlatformInfo *mAudioPlatformInfo;

    int mPlatformInfoValue[NUM_PLATFORM_FO_TYPE_INT];
    const char* mPlatformInfoStr[NUM_PLATFORM_FO_TYPE_STR];
};
...
extern const char *streamout_log_propty;
extern const char *streamin_log_propty;
...
void setupCustomInfoStr(char* str, size_t strLength, const char* sceneName, int volLevel, int btCodec);
 ...
 * adaptive sleep
 */
#define UNIT_SLEEP_US (1000)
#define ERROR_THD_SLEEP_US (2000)
class AudioThrottleTimeControl {
public:
    AudioThrottleTimeControl(bool isOutput, uint32_t unitSleepUs = UNIT_SLEEP_US, uint32_t errorThdSleepUs = ERROR_THD_SLEEP_US);
    ~AudioThrottleTimeControl();
    int64_t calculateSleepUs(uint64_t totalbytes, stream_attribute_t streamAttribute);
    int adaptiveSleepUs(ssize_t readsize, stream_attribute_t streamAttribute);
    void resetTimeStampInfo();
    void updateTimeStamp(ssize_t readsize);

private:
    uint64_t mBytesSum;
    int64_t mThrottleControlStartTime;
    uint32_t mThrottleControlUnitSleepUs;
    uint32_t mThrottleControlErrorThdSleepUs;
    uint32_t mIsOutput;
};
