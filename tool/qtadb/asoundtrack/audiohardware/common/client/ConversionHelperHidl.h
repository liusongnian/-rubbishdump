...
protected:
    ...
    status_t processReturn(const char* funcName, const Return<hardware::audio::V7_0::Result>& ret) {
        if (!ret.isOk()) {
            emitError(funcName, ret.description().c_str());
        }
    ...
    status_t processReturn(
        const char* funcName, const Return<T>& ret, hardware::audio::V7_0::Result retval) {
        if (!ret.isOk()) {
            emitError(funcName, ret.description().c_str());
        }
...
private:
    ...
    static status_t analyzeResult(const hardware::audio::V7_0::Result& result);
