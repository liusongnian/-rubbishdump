...
int Play2Way::Start() {
    ...
    if (mIsPlay2WayDumpEnable) {
        ...
        if (timeinfo == NULL) {
            ...
            int ret = strftime(fileNamePlay2Way + kSizeOfPrefixFileNamePlay2Way,
                               kMaxSizeOfFileName - kSizeOfPrefixFileNamePlay2Way - 1, "_%Y_%m_%d_%H%M%S.pcm", timeinfo);
            if (ret == 0) {
                ALOGE("%s(), strftime() fail!!", __FUNCTION__);
                return -EINVAL;
            }
        }
...
int Play2Way::Stop() {
    ...
    if (pPlay2WayDumpFile != NULL) {
        int ret = fclose(pPlay2WayDumpFile);
        if (ret != 0) {
...
uint16_t Play2Way::PutDataToSpeaker(char *target_ptr, uint16_t num_data_request) {
    ...
    if (mIsPlay2WayDumpEnable) {
        if (pPlay2WayDumpFile != NULL) {
            uint32_t write_bytes = fwrite(target_ptr, sizeof(char), num_data_request, pPlay2WayDumpFile);
            if (write_bytes != num_data_request) {
                ALOGE("%s(), fwrite fail! data_count: %d, write_bytes: %d", __FUNCTION__, num_data_request, write_bytes);
            }
        } else {
...
int Record2Way::Start() {
    ...
    if (mIsRecord2WayDumpEnable) {
        ...
        if (timeinfo == NULL) {
            ...
            int ret = strftime(fileNameRecord2Way + kSizeOfPrefixFileNameRecord2Way,
                               kMaxSizeOfFileName - kSizeOfPrefixFileNameRecord2Way - 1, "_%Y_%m_%d_%H%M%S.pcm", timeinfo);
            if (ret == 0) {
                ALOGE("%s(), strftime() fail!!", __FUNCTION__);
                return -EINVAL;
            }
...
int Record2Way::Stop() {
    ...
    if (pRecord2WayDumpFile != NULL) {
        int ret = fclose(pRecord2WayDumpFile);
        if (ret != 0) {
            ALOGE("fclose file error %d", ret);
        }
        pRecord2WayDumpFile = NULL;
...
void Record2Way::GetDataFromMicrophone(RingBuf ul_ring_buf) {
    ...
    if (mIsRecord2WayDumpEnable) {
        ...
        if (pRecord2WayDumpFile != NULL) {
            uint32_t write_bytes = fwrite(linear_buffer, sizeof(char), ShareBuf_dataCnt, pRecord2WayDumpFile);
            if (write_bytes != ShareBuf_dataCnt) {
                ALOGE("%s(), fwrite fail! data_count: %zu, write_bytes: %d", __FUNCTION__, ShareBuf_dataCnt, write_bytes);
            }
        } else {
