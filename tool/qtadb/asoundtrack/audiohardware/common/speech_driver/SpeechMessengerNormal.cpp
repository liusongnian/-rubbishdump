...
int SpeechMessengerNormal::openCcciDriver() {
    ...
    strncpy(dev_name, CCCI_DEV_NODE_DRIVER, k_max_device_name_size - 1);
    dev_name[k_max_device_name_size - 1] = '\0'; // for coverity check null-terminated string
...
uint16_t SpeechMessengerNormal::getMdVersion() {
    if (mSpeechShareMem == NULL) {
        ALOGE("%s(), mSpeechShareMem == NULL!!", __FUNCTION__);
        return false;
    }
    return mSpeechShareMem->getMdVersion();
}
