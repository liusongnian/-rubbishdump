...
typedef struct sph_shm_t {
    ...
    /* 2 bytes mdVersion info for 95 parser sync with XML version */
    uint16_t mdVersion;       // enh parameter md version
...
int SpeechExtMemCCCI::formatShareMemory() {
    ...
    /* md version info no need to format
    1.MD write before this
    2.EMI will self-init to 0 before audio using
    */
...
uint16_t SpeechExtMemCCCI::getMdVersion() {
    if (!mShareMemory) {
        ALOGW("%s(), mShareMemory NULL!! return false", __FUNCTION__);
        return 0;
    }
    uint16_t mdVersion = mShareMemory->mdVersion;

    ALOGD("%s(), mdVersion: 0x%x\n", __FUNCTION__, mdVersion);
    return mdVersion;
}
