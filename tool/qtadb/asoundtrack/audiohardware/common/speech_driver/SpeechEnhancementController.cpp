...
SpeechEnhancementController::SpeechEnhancementController() {
    ...
    int ret = sscanf(property_value, "%" SCNd16 ",%" SCNd32,
                 &mSpeechEnhancementMask.main_func,
                 &mSpeechEnhancementMask.dynamic_func);
    if (ret != 2) {
        ALOGD("%s(), get value from propterty %s fail! value:%s, use default value",
            __FUNCTION__, PROPERTY_KEY_SPH_ENH_MASKS, property_value);
        mSpeechEnhancementMask.main_func = default_mask.main_func;
        mSpeechEnhancementMask.dynamic_func = default_mask.dynamic_func;
    }
    ...
    // HAC
    uint32 isHacOn = get_uint32_from_property(PROPERTY_KEY_HAC_ON);
    mHACOn = (isHacOn == 0) ? false : true;

    // BT Headset NREC
    uint32 isBtNrecOn = get_uint32_from_property(PROPERTY_KEY_BT_HEADSET_NREC_ON);
...
status_t SpeechEnhancementController::SetSpeechEnhancementMaskToAllModem(const sph_enh_mask_struct_t &mask) {
    ...
    int ret = snprintf(property_value, sizeof(property_value), "%d,%d",
                       mask.main_func, mask.dynamic_func);
    if (ret < 0 || ret >= sizeof(property_value)) {
        ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d",
              __FUNCTION__, property_value, sizeof(property_value), ret);
    } else {
...
void SpeechEnhancementController::SetHACOn(const bool hac_on) {
    ...
    set_uint32_to_property(PROPERTY_KEY_HAC_ON, (hac_on == false) ? 0 : 1);
...
void SpeechEnhancementController::SetBtHeadsetNrecOnToAllModem(const bool bt_headset_nrec_on) {
    ...
    set_uint32_to_property(PROPERTY_KEY_BT_HEADSET_NREC_ON, (bt_headset_nrec_on == false) ? 0 : 1);
