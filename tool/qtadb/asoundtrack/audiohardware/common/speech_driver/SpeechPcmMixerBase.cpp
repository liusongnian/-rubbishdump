...
static const char     kFolderOfDumpFile[] = LOG_PATH_AUDIO_DUMP;
// PROPERTY_KEY_PCM_MIXER_DUMP_ON
//   SpeechPcmMixerBase::PutDataToSpeaker()
static char gFileNamePcmMixer[kMaxSizeOfFileName] = {0};

// PROPERTY_KEY_PCM_MIXER_BLISRC_DUMP_ON
//   SpeechPcmMixerBaseBuffer::Write()
//   before SRC, same as AudioALSAPlaybackHandlerVoice dump
static char gFileNamePcmMixerBlisrc[kMaxSizeOfFileName] = {0};

static uint32_t gSizeOfPrefixFileNamePcmMixer = 0;
static uint32_t gSizeOfPrefixFileNamePcmMixerBlisrc = 0;
...
bool SpeechPcmMixerBaseBuffer::IsPcmMixerBlisrcDumpEnable() {
    ...
    char property_value_pcm_mixer[PROPERTY_VALUE_MAX] = {0};
    char property_value_streamout[PROPERTY_VALUE_MAX] = {0};
    property_get(PROPERTY_KEY_PCM_MIXER_BLISRC_DUMP_ON, property_value_pcm_mixer, "0"); //"0": default off
    property_get(streamout_propty, property_value_streamout, "0"); //"0": default off

    if (property_value_pcm_mixer[0] == '1' || property_value_streamout[0] == '1') {
        return true;
    } else {
        return false;
    }
...
status_t SpeechPcmMixerBaseBuffer::InitPcmMixerBuffer(SpeechPcmMixerBase *playPointer,
    ...
    status_t ret_time = NO_ERROR;
    int ret = 0;
    ALOGV("%s(), initPcmMixerBuffer sampleRate=%d, ch=%d, format=%d", __FUNCTION__, sampleRate, chNum, format);
    (void)playPointer;
    // keep the format
    ...
    ret = snprintf(gFileNamePcmMixer, sizeof(gFileNamePcmMixer), "%s%s", kFolderOfDumpFile, "PcmMixer_type_");
    if (ret < 0 || ret >= sizeof(gFileNamePcmMixer)) {
        ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d",
              __FUNCTION__, gFileNamePcmMixer, sizeof(gFileNamePcmMixer), ret);
    }
    gSizeOfPrefixFileNamePcmMixer = strlen(gFileNamePcmMixer) - 1;
    ret = snprintf(gFileNamePcmMixerBlisrc, sizeof(gFileNamePcmMixerBlisrc), "%s%s", kFolderOfDumpFile,
                   "PcmMixer_before_Blisrc_type_");
    if (ret < 0 || ret >= sizeof(gFileNamePcmMixerBlisrc)) {
        ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d",
              __FUNCTION__, gFileNamePcmMixerBlisrc, sizeof(gFileNamePcmMixerBlisrc), ret);
    }
    gSizeOfPrefixFileNamePcmMixerBlisrc = strlen(gFileNamePcmMixerBlisrc) - 1;
    ...
    if (mIsPcmMixerBlisrcDumpEnable) {
        ...
        audio_strncpy(fileNamePcmMixerBlisrc, gFileNamePcmMixerBlisrc, kMaxSizeOfFileName);
        ret = snprintf(pcmMixerTypeString, sizeof(pcmMixerTypeString), "%u", pcmMixerType);
        if (ret < 0 || ret >= sizeof(pcmMixerTypeString)) {
            ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d",
                  __FUNCTION__, pcmMixerTypeString, sizeof(pcmMixerTypeString), ret);
            ret_time = -EINVAL;
        } else {
        ...
        if (timeinfo == NULL) {
            ALOGE("%s(), localtime fail", __FUNCTION__);
            ret_time = -EINVAL;
        } else {
            ret = strftime(fileNamePcmMixerBlisrc + gSizeOfPrefixFileNamePcmMixerBlisrc,
                           kMaxSizeOfFileName - gSizeOfPrefixFileNamePcmMixerBlisrc - 1, "_%Y_%m_%d_%H%M%S.pcm", timeinfo);
            if (ret == 0) {
                ALOGE("%s(), %s strftime() fail!!", __FUNCTION__, gFileNamePcmMixerBlisrc);
                ret_time = -EINVAL;
            }
        }
        if (ret_time != NO_ERROR) {
            ...
            ret = snprintf(fileIndex, kMaxSizeOfFileName, "_%u.pcm", pcm_mixer_file_number++);
            if (ret < 0 || ret >= kMaxSizeOfFileName) {
                ALOGE("%s(), snprintf %s fail!! sz %d, ret %d",
                      __FUNCTION__, fileIndex, kMaxSizeOfFileName, ret);
            } else {
                audio_strncat(fileNamePcmMixerBlisrc, fileIndex, sizeof(fileNamePcmMixerBlisrc));
            }
            ret = snprintf(property_value, PROPERTY_VALUE_MAX, "%u", pcm_mixer_file_number & 0x7);
            if (ret < 0 || ret >= PROPERTY_VALUE_MAX) {
                ALOGE("%s(), snprintf %s fail!! sz %d, ret %d",
                      __FUNCTION__, property_value, PROPERTY_VALUE_MAX, ret);
            } else {
...
SpeechPcmMixerBaseBuffer::~SpeechPcmMixerBaseBuffer() {
    ...
    if (pDumpFile != NULL) {
        int ret = fclose(pDumpFile);
        if (ret != 0) {
            ALOGE("fclose file error %d", ret);
        }
        pDumpFile = NULL;
    }
...
uint32_t SpeechPcmMixerBaseBuffer::Write(char *buf, uint32_t num) {
    ...
    if (mIsPcmMixerBlisrcDumpEnable) {
        if (pDumpFile != NULL) {
            uint32_t write_bytes = fwrite(buf, sizeof(char), num, pDumpFile);
            if (write_bytes != num) {
                ALOGE("%s(), fwrite fail! data_count: %d, write_bytes: %d", __FUNCTION__, num, write_bytes);
            }
        }
    }
...
bool SpeechPcmMixerBase::IsPcmMixerDumpEnable() {
    ...
    char property_value_pcm_mixer[PROPERTY_VALUE_MAX] = {0};
    char property_value_streamout[PROPERTY_VALUE_MAX] = {0};
    property_get(PROPERTY_KEY_PCM_MIXER_DUMP_ON, property_value_pcm_mixer, "0"); //"0": default off
    property_get(streamout_propty, property_value_streamout, "0"); //"0": default off
...
uint32_t SpeechPcmMixerBase::Write(SpeechPcmMixerBaseBuffer *pPcmMixerBuffer, void *buf, uint32_t num) {
    ...
    uint32_t ret = 0;
    if (pPcmMixerBuffer) {
        ret = pPcmMixerBuffer->Write((char *)buf, num);
    }
    return ret;
}
...
bool SpeechPcmMixerBase::Open(SpeechDriverInterface *pSpeechDriver) {
    ...
    if (mIsPcmMixerDumpEnable) {
        ...
        audio_strncpy(fileNamePcmMixer, gFileNamePcmMixer, kMaxSizeOfFileName);

        int ret = snprintf(pcmMixerTypeString, sizeof(pcmMixerTypeString), "%u", getPcmMixerType());
        if (ret < 0 || ret >= sizeof(pcmMixerTypeString)) {
            ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d",
                  __FUNCTION__, pcmMixerTypeString, sizeof(pcmMixerTypeString), ret);
        } else {
        ...
        if (timeinfo == NULL) {
            ...
            ret = strftime(fileNamePcmMixer + gSizeOfPrefixFileNamePcmMixer,
                           kMaxSizeOfFileName - gSizeOfPrefixFileNamePcmMixer - 1, "_%Y_%m_%d_%H%M%S.pcm", timeinfo);
            if (ret == 0) {
                ALOGE("%s(), %s strftime() fail!!", __FUNCTION__, gFileNamePcmMixer);
                ret_time = -EINVAL;
            }
        }
        if (ret_time != NO_ERROR) {
...
uint16_t SpeechPcmMixerBase::PutDataToSpeaker(char *target_ptr, uint16_t num_data_request) {
    ...
    if (mIsPcmMixerDumpEnable) {
        if (pDumpFile != NULL) {
            uint32_t write_bytes = fwrite(target_ptr, sizeof(char), write_count, pDumpFile);
            if (write_bytes != write_count) {
                ALOGE("%s(), fwrite fail! data_count: %d, write_bytes: %d", __FUNCTION__, write_count, write_bytes);
            }
        }
    }
    return write_count;
