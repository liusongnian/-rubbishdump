...
typedef bool (*SpeakerStatusChangeCb)(const DeviceStatus, const uint32_t);

#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
typedef void (*OutputDevicesChangeCb)(const audio_devices_t, const DeviceStatus);
#endif
...
public:
    ...
    virtual status_t stopOutputDeviceWithoutNotify();
    ...
    virtual status_t stopOutputDeviceWithoutNotify_l();
...
#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
    // Ultrasound feature will always use receiver and build-in mic to transfer ultrasound.
    virtual status_t startForceUseReceiver(const uint32_t SampleRate);
    virtual status_t stopForceUseReceiver();
    virtual bool getForceUseReceiver();
#endif
    ...
    virtual status_t openAddaOutput(const uint32_t sample_rate,
                                    const audio_devices_t devices = AUDIO_DEVICE_OUT_EARPIECE);
    ...
    */
    ...
    bool notifyOutputDeviceStatusChange(const audio_devices_t device, const DeviceStatus status, const uint32_t sampleRate);

#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
    bool setOutputDevicesChangeCb(const OutputDevicesChangeCb cb);
#endif
    ...
    // Latency unit us
    void setA2dpDeviceLatency(int latency);
...
#endif
    uint32_t getOutputDeviceSampleRate() { return mOutputDeviceSampleRate; }
    ...
    status_t applySettingSequence(String8 sequence);
    ...
    bool is32BitI2sSupport(void);
    int getSpkNum();
    bool isSpkApSwMixSupport(audio_devices_t devices);

    int getTVOutFileDescriptor(void);
    void setTVOutFileDescriptor(int fd);
...
private:
    ...
    int setNumSpkSupport();
    void setSpkApSwMixSupport();
    ...
    AudioLock mLockSpeakerCount;
    int32_t mOpenSpeakerPathCount;
    SpeakerStatusChangeCb mSpeakerStatusChangeCb;

    int mNumPhoneMicSupport;
    int mNumSpeakerSupport;
    bool mMicInverse;
    bool mSpkApSwMixUsed;
...
#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
    OutputDevicesChangeCb mOutputDevicesChangeCb;
#endif
    int mNumHSPole;
    bool mHeadchange;
    ...
    int mTVOutFd;
    AudioLock mGetTVFdLock;
#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
    bool mForceUseReceiver;
#endif
    bool mHDRRecordOn;
};
