...
public:
    ...
    status_t dynamicSetStreamInAudioDump();
...
protected:
     ...
     */
    virtual bool checkOpenStreamFormat(const audio_devices_t devices, int *format);
    virtual bool checkOpenStreamChannels(const audio_devices_t devices, uint32_t *channels);
...
private:
    ...
    unsigned int        mPreProcessEffectBackupCount;
    bool                mLowLatencyMode;
    uint32_t            mKernelBuffersize;
    size_t              mReduceBuffersize;
    uint32_t            mReadThrottleBufferSize;
    uint64_t            mReadTimes;
