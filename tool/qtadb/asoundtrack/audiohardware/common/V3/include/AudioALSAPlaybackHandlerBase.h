...
public:
    ...
    virtual int getQueuedFramesInfo(uint64_t *hdlWriteFrames, uint64_t *queuedFrames, struct timespec *timestamp);
    virtual int getPresentationPosition(uint64_t totalWriteFrames, uint64_t *presentedFrames, struct timespec *timestamp);
    virtual int getPresentationQueuedOffset(uint64_t *queued, uint64_t *offset, struct timespec *timestamp);
    virtual int compensatePresentationPosition(uint64_t *frames, struct timespec *timestamp);
    ...
    void dynamicSetPlaybackHandlerAudioDump();

    /**
     * get extra buffer output side pcm, base on outputdevice or playback type
     * return extera pcm buffer
     */
    ...
    bool isAdspTaskDumpEnable(int property, int taskScene);
     ...
     * sw mixer wrapper
     */
    static int wrapSwMixerWrite(void *buffer, uint32_t bytes, void *arg);
    ...
    /* init log flag*/
    status_t initLogFlag(void);
    status_t initOutMonitor(void);
    /* cal time by enum */
    status_t calHoldTime(unsigned int holdtype);
    /* cal time threshold */
    status_t calThresTime(struct pcm *Pcm);
    /* check threshold */
    status_t checkThresTime(audio_output_flags_t audioFlag);
...
private:
    ...
    struct timespec mTimepre, mTimepost;
    double mPcmBufferMs;
    double mTimeSpent[AUD_THRES_NUMS];
    double mThreadholdTime[AUD_THRES_NUMS];
#endif

    AudioLock mMixerLock;
    volatile int32_t mMixerLockCount;
    uint64_t mHdlWriteFrames;

    bool mLastPositionInfoValid;
    uint64_t mLastHdlWriteFrames;
    int mLastHalQueuedFrame;
    unsigned int mLastKernelAvailFrames;
