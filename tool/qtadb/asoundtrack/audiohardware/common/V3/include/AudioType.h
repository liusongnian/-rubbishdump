...
#ifdef UPLINK_MAX_LATENCY_CHIP_MS
#define UPLINK_MAX_LATENCY_MS (UPLINK_MAX_LATENCY_CHIP_MS)
#else
#define UPLINK_MAX_LATENCY_MS (100)
#endif
#endif

#define MMAP_DL_PERIOD_SIZE      96
#define MMAP_UL_PERIOD_SIZE      48
#define MMAP_UL_POSITION_OFFSET  48

#define MIN_MMAP_DL_PERIOD_COUNT    16           // depend MMAP_UL_PERIOD_SIZE
#define MIN_MMAP_UL_PERIOD_COUNT    32           // depend MMAP_UL_PERIOD_SIZE
#define MAX_MMAP_FRAME_COUNT        8192         // max hw buffer size of share mode
#define MAX_MMAP_HW_BUFFER_SIZE     (24*1024)    // max hw buffer size of exclusive mode

typedef enum {
    AUDIO_DUMP_DOWNLINK,
    AUDIO_DUMP_UPLINK,
    AUDIO_DUMP_DOWNLINK_DSP,
    AUDIO_DUMP_UPLINK_DSP,
    AUDIO_DUMP_A2DP,
    NUM_OF_AUDIO_DUMP
} AUDIO_DUMP_TYPE;
...
enum capture_handler_t {
    ...
    CAPTURE_HANDLER_USB       = (1 << 12),
    CAPTURE_HANDLER_IEM       = (1 << 13),
...
typedef struct {
    ...
    unsigned int PreProcessEffect_Count;
...
struct stream_attribute_t {
    ...
    bool isMixerOut;
    bool isBypassAurisys;

    bool isIEMsSource; // In-ear monitors (IEMs)
    uint32_t periodUs;
    uint32_t periodCnt;

    audio_devices_t dualDevice; // (Earphone+Spk, BT+Spk, USB+SPK)
...
enum audio_a2dp_status {
    A2DP_START_SUCCESS = 0,
    A2DP_START_UNSUPPORTED_CODEC_CONFIGURATION = 1,
    A2DP_START_FAILURE = 2,
    A2DP_SUSPEND_SUCCESS = 3,
    A2DP_SUSPEND_UNSUPPORTED_CODEC_CONFIGURATION = 4,
    A2DP_SUSPEND_FAILURE = 5,
};

enum audio_type_dynamic_param_t {
    AUDIO_TYPE_SPEECH               = 0,
    AUDIO_TYPE_VOIP                 = 1,
    AUDIO_TYPE_RECORD               = 2,
    AUDIO_TYPE_PLAYBACK             = 3,

    NUM_AUDIO_TYPE_TYPE  /* the #types of audio_type_dynamic_param_t */
};
