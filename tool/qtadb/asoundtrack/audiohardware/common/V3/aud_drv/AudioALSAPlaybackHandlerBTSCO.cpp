...
ssize_t AudioALSAPlaybackHandlerBTSCO::write(const void *buffer, size_t bytes) {
...
#endif
    {
        ...
        if (pBufferAfterBitConvertion != NULL) {
            dodataPending(pBufferAfterBitConvertion, bytesAfterBitConvertion, &pBufferAfterPending,
                          &bytesAfterpending);
        }
    }
    ...
    mOldtime = mNewtime;
#endif
    if (pBufferAfterPending != NULL) {
        // write data to pcm driver
        WritePcmDumpData(pBufferAfterPending, bytesAfterpending);
        retval = pcm_write(mPcm, pBufferAfterPending, bytesAfterpending);
    } else {
        bytes = 0;
        retval = EINVAL;
    }
