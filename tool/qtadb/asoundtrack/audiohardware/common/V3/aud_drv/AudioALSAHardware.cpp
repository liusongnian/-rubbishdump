...
#if defined(MTK_AUDIO_DSP_SERVICE)
...
typedef std::match_results<const char*> smatch;
...
static String8 keySet_SpeechCall_Dynamic_DL_Mute = String8("Set_SpeechCall_Dynamic_DL_Mute");
static String8 keySet_SpeechCall_UL_Mute         = String8("Set_SpeechCall_UL_Mute");

// Set Phone Call Record DL Position
static String8 keySet_SpeechRec_DL_Pos  = String8("Set_SpeechRec_DL_Pos");

// Set TelephonyTx data type (0:replace, 1:mix)
static String8 keySet_ScreenCall_Data_Type  = String8("Set_ScreenCall_Data_Type");
...
static String8 keySetRttCallType = String8("ims_rtt_capability");
#endif
...
static String8 keySetDumpAudioStreamOut = String8("SetDumpAudioStreamOut");
static String8 keySetDumpA2DPStreamOut = String8("SetDumpA2DPStreamOut");
static String8 keySetDumpAudioStreamIn = String8("SetDumpAudioStreamIn");
static String8 keySetDumpApSpeechEPL = String8("SetDumpApSpeechEPL");


static String8 keySetAudioHALDump_DL = String8("vendor.streamout.pcm.dump");
static String8 keySetAudioHALDump_UL = String8("vendor.streamin.pcm.dump");
static String8 keySetAudioHALDump_DL_Dsp = String8("vendor.streamout.dsp.dump");
static String8 keySetAudioHALDump_UL_Dsp = String8("vendor.streamin.dsp.dump");
...
static String8 keyFmPlaybackToSpeakerUsingIndirect = String8("FmPlaybackToSpeakerUsingIndirect");

static String8 keySetDSM = String8("DSM");

#if defined(HAVE_IEMS) && defined(HAVE_SW_MIXER)
static String8 keyHalIEM = String8("HAL_IEM");
#endif
...
static String8 keyDspParam  = String8("DSPPARAM");
static String8 keyAdspPhoneCallEnh = String8("adspPhoneCallEnh");
static String8 keyAurisysLibDspVoIPDumpDisable = String8("DSP,VOIP,LIB_ALL,ENABLE_LIB_DUMP,0=SET");
static String8 keyAurisysLibDspVoIPDumpEnable = String8("DSP,VOIP,LIB_ALL,ENABLE_LIB_DUMP,1=SET");
static String8 keyAurisysLibDspRecordDumpDisable = String8("DSP,RECORD,LIB_ALL,ENABLE_LIB_DUMP,0=SET");
static String8 keyAurisysLibDspRecordDumpEnable = String8("DSP,RECORD,LIB_ALL,ENABLE_LIB_DUMP,1=SET");
static String8 keyAurisysLibDspRecordFastDumpDisable = String8("DSP,RECORD_FAST,LIB_ALL,ENABLE_LIB_DUMP,0=SET");
static String8 keyAurisysLibDspRecordFastDumpEnable = String8("DSP,RECORD_FAST,LIB_ALL,ENABLE_LIB_DUMP,1=SET");
#endif
...
#if defined(MTK_COMBO_MODEM_SUPPORT)
static char keySpeechParserSetParam[] = "SPEECH_PARSER_SET_PARAM,PHONE_CALL";
static char keySpeechParserGetParam[] = "SPEECH_PARSER_GET_PARAM,PHONE_CALL";
#endif
...
static String8 keyAurisysLibDumpDisable = String8("HAL,ALL,MTKSE,ENABLE_LIB_DUMP,0=SET");
static String8 keyAurisysLibDumpEnable = String8("HAL,ALL,MTKSE,ENABLE_LIB_DUMP,1=SET");
#endif
...
static String8 keyCallFwdPath = String8("g_call_forwarding_enable");
static String8 keyCallNbQuality = String8("l_call_nb_quality_enable");
static String8 keyCallMemoState = String8("g_call_memo_state");
static String8 keyEnforcedActive = String8("l_stream_enforced_active_in_call");
...
// HAP (Hearing Aid Device Phonecall)
static String8 keySET_SOFTWARE_BRIDGE_ENABLE = String8("SwBridge");
static String8 keySET_FORCE_ASHA_HAL_TEST_ENABLE = String8("FORCE_ASHA_HAL_TEST_ENABLE");
...
static String8 keyMTK_ADSP_VA_ENABLE = String8("MTK_ADSP_VA_ENABLE");
static String8 keyMTK_ADSP_VA_DETECT_ENABLE = String8("MTK_ADSP_VA_DETECT_ENABLE");
#endif

// Voice UnLock
static String8 keyMTK_VOW_SUPPORT = String8("MTK_VOW_SUPPORT");
static String8 keyMTK_VOW_AMAZON_SUPPORT = String8("MTK_VOW_AMAZON_SUPPORT");
...
static String8 keyMTK_VOW_MAX_PDK_NUMBER = String8("MTK_VOW_MAX_PDK_NUMBER");
static String8 keyMTK_VOW_MULTI_KEYPHRASE_SUPPORT = String8("MTK_VOW_MULTI_KEYPHRASE_SUPPORT");
static String8 keyMTK_VOW_DSP_LIB_VERSION = String8("MTK_VOW_DSP_LIB_VERSION");

static String8 keyBargeInForceOff = String8("barge_in_force_off");
...
static String8 keyTUNING_GET_PARAM = String8("TUNING_GET_PARAM");
#endif
...
// HDR Record
static String8 keyHDRRecord = String8("HDR_RECORD");
...
// 3GVT/VoWifi
static String8 keyCallState = String8("g_call_state");
// VoWifi NB/WB
static String8 keyCallBand = String8("g_call_band");
#endif // MTK_TC10_FEATURE
...
public:
    ...
    virtual ~AudioParameterChangedHidlCallback() {}
...
bool isAudioFOOn(const char* foName) {
    bool result = false;
    ...
    if (foName) {
        ...
        if (appOps != NULL) {
            AppHandle *appHandle = appOps->appHandleGetInstance();
            result = appOps->appHandleIsFeatureOptionEnabled(appHandle, foName);
        } else {
            ALOGE("%s(), Error: AppOps == NULL", __FUNCTION__);
            ASSERT(0);
        }
...
String8 getAudioLastCategoryPath(String8 audioTypeName) {
    char *restOfStr = NULL;
    String8 returnStr = String8("");
    bool isEnable = false;
    const char *appendCategoryStr = ", Category: ";
    const char *speechTypeName = "Speech";
    const char *recordTypeName = "Record";
    const char *voipTypeName = "VoIPv2";
    const char *fmTypeName = "FM";

    if (audioTypeName == String8("Playback")) {
        AudioALSAStreamManager *mStreamManager = AudioALSAStreamManager::getInstance();
        if ((mStreamManager->getActiveStreamOutSize() > 0) && (!mStreamManager->isEchoRefUsing())) {
            isEnable = true;
        }
        returnStr.appendFormat("%s %s, ", audioTypeName.string(), isEnable ? "On" : "Off");
        returnStr.append(AudioMTKGainController::getInstance()->getPlaybackCategoryString().string());
        return returnStr;
    }

    if (audioTypeName) {
        if (!strncmp(audioTypeName.string(), speechTypeName, strlen(speechTypeName))) {
            AudioALSASpeechPhoneCallController *speechPhoneCallController = AudioALSASpeechPhoneCallController::getInstance();
            if (!speechPhoneCallController) {
                return returnStr;
            }
            if (speechPhoneCallController->getCallStatus()) {
                returnStr.append("Voice On");
            } else {
                returnStr.append("Voice Off");
            }

            returnStr.append(appendCategoryStr);
            AppOps *appOps = appOpsGetInstance();
            if (appOps != NULL) {
                char *lastCategoryPath = appOps->utilNativeGetLastCategoryPath(audioTypeName.string());
                // replace value of band and network
                if (lastCategoryPath) {
                    char *speechBandName = speechPhoneCallController->getSpeechBandName();
                    char *speechNetworkName = speechPhoneCallController->getSpeechNetworkName();
                    char *tempStr = strtok_r(lastCategoryPath, ARRAY_SEPERATOR, &restOfStr);
                    while (tempStr != NULL) {
                        returnStr.append(tempStr);
                        returnStr.append(ARRAY_SEPERATOR);
                        if (!strcmp(SpeechInfoFromModemType[SPEECH_INFO_FROM_MODEM_BAND], tempStr)) {
                            tempStr = strtok_r(NULL, ARRAY_SEPERATOR, &restOfStr);
                            returnStr.append(speechBandName? speechBandName : tempStr);
                            returnStr.append(ARRAY_SEPERATOR);
                        } else if (!strcmp(SpeechInfoFromModemType[SPEECH_INFO_FROM_MODEM_NETWORK], tempStr)) {
                            tempStr = strtok_r(NULL, ARRAY_SEPERATOR, &restOfStr);
                            returnStr.append(speechNetworkName? speechNetworkName : tempStr);
                        }
                        tempStr = strtok_r(NULL, ARRAY_SEPERATOR, &restOfStr);
                    }
                    free(lastCategoryPath);
                }
            } else {
                ALOGE("%s(), Error: AppOps == NULL", __FUNCTION__);
                ASSERT(0);
            }
        } else if (!strncmp(audioTypeName.string(), recordTypeName, strlen(recordTypeName))) {
            AudioALSAStreamManager *mStreamManager = AudioALSAStreamManager::getInstance();
            if (mStreamManager->isNormalRecordUsing() && !mStreamManager->isEchoRefUsing()) {
                returnStr.append("Record On");
            } else {
                returnStr.append("Record Off");
            }
            ...
            if (appOps != NULL) {
                ...
                if (lastCategoryPath) {
                    ...
                    returnStr.append(" gain: ");
                    returnStr.append(AudioMTKGainController::getInstance()->getUlGainCategoryString().string());
                    free(lastCategoryPath);
                }
            } else {
                ALOGE("%s(), Error: AppOps == NULL", __FUNCTION__);
        ...
        } else if (!strncmp(audioTypeName.string(), voipTypeName, strlen(voipTypeName))) {
            AudioALSAStreamManager *mStreamManager = AudioALSAStreamManager::getInstance();
            if (mStreamManager->isEchoRefUsing()) {
                returnStr.append("VoIP On");
            } else {
                returnStr.append("VoIP Off");
            }
            ...
            if (appOps != NULL) {
                ...
                if (lastCategoryPath) {
                    ...
                    returnStr.setTo(returnStr, strlen(returnStr) - 1);
                    free(lastCategoryPath);
                }
            } else {
                ALOGE("%s(), Error: AppOps == NULL", __FUNCTION__);
        ...
        } else if (!strncmp(audioTypeName.string(), fmTypeName, strlen(fmTypeName))) {
            AudioALSAStreamManager *mStreamManager = AudioALSAStreamManager::getInstance();
            if (mStreamManager->getFmEnable()) {
                returnStr.append("FM On");
            } else {
                returnStr.append("FM Off");
            }
...
AudioALSAHardware::AudioALSAHardware() :
    /* device parser must construct at first to ensure pcm all closed when audioserver initial */
    ...
    audio_fmt_conv_hal_init();
#if defined(HAVE_SW_MIXER)
    init_audio_sw_mixer();
#endif

#ifdef MTK_AURISYS_FRAMEWORK_SUPPORT
    init_aurisys_controller(isAdspOptionEnable());
#endif
...
#if defined(MTK_AUDIO_KS) && defined(MTK_AUDIO_AAUDIO_SUPPORT)
...
AudioALSAHardware::~AudioALSAHardware() {
    ...
    audio_fmt_conv_hal_deinit();
#if defined(HAVE_SW_MIXER)
    deinit_audio_sw_mixer();
#endif
...
status_t AudioALSAHardware::setRttEnabled(bool enabled) {
    ALOGD("%s() : %d", __FUNCTION__, enabled);
    return NO_ERROR;
}

#if defined(MTK_TC10_FEATURE)
void AudioALSAHardware::updateCustomSceneForVoIP()
{
    if (isAudioFOOn("VIR_SCENE_CUSTOMIZATION_SUPPORT")) {
        String8 scene = String8("");
        if (mIs3GVT== true) {
            scene = String8("3GVT");
        } else if (mIsVoWifi == true && mIsVoWifiWbAmr == true && mIsVoWifiSwbAmr == false) {
            scene = String8("VxWifi_WB");
        } else if (mIsVoWifi == true && mIsVoWifiWbAmr == false && mIsVoWifiSwbAmr == false) {
            scene = String8("VxWifi_NB");
        } else if (mIsVoWifi == true && mIsVoWifiWbAmr == false && mIsVoWifiSwbAmr == true) {
            scene = String8("VxWifi_SWB");
        } else {
            scene = String8("Default");
        }

        mStreamManager->setCustScene(scene);

        ALOGD("%s(), scene = %s (3gvt=%d, vxwifi=%d, wb=%d, swb=%d)",
            __FUNCTION__,
            scene.string(),
            mIs3GVT ? 1 : 0,
            mIsVoWifi ? 1 : 0,
        ...
        ALOGD("%s(), scene disabled", __FUNCTION__);
    }
}
#endif

String8 trimMacAddress(std::string str){
    if(str.empty())
        return String8("");
    std::regex delimiter("\\;");
    std::vector<std::string> toLogKVPairs(
                     std::sregex_token_iterator(str.begin(), str.end(), delimiter, -1),
                     std::sregex_token_iterator()
                     );
    std::string expression = "([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})";
    std::regex rx(expression);
    std::smatch m;
    std::string newKeyValuePairs = "";
    uint32_t n = toLogKVPairs.size();
    for(uint32_t i = 0; i < n; i++) {
        std::string val = toLogKVPairs[i];
        if(std::regex_search(val, m, rx))
            continue;
        newKeyValuePairs.append(val);
        newKeyValuePairs.append(";");
    }
    if(!newKeyValuePairs.empty())
        newKeyValuePairs.pop_back();
    if(std::regex_search(str, m, rx))
        return String8(newKeyValuePairs.c_str());
    else
        return String8("");
}

status_t AudioALSAHardware::setParameters(const String8 &keyValuePairs) {
    String8 newKeyValuePairs = trimMacAddress(keyValuePairs.string());
    if(!newKeyValuePairs.isEmpty() && user_load==true)
        ALOGD("+%s(): %s", __FUNCTION__, newKeyValuePairs.string());
    else
...
#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
    /* Added for Ultrasound */
    if (param.getInt(String8("ultrasound-proximity"), value) == NO_ERROR) {
        UltrasoundOutOfCallManager *pUltrasoundOutOfCallManager = UltrasoundOutOfCallManager::GetInstance();
        UltrasoundUsecaseInfo usecase;

        pUltrasoundOutOfCallManager->GetUsecaseInfo(param, usecase);
        if (usecase.IsApplicable()) {
            status = pUltrasoundOutOfCallManager->HandleUsecaseRequest(usecase);
            /* Related to ultrasound usecase. No need to do more processing here. */
            goto SET_PARAM_EXIT;
        }
    }
#endif
    ...
    if (isAdspOptionEnable() &&
        (param.getInt(keyAdspPhoneCallEnh, value) == NO_ERROR)) {
        param.remove(keyAdspPhoneCallEnh);
        SpeechDriverOpenDSP::GetInstance(MODEM_1)->setAdspPhoneCallEnh(value);
    }
#endif
    ...
    if (param.getInt(String8("SET_MODE"), value) == NO_ERROR) {
        ...
        case 4: {
            setMode(AUDIO_MODE_CALL_SCREEN);
            mStreamManager->getStreamOut(0)->setParameters(String8("routing=1"));
            break;
        }
        default: {
...
#if defined(HAVE_SW_MIXER)
    if (param.getInt(String8("HAL_MIXER_LOG_MASK"), value) == NO_ERROR) {
        param.remove(String8("HAL_MIXER_LOG_MASK"));
        set_sw_mixer_log_mask(value);
    }
#endif

    if (param.getInt(String8("PLB_TS_ON"), value) == NO_ERROR) {
        param.remove(String8("PLB_TS_ON"));
        setPlaybackTimestampLogOn(value);
    }

    if (param.getInt(keySetAudioHALDump_DL, value) == NO_ERROR) {
        param.remove(keySetAudioHALDump_DL);
        ALOGD("%s(), Set Audio HAL Dump DL = %d", __FUNCTION__, value);

        char valueDump[PROPERTY_VALUE_MAX];
        property_get("vendor.streamout.pcm.dump", valueDump, "0");
        int isDumpEnabled = atoi(valueDump);

        if (!isDumpEnabled && value) { /* audiodump from disable to enable */
            property_set(streamout_propty, "1");
            mStreamManager->dynamicSetAudioDump(AUDIO_DUMP_DOWNLINK);
        }
    }

    if (param.getInt(keySetAudioHALDump_UL, value) == NO_ERROR) {
        param.remove(keySetAudioHALDump_UL);
        ALOGD("%s(), Set Audio HAL Dump UL = %d", __FUNCTION__, value);
        ...
        if (!isDumpEnabled && value) { /* audiodump from disable to enable */
            ...
            mStreamManager->dynamicSetAudioDump(AUDIO_DUMP_UPLINK);
        }
    }
    if (param.getInt(keySetDumpAudioStreamOut, value) == NO_ERROR) {
        param.remove(keySetDumpAudioStreamOut);
        ALOGD("%s(), DUMP_AUDIO_STREAM_OUT(%d)", __FUNCTION__, value);
    ...
    if (param.getInt(keySetDumpA2DPStreamOut, value) == NO_ERROR) {
        param.remove(keySetDumpA2DPStreamOut);
        ALOGD("%s(), DUMP_A2DP_STREAM_OUT(%d)", __FUNCTION__, value);
        if (value == 0) {
    ...
    if (param.getInt(keySetDumpAudioStreamIn, value) == NO_ERROR) {
        param.remove(keySetDumpAudioStreamIn);
        ALOGD("%s(), DUMP_AUDIO_STREAM_IN(%d)", __FUNCTION__, value);
    ...
    if (param.getInt(keySetDumpApSpeechEPL, value) == NO_ERROR) {
        param.remove(keySetDumpApSpeechEPL);
            ALOGD("%s(), DUMP_AP_SPEECH_EPL(%d)", __FUNCTION__, value);
            if (value == 0) {
...
#ifdef MTK_AURISYS_FRAMEWORK_SUPPORT
            if (value == 0) {
                aurisys_set_parameter(keyAurisysLibDumpDisable.string());
#ifdef MTK_AUDIODSP_SUPPORT
                if (isAdspOptionEnable()) {
                    aurisys_set_parameter(keyAurisysLibDspVoIPDumpDisable.string());
                    aurisys_set_parameter(keyAurisysLibDspRecordDumpDisable.string());
                    aurisys_set_parameter(keyAurisysLibDspRecordFastDumpDisable.string());
                }
#endif
            } else {
                aurisys_set_parameter(keyAurisysLibDumpEnable.string());
#ifdef MTK_AUDIODSP_SUPPORT
                if (isAdspOptionEnable()) {
                    aurisys_set_parameter(keyAurisysLibDspVoIPDumpEnable.string());
                    aurisys_set_parameter(keyAurisysLibDspRecordDumpEnable.string());
                    aurisys_set_parameter(keyAurisysLibDspRecordFastDumpEnable.string());
                }
#endif
            }
#endif
...
#if defined(MTK_AUDIO_DSP_SERVICE)
    if (param.getInt(String8("IPI"), value) == NO_ERROR) {
        param.remove(String8("IPI"));
        ...
        for(int idx = 0; idx < buf_size; idx++) {
            buf[idx] = (uint8_t)((value + idx) & 0xFF);
        }
        for (int task = TASK_SCENE_AUDIO_CONTROLLER_HIFI3_A; task <= TASK_SCENE_AUDIO_CONTROLLER_HIFI3_B; task++) {
            mAudioMessengerIPI->registerAdspFeature(AUDIO_CONTROLLER_FEATURE_ID);
            ...
            mAudioMessengerIPI->sendIpiMsg(
                               ...
                               0,
                               buf);
            mAudioMessengerIPI->deregisterAdspFeature(AUDIO_CONTROLLER_FEATURE_ID);
        }
    }
#endif
    ...
    if (param.getInt(String8("IPC_TEST"), value) == NO_ERROR) {
        ALOGD("%s(), IPC_TEST=%d IPC Loopback", __FUNCTION__, (int)value);
        LoopbackManager::GetInstance();
        SpeechDriverFactory::GetInstance()->GetSpeechDriver()->sendIpcCmd((int)value);
        param.remove(String8("IPC_TEST"));
    }

#if defined(HAVE_IEMS) && defined(HAVE_SW_MIXER)
    if (param.getInt(keyHalIEM, value) == NO_ERROR) {
        param.remove(keyHalIEM);
        if (value) {
            AudioIEMsController::getInstance()->enable();
        } else {
            AudioIEMsController::getInstance()->disable();
        }
    }
#endif
...
#if defined(MTK_AUDIODSP_SUPPORT)
    if (isAdspOptionEnable() &&
        (param.getInt(keyEarFeedbackInDSP, value) == NO_ERROR)) {
    ...
    if (param.getInt(keyMuteBGS_DLVoice, value) == NO_ERROR) {
        ...
        mSpeechPhoneCallController->muteDlForEnforcedAudibleSound(value != 0);
    }
    ...
    // Set SpeechCall Dynamic DL Mute (true) / Unmute(false)
    if (param.getInt(keySet_SpeechCall_Dynamic_DL_Mute, value) == NO_ERROR) {
        param.remove(keySet_SpeechCall_Dynamic_DL_Mute);
        ALOGD("%s(), %s", __FUNCTION__, keyValuePairs.string());
        mSpeechPhoneCallController->setDynamicDlMute((bool)value);
    }
...
#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
        AudioALSAUltrasoundOutOfCallController::getInstance()->updateDeviceConnectionState(device, true);
#endif
...
#ifdef PRIMARY_USB
            AudioUSBCenter::getInstance()->setUSBOutConnectionState(device, true, usb_card, usb_device);
#endif
...
#ifdef PRIMARY_USB
            AudioUSBCenter::getInstance()->setUSBInConnectionState(device, true, usb_card, usb_device);
#endif
        }

#if defined(HAVE_IEMS) && defined(HAVE_SW_MIXER)
        AudioIEMsController::getInstance()->connectDevice(device);
#endif
...
#if defined(HAVE_IEMS) && defined(HAVE_SW_MIXER)
        AudioIEMsController::getInstance()->disconnectDevice(device);
#endif

        mStreamManager->updateDeviceConnectionState(device, false);

#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
        AudioALSAUltrasoundOutOfCallController::getInstance()->updateDeviceConnectionState(device, false);
#endif
...
#ifdef PRIMARY_USB
            AudioUSBCenter::getInstance()->setUSBOutConnectionState(device, false, -1, -1);
#endif
        } else if (audio_is_usb_in_device(device)) {
            AudioUSBPhoneCallController::getInstance()->setUSBInConnectionState(device, false, -1, -1);
#ifdef PRIMARY_USB
            AudioUSBCenter::getInstance()->setUSBInConnectionState(device, false, -1, -1);
#endif
        }
        param.remove(String8("card"));
        param.remove(String8("device"));
    ...
    if (param.getInt(keyHDREC_SET_VOICE_MODE, value) == NO_ERROR) {
        ...
        memset((void *)&hdRecordSceneTable, 0, sizeof(hdRecordSceneTable));
    ...
    if (param.get(keySetBTMode, value_str) == NO_ERROR) {
        ...
        if (value_str == "on") {
            ...
            AudioBTCVSDControl::getInstance()->BT_SCO_SetMode(true);
            mSpeechPhoneCallController->setBTMode(true);
            mStreamManager->SetBtCodec(1);
        } else if (value_str == "off") {
            set_uint32_to_mixctrl(PROPERTY_KEY_BTSCO_WB_ON, 0);
            WCNChipController::GetInstance()->SetBTCurrentSamplingRateNumber(8000);
            AudioBTCVSDControl::getInstance()->BT_SCO_SetMode(false);
            mSpeechPhoneCallController->setBTMode(false);
            mStreamManager->SetBtCodec(0);
        }
        ...
        if (appIsFeatureOptionEnabled("MTK_BESLOUDNESS_RUN_WITH_HAL") == true) {
            ...
            if (isAdspOptionEnable()) {
                ...
                snprintf(strdspbuffer, sizeof(strdspbuffer),
                         "AURISYS_SET_PARAM,DSP,ALL,MTKBESSOUND,KEY_VALUE,SetBesLoudnessStatus,%d=SET", value);
                aurisys_set_parameter(strdspbuffer + strlen(aurisys_set_key_prefix) + 1);
            }
#endif
            char strbuffer [100];
            snprintf(strbuffer, sizeof(strbuffer), "AURISYS_SET_PARAM,HAL,ALL,MTKBESSOUND,KEY_VALUE,SetBesLoudnessStatus,%d=SET", value);
...
#endif
    {
        if (param.getInt(keyDualMicRecPly, value) == NO_ERROR) {
            unsigned short cmdType = (unsigned short)value & 0x000F;
            bool bWB = ((unsigned short)value >> 4) & 0x000F;
    ...
    if (param.getInt(keyBargeInForceOff, value) == NO_ERROR) {
        param.remove(keyBargeInForceOff);
        AudioALSAVoiceWakeUpController::getInstance()->setBargeInForceOff((bool)value);
    }
#endif

#ifdef MTK_ADSP_VA_SUPPORT
    // Voice Wake Up
    if (param.getInt(keyMTK_ADSP_VA_ENABLE, value) == NO_ERROR) {
        param.remove(keyMTK_ADSP_VA_ENABLE);
        //ADSP_TODO need move this to stream manager
        AudioALSAAdspVaController::getInstance()->setAdspEnable(value);
    }
    if (param.getInt(keyMTK_ADSP_VA_DETECT_ENABLE, value) == NO_ERROR) {
        param.remove(keyMTK_ADSP_VA_DETECT_ENABLE);
    ...
    if (param.getInt(keySetRttCallType, value) == NO_ERROR) {
        ...
        mSpeechPhoneCallController->setRttMode(value);
    }
#endif
    ...
    if (isAdspOptionEnable() &&
        (param.get(keySetMP3PCM_DUMP, value_str) == NO_ERROR)) {
        mStreamManager->setParametersToStreamOut(keyValuePairs);
        param.remove(keySetMP3PCM_DUMP);
    }
    if (isAdspOptionEnable() &&
        (param.get(kKeySampleRate, value_str) == NO_ERROR)) {
        ALOGD("%s(), kKeySampleRate", __FUNCTION__);
        mStreamManager->setParametersToStreamOut(keyValuePairs);
        param.remove(kKeySampleRate);
    }
    if (isAdspOptionEnable() &&
        (param.get(kKeyBitRate, value_str) == NO_ERROR)) {
        ALOGD("%s(), kKeyBitRate", __FUNCTION__);
        mStreamManager->setParametersToStreamOut(keyValuePairs);
        param.remove(kKeyBitRate);
    }
    if (isAdspOptionEnable() &&
        (param.get(keyDspParam, value_str) == NO_ERROR)) {
        ALOGD("%s(), DSPPARAM", __FUNCTION__);
        AudioDspStreamManager::getInstance()->setParameters(value_str);
        param.remove(keyDspParam);
    }
#endif
    ...
    if (param.get(keyCallNbQuality, value_str) == NO_ERROR) {
        if (value_str == "true") {
            mSpeechPhoneCallController->setParam(String8("CUSTOM_INFO=NbQuality=1"));
        } else {
            mSpeechPhoneCallController->setParam(String8("CUSTOM_INFO=NbQuality=0"));
        }
        param.remove(keyCallNbQuality);
    }

    if (param.get(keyCallFwdPath, value_str) == NO_ERROR) {
        if (value_str == "true") {
            mStreamManager->forceTelephonyTX(true);
        } else if (value_str == "false") {
            mStreamManager->forceTelephonyTX(false);
        }
        param.remove(keyCallFwdPath);
    }

    if (param.get(keyCallMemoState, value_str) == NO_ERROR) {
        if (value_str == "true") {
            mStreamManager->callMemo(CALL_MEMO_STATE_ON);
        } else if (value_str == "false") {
            mStreamManager->callMemo(CALL_MEMO_STATE_INIT);
        } else if (value_str == "recording") {
            mStreamManager->callMemo(CALL_MEMO_STATE_REC);
        }
        param.remove(keyCallMemoState);
    }

    if (param.get(keyEnforcedActive, value_str) == NO_ERROR) {
        if (value_str == "true") {
            mStreamManager->setMicMute(true);
            mSpeechPhoneCallController->muteDlForEnforcedAudibleSound(true);
        } else if (value_str == "false") {
            mStreamManager->setMicMute(false);
            mSpeechPhoneCallController->muteDlForEnforcedAudibleSound(false);
        }
        param.remove(keyEnforcedActive);
    }

#if defined(MTK_TC10_FEATURE)
    if (param.get(keyCallState, value_str) == NO_ERROR) {
#if defined(NON_SCENE_3GVT_VOWIFI_SUPPORT)
        if (value_str == "32") {
            mStreamManager->SetVxWifiOn(true);
            mStreamManager->Set3GVTModeOn(false);
        } else if (value_str == "8192") {
            mStreamManager->SetVxWifiOn(false);
            mStreamManager->Set3GVTModeOn(true);
        } else if (value_str == "0x00000010" || value_str == "16") {
            mStreamManager->SetVxWifiOn(false);
            mStreamManager->Set3GVTModeOn(false);
        }
#else
...
#endif
        updateCustomSceneForVoIP();
        param.remove(keyCallState);
    }

    if (param.get(keyCallBand, value_str) == NO_ERROR) {
#if defined(NON_SCENE_3GVT_VOWIFI_SUPPORT)
        if (value_str == "wb") {
            mStreamManager->SetVxWifiWBMode(true);
        } else if (value_str == "nb") {
            mStreamManager->SetVxWifiWBMode(false);
        }
#else
        if (value_str == "wb") {
            mIsVoWifiWbAmr = true;
            ...
            mIsVoWifiWbAmr = false;
            mIsVoWifiSwbAmr = false;
        } else if (value_str == "swb") {
            mIsVoWifiWbAmr = false;
            mIsVoWifiSwbAmr = true;
        }
#endif
        updateCustomSceneForVoIP();
        param.remove(keyCallBand);
    }
#endif // MTK_TC10_FEATURE
    ...
    if (isAdspOptionEnable() &&
        (param.get(keyA2dpSuspended, value_str) == NO_ERROR)) {
        if (value_str == "true") {
            ALOGD("%s() setA2dpSuspend true", __FUNCTION__);
            AudioDspStreamManager::getInstance()->setA2dpSuspend(true);
        } else if (value_str == "false") {
            ALOGD("%s()  setA2dpSuspend false", __FUNCTION__);
            AudioDspStreamManager::getInstance()->setA2dpSuspend(false);
        }
    ...
    // a2dp Reconfigure
    if (param.get(String8(AUDIO_PARAMETER_RECONFIG_A2DP), value_str) == NO_ERROR) {
        if (value_str == "true") {
            ALOGD("%s() Reconfigure value_str=%s", __FUNCTION__, value_str.string());
        } else if (value_str == "false") {
            ALOGD("%s() Reconfigure value_str=%s", __FUNCTION__, value_str.string());
        }
        param.remove(String8(AUDIO_PARAMETER_RECONFIG_A2DP));
    }

    if (param.get(keyHDRRecord, value_str) == NO_ERROR) {
        if (value_str == "on") {
            mStreamManager->setHDRRecord(true);
        } else if (value_str == "off") {
            mStreamManager->setHDRRecord(false);
        }
        param.remove(keyHDRRecord);
    }
    ...
    // for ASHA HAL UT without device
    if (param.getInt(keySET_FORCE_ASHA_HAL_TEST_ENABLE, value) == NO_ERROR) {
        if (appIsFeatureOptionEnabled("MTK_BT_HEARING_AID_SUPPORT")) {
            ALOGD("%s(), Force ASHA HAL test = %d", __FUNCTION__, value);
            set_uint32_to_property("persist.vendor.audiohal.asha_test", value);
            mStreamManager->setSoftwareBridgeEnable(value);
            if (value == 1) {
                mStreamManager->routingOutputDevicePhoneCall(AUDIO_DEVICE_OUT_HEARING_AID);
            } else {
                mStreamManager->routingOutputDevicePhoneCall(AUDIO_DEVICE_OUT_EARPIECE);
            }
            param.remove(keySET_FORCE_ASHA_HAL_TEST_ENABLE);
        }
    }
#endif

    if (param.getInt(keySet_SpeechRec_DL_Pos, value) == NO_ERROR) {
        ALOGD("%s(), Set Phonecall Record DL position = %d", __FUNCTION__, value);
        SpeechDataProcessingHandler::getInstance()->setRecDlPos((RecordPositionDlType)value);
        param.remove(keySet_SpeechRec_DL_Pos);
    }
#if defined(MTK_COMBO_MODEM_SUPPORT)
    if (param.getInt(keySet_ScreenCall_Data_Type, value) == NO_ERROR) {
        ALOGD("%s(), Set Screen Call data type = %d (0:replace, 1:mix)", __FUNCTION__, value);
        SpeechPcmMixerTelephonyTx::GetInstance()->setTelephonyTxType((uint16_t)value);
        param.remove(keySet_ScreenCall_Data_Type);
    }
#endif
...
String8 AudioALSAHardware::getParameters(const String8 &keys) {
    ALOGD_IF(mLogEnable, "+%s(), key = %s", __FUNCTION__, keys.string());
    ...
    if (strncmp(keySpeechParserGetParam,
        ...
        if (pSpeechDriver == NULL) {
            ALOGE("%s(), NULL pSpeechDriver", __FUNCTION__);
        } else {
            const char *retParam = pSpeechDriver->getParameter(keys.string());
            if (retParam == NULL) {
                ALOGE("%s(), keys.string: %s, getParameter Fail! retParam == NULL!", __FUNCTION__, keys.string());
            } else {
                returnParam.add(keys, String8(retParam));
            }
        }
        goto GET_PARAM_EXIT;
    }
...
#if defined(HAVE_SW_MIXER)
    if (param.get(String8("HAL_MIXER_LOG_MASK"), value) == NO_ERROR) {
        param.remove(String8("HAL_MIXER_LOG_MASK"));
        returnParam.add(String8("HAL_MIXER_LOG_MASK"), String8(get_sw_mixer_log_mask()));
    }
#endif
    ...
    if (param.get(keyFmPlaybackToSpeakerUsingIndirect, value) == NO_ERROR) {
        param.remove(keyFmPlaybackToSpeakerUsingIndirect);
#if defined(MTK_AUDIODSP_SUPPORT)
        if (isAdspOptionEnable() &&
            AudioDspStreamManager::getInstance()->getDspVersion() == DSP_VER_SW_MIX &&
            AudioDspStreamManager::getInstance()->getDspFmEnable() == true) {
            value = "false";
        } else
#else
        {
            if (AudioSmartPaController::getInstance()->isSwDspSpkProtect(AUDIO_DEVICE_OUT_SPEAKER) ||
                AudioALSAHardwareResourceManager::getInstance()->isSpkApSwMixSupport(AUDIO_DEVICE_OUT_SPEAKER)) {
                value = "true";
            } else {
                value = "false";
            }
...
#endif
        returnParam.add(keyFmPlaybackToSpeakerUsingIndirect, value);
    }
    ...
    if (param.get(keyMTK_BESLOUDNESS_RUN_WITH_HAL, value) == NO_ERROR) {
        ...
        bool foValue_MTK_BESLOUDNESS_RUN_WITH_HAL = appIsFeatureOptionEnabled("MTK_BESLOUDNESS_RUN_WITH_HAL");
        if (foValue_MTK_BESLOUDNESS_RUN_WITH_HAL) {
            value = "true";
        } else {
            value = "false";
        }
    ...
    if (param.get(keyMTK_VOW_SUPPORT, value) == NO_ERROR) {
        ...
        if (appIsFeatureOptionEnabled("MTK_VOW_SUPPORT")) {
            value = "true";
        } else {
            value = "false";
        }
    ...
    if (param.get(keyMTK_VOW_AMAZON_SUPPORT, value) == NO_ERROR) {
        param.remove(keyMTK_VOW_AMAZON_SUPPORT);
        if (appIsFeatureOptionEnabled("MTK_VOW_AMAZON_SUPPORT")) {
            value = "true";
        } else {
            value = "false";
        }
        returnParam.add(keyMTK_VOW_AMAZON_SUPPORT, value);
    }

    if (param.get(keyMTK_VOW_MULTI_KEYPHRASE_SUPPORT, value) == NO_ERROR) {
        param.remove(keyMTK_VOW_MULTI_KEYPHRASE_SUPPORT);
        if (appIsFeatureOptionEnabled("MTK_VOW_MULTI_KEYPHRASE_SUPPORT")) {
            value = "true";
        } else {
            value = "false";
        }
        returnParam.add(keyMTK_VOW_MULTI_KEYPHRASE_SUPPORT, value);
    }

    if (param.get(keyMTK_VOW_MAX_PDK_NUMBER, value) == NO_ERROR) {
        int maxPdkNumber = 0;
        /* Get AudioParamOptions.xml Project config */;
        const char *maxPdkNumberString = appGetFeatureOptionValue("MTK_VOW_MAX_PDK_NUMBER");
        maxPdkNumber = atoi(maxPdkNumberString);
        ALOGD("%s(), maxPdkNumber = %d", __FUNCTION__, maxPdkNumber);
        returnParam.addInt(keyMTK_VOW_MAX_PDK_NUMBER, maxPdkNumber);
    }

    if (param.get(keyMTK_VOW_COMMAND, value) == NO_ERROR) {
        param.remove(keyMTK_VOW_COMMAND);
#ifdef MTK_VOW_SUPPORT
        value = AudioALSAVoiceWakeUpController::getInstance()->getVoiceCommand(VOICE_COMMAND_1ST);
#else
        value = "none";
#endif
...
#ifdef MTK_VOW_SUPPORT
        value = AudioALSAVoiceWakeUpController::getInstance()->getVoiceCommand(VOICE_COMMAND_2ND);
#else
        value = "none";
#endif
...
#ifdef MTK_VOW_SUPPORT
        value = AudioALSAVoiceWakeUpController::getInstance()->getVoiceCommand(VOICE_COMMAND_3RD);
#else
        value = "none";
#endif
    ...
    if (param.get(keyMTK_VOW_DSP_LIB_VERSION, value) == NO_ERROR) {
        param.remove(keyMTK_VOW_DSP_LIB_VERSION);
#ifdef MTK_VOW_SUPPORT
        if (appIsFeatureOptionEnabled("MTK_VOW_AMAZON_SUPPORT")) {
            value = AudioALSAVoiceWakeUpController::getInstance()->getAlexaDspLibVersion();
        } else {
            value = "none";
        }
#else
        value = "none";
#endif
        ALOGD("%s(), get MTK_VOW_DSP_LIB_VERSION = %s", __FUNCTION__, value.string());
        returnParam.add(keyMTK_VOW_DSP_LIB_VERSION, value);
    }
    ...
    if (AudioALSAHardwareResourceManager::getInstance()->getNumPhoneMicSupport() >= 2) {
        if (strncmp(keyDUALMIC_GET_GAIN,
            ...
            if (mAudioALSAParamTunerInstance->getDMNRGain((unsigned short)cmdType, &gain) == NO_ERROR) {
                ret_str = snprintf(buf, sizeof(buf), "%d", gain);
                if (ret_str >= 0 && ret_str < sizeof(buf)) {
    ...
    if (strncmp(keyMTK_VOW_MIC_TYPE,
        ...
        ret_str = snprintf(buf, sizeof(buf), "%d", mic_type);
        if (ret_str >= 0 && ret_str < sizeof(buf)) {
    ...
    if (param.get(keyHYBRID_NLE_EOP_SET, value) == NO_ERROR) {
        ...
        param.remove(keyHYBRID_NLE_EOP_SET);
        ret_str = snprintf(buf, sizeof(buf), "%d", AudioALSAHyBridNLEManager::getInstance()->getNleEopDb());
        if (ret_str >= 0 && ret_str < sizeof(buf)) {
    ...
    if (strncmp(keyTUNING_GET_PARAM.string(),
                keys.string(),
                keyTUNING_GET_PARAM.length()) == 0) {
        value = String8(keys.string() + keyTUNING_GET_PARAM.length() + 1);
        value = getAudioLastCategoryPath(value);
        returnParam.add(keys, value);
        param.remove(keys);
    }
    ...
    if (param.get(keyGetSpeechOutFirIdxFromNvRam, value) == NO_ERROR) {
        ...
        memset((void *)&pMedPara, 0, sizeof(pMedPara));
    ...
    if (param.get(String8("hifi_samplerate"), value) == NO_ERROR) {
        param.remove(String8("hifi_samplerate"));
        char buf[32];
        int ret_str = 0;

        int sample_rate = 48000;
        sample_rate = AudioALSAHardwareResourceManager::getInstance()->getOutputDeviceSampleRate();

        ret_str = snprintf(buf, sizeof(buf), "%d", sample_rate);
        if (ret_str >= 0 && ret_str < sizeof(buf)) {
            ALOGD("%s(), hifi_samplerate = %d", __FUNCTION__, sample_rate);
            returnParam.add(String8("hifi_samplerate"), String8(buf));
        }
    }
#endif
...
#endif
    // a2dp reconfig
    if (param.get(String8(AUDIO_PARAMETER_A2DP_RECONFIG_SUPPORTED), value) == NO_ERROR) {
        value_int = 0;
#if defined(MTK_A2DP_OFFLOAD_SUPPORT)
        if (isAdspOptionEnable()) {
            value_int = 1;
...
#endif
        ALOGD("isReconfigA2dpSupported value_int = %d", value_int);
        param.remove(String8(AUDIO_PARAMETER_A2DP_RECONFIG_SUPPORTED));
        returnParam.addInt(String8(AUDIO_PARAMETER_A2DP_RECONFIG_SUPPORTED), value_int);
    }
...
#if defined(HAVE_IEMS) && defined(HAVE_SW_MIXER)
    if (param.get(keyHalIEM, value) == NO_ERROR) {
        param.remove(keyHalIEM);
        value = (AudioIEMsController::getInstance()->isIEMsEnable()) ? "1" : "0";
        returnParam.add(keyHalIEM, value);
    }
#endif

#if defined(MTK_AUDIODSP_SUPPORT)
    if (isAdspOptionEnable() &&
        (param.get(keyEarFeedbackInDSP, value) == NO_ERROR)) {
        param.remove(keyEarFeedbackInDSP);
...
status_t AudioALSAHardware::SetAudioCommand(int par1, int par2) {
    ...
    case DUMP_AUDIO_STREAM_OUT: {
        ...
        char valueDump[PROPERTY_VALUE_MAX] = {0};
        property_get("vendor.streamout.pcm.dump", valueDump, "0");
        int isDumpEnabled = atoi(valueDump);
        ...
        if (!isDumpEnabled && par2) { /* audiodump from disable to enable */
            mStreamManager->dynamicSetAudioDump(AUDIO_DUMP_DOWNLINK);
        }
...
status_t AudioALSAHardware::SetAudioCommonData(int par1, size_t len, void *ptr) {
    ...
    case AUD_TASTE_TUNING: {
        ...
        case AUD_TASTE_START: {
            ...
            audioTasteTuningParam.input_file[FILE_NAME_LEN_MAX - 1] = '\0'; // for coverity check null-terminated string
...
status_t AudioALSAHardware::GetAudioCommonData(int par1, size_t len, void *ptr) {
    ...
    case GET_AUDIO_POLICY_VOL_FROM_VER1_DATA: {
        ...
        if (len == sizeof(AUDIO_CUSTOM_VOLUME_STRUCT)) {
            ...
            if (mHACon) {
                ...
                memset((void *)&mHacParam, 0, sizeof(mHacParam));
    ...
    case VOICE_RECOG_FIR_COEF: {
        ...
        memset((void *)&custHDRECParam, 0, sizeof(custHDRECParam));
    ...
    case VOICE_RECOG_GAIN: {
        ...
        memset((void *)&custGainParam, 0, sizeof(custGainParam));
...
int AudioALSAHardware::setAudioParameterChangedCallback(device_audio_parameter_changed_callback_t callback, void *cookie) {
    ...
    Vector<AudioParameterChangedHidlCallback *>::iterator iter = mAudioParameterChangedHidlCallbackList.begin();
    if (iter != NULL) {
        for (; iter != mAudioParameterChangedHidlCallbackList.end(); iter++) {
...
int AudioALSAHardware::clearAudioParameterChangedCallback(void *cookie) {
    ...
    if (iter != NULL) {
        for (; iter != mAudioParameterChangedHidlCallbackList.end();) {
...
void AudioALSAHardware::onXmlChangedCallback(AppHandle *appHandle, const char *audioType) {
...
#if defined(MTK_AURISYS_FRAMEWORK_SUPPORT)
    int xml_value = -1;
    bool RecVoipTuning = false;
    String8 key_value_pair = String8("");
    int ret = 0;
    ...
    if (strcmp(audioType, "PlaybackACF") == 0) {
        ...
        xml_value = 1;
    } else if (strncmp(audioType, "Record", 6) == 0) {
        RecVoipTuning = true;
    } else if (strncmp(audioType, "VoIP", 4) == 0) {
        RecVoipTuning = true;
    }
    if (xml_value >= 0) {
#if defined(MTK_AUDIODSP_SUPPORT)
        if (isAdspOptionEnable()) {
            key_value_pair = String8::format("DSP,%s,%s,APPLY_PARAM,0=SET", "ALL", "MTKBESSOUND");
            aurisys_set_parameter(key_value_pair.string());
        }
#endif
        key_value_pair = String8::format("HAL,%s,%s,KEY_VALUE,UpdateACFHCFParameters,%d=SET",
    ...
    if (RecVoipTuning) {
        key_value_pair = String8::format("HAL,%s,%s,APPLY_PARAM,0=SET", "ALL", "MTKSE");
        aurisys_set_parameter(key_value_pair.string());
#if defined(MTK_AUDIODSP_SUPPORT)
        if (isAdspOptionEnable()) {
            key_value_pair = String8::format("DSP,%s,%s,APPLY_PARAM,0=SET", "ALL", "MTK_SPH");
            aurisys_set_parameter(key_value_pair.string());
        }
#endif
    }
...
int AudioALSAHardware::setParametersByCallback(const String8 &keyValuePairs) {  // Static for easy use
    if (mHwParametersCbk == NULL || mHwParametersCbkCookie == NULL) {
        ...
        unsigned int stringlen = strlen(keyValuePairs.string());
        if (stringlen + 1 > 1024) {
...
/*
 * setBluetoothAudioOffloadParam and setA2dpSuspendStatus
 * need exist when MTK_A2DP_OFFLOAD_SUPPORT is set.
 */
#ifdef MTK_A2DP_OFFLOAD_SUPPORT
...
void AudioALSAHardware::setBluetoothAudioOffloadParam(const sp<IBluetoothAudioPort> &hostIf,
    ...
    AudioDspStreamManager::getInstance()->setBluetoothAudioOffloadParam(hostIf, codecConfig, on);
...
#if defined(MTK_A2DP_OFFLOAD_SUPPORT)
    AudioDspStreamManager::getInstance()->setA2dpStatus(status);
#endif
}

#endif

bool AudioALSAHardware::UpdateOutputFIR(uint32_t mode, uint32_t index) {
...
int AudioALSAHardware::createAudioPatch(unsigned int num_sources,
    ...
    ALOGD("+%s() num_sources [%d] , num_sinks [%d], handle [0x%x], current mAudioHalPatchVector size %zu",
          __FUNCTION__, num_sources, num_sinks, (int)(*handle), mAudioHalPatchVector.size());
    ...
    do {
        ...
        if (sources[0].type == AUDIO_PORT_TYPE_MIX) {
            ...
            for (dDeviceIndex = 0; dDeviceIndex < num_sinks; dDeviceIndex++) {
                eOutDeviceList =
                        (audio_devices_t)(eOutDeviceList | sinks[dDeviceIndex].ext.device.type);
            }
            ...
            // avoid using USB_PRIMARY in HAL TODO: remove after policy refined
            if (eOutDeviceList == AUDIO_DEVICE_OUT_BUS) {
                eOutDeviceList = AUDIO_DEVICE_OUT_USB_HEADSET;
            }

#if defined(MTK_COMBO_MODEM_SUPPORT) && defined(MTK_BT_HEARING_AID_SUPPORT)
            // Hearing Aid Device Phonecall (HAP)
            if (appIsFeatureOptionEnabled("MTK_BT_HEARING_AID_SUPPORT")) {
                if (mStreamManager->getSoftwareBridgeEnable() == true && eOutDeviceList == AUDIO_DEVICE_OUT_EARPIECE) {
                    ALOGD("%s(), SoftwareBridgeEnable: %d,	Hearing Aid Device Scenario, change output device: 0x%x -> 0x%x",
                          __FUNCTION__, mStreamManager->getSoftwareBridgeEnable(), eOutDeviceList, AUDIO_DEVICE_OUT_HEARING_AID);
                    eOutDeviceList = AUDIO_DEVICE_OUT_HEARING_AID;
                }
            }
#endif


            ALOGV("%s() createAudioPatch Mixer->0x%08x, handle %d", __FUNCTION__, eOutDeviceList, *handle);
            ...
            if (sinks[0].type == AUDIO_PORT_TYPE_MIX) {
                ...
                ALOGV("%s() createAudioPatch 0x%08x->Mixer Src %x, handle %d", __FUNCTION__, eInputDeviceList, eInputSource, *handle);
                AudioParameter param;
                ...
                for (dDeviceIndex = 0; dDeviceIndex < num_sinks; dDeviceIndex++) {
                    eOutDeviceList =
                            (audio_devices_t)(eOutDeviceList | sinks[dDeviceIndex].ext.device.type); //should be only one device , limited by frameworks
                }
                ...
                ALOGD("%s() createAudioPatch device 0x%08x->0x%08x, handle %d",
                      __FUNCTION__, eInputDeviceList, eOutDeviceList, *handle);
                if (eInputDeviceList == AUDIO_DEVICE_IN_TELEPHONY_RX) {
                    status = mStreamManager->routingOutputDevicePhoneCall(eOutDeviceList);
                    ...
                    break;
                } else if (eOutDeviceList == AUDIO_DEVICE_OUT_TELEPHONY_TX) {
                    // do nothing
                    status = NO_ERROR;
                    break;
                } else if ((eInputDeviceList != AUDIO_DEVICE_IN_FM_TUNER) ||
    ...
    if (status == NO_ERROR) {
        ...
        *handle = static_cast<audio_patch_handle_t>(android_atomic_inc(&mNextUniqueId));
        ...
        ALOGV("%s() new handle [0x%x] current mAudioHalPatchVector size %zu", __FUNCTION__, (int)(*handle), mAudioHalPatchVector.size());
    } else {
        ALOGE("Fail status %d", (int)(status));
    }
    ALOGD("-%s() num_sources [%d] , num_sinks [%d], handle [0x%x]", __FUNCTION__, num_sources, num_sinks, (int)(*handle));
#endif
    return status;
}
...
int AudioALSAHardware::releaseAudioPatch(audio_patch_handle_t handle) {
    ...
    AudioHalPatch *patch;
    ALOGD("+%s() handle [0x%x] current mAudioHalPatchVector size %zu", __FUNCTION__, handle, mAudioHalPatchVector.size());
    do {
        ...
        if (patch->sources[0].type == AUDIO_PORT_TYPE_MIX) {
            ...
            ALOGD("%s() releaseAudioPatch Mixer->0x%08x, handle %d", __FUNCTION__, patch->sinks[0].ext.device.type, handle);
#if 1
            ...
            if (patch->sinks[0].type == AUDIO_PORT_TYPE_MIX) {
                ...
                ALOGD("%s() releaseAudioPatch 0x%08x->Mixer, handle %d", __FUNCTION__, patch->sources[0].ext.device.type, handle);
                if (mUseAudioPatchForFm == true) {
                ...
                if (status) {
                    ALOGW("%s(), setParameters(keyRouting) fail return %d", __FUNCTION__, status);
                    break;
                }
#ifdef MTK_AUDIODSP_SUPPORT
                if (isAdspOptionEnable()) {
                    status = mStreamManager->updateInputSource(patch->sinks[0].ext.mix.handle);
                }
#endif
            } else if (patch->sinks[0].type == AUDIO_PORT_TYPE_DEVICE) {
                ALOGD("%s() releaseAudioPatch device 0x%08x->0x%08x, handle %d", __FUNCTION__, patch->sources[0].ext.device.type, patch->sinks[0].ext.device.type, handle);
                //ALOGW("sinks[0].type == AUDIO_PORT_TYPE_DEVICE");
                if (patch->sources[0].ext.device.type == AUDIO_DEVICE_IN_TELEPHONY_RX) {
                    // do nothing
                    status = NO_ERROR;
                    break;
                } else if (patch->sinks[0].ext.device.type == AUDIO_DEVICE_OUT_TELEPHONY_TX) {
                    // do nothing
                    status = NO_ERROR;
                    break;
                } else if ((patch->sources[0].ext.device.type == AUDIO_DEVICE_IN_FM_TUNER) && (patch->sinks[0].ext.device.type & FM_DEVICE_TO_DEVICE_SUPPORT_OUTPUT_DEVICES)) {
    ...
    if (status == NO_ERROR) {
        ALOGD("%s() remove handle [%x] OK, current mAudioHalPatchVector size %zu", __FUNCTION__, handle, mAudioHalPatchVector.size());
        delete (patch);
    } else {
        ALOGD("%s() handlecheck remove handle [%x] NG", __FUNCTION__, handle);
