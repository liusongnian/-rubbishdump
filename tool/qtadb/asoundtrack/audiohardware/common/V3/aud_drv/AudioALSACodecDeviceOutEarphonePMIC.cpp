...
#define DC_TRIM_CALIBRATING_STRING "Calibrating"
    ...
    struct mixer_ctl *mixer_ctl = mixer_get_ctl_by_name(mMixer, DC_TRIM_CONTROL_SWITCH_NAME);
    if (!mixer_ctl) {
        ALOGD("%s(), mixer_ctl is not found", __FUNCTION__);
        return INVALID_OPERATION;
    }

    int dcTrimState = mixer_ctl_get_value(mixer_ctl, 0);
    ALOGD("%s(), ctl %s = %d", __FUNCTION__, DC_TRIM_CONTROL_SWITCH_NAME, dcTrimState);

    // check if calibrated
    const char *dctrim_cali_state = mixer_ctl_get_enum_string(mixer_ctl, dcTrimState);
    if (!dctrim_cali_state) {
        ALOGD("%s(), dctrim_cali_state is NULL", __FUNCTION__);
        return 0;
    }

    if (strcmp(dctrim_cali_state, DC_TRIM_CALIBRATED_STRING) == 0) {
        ALOGD("%s(), dc trimmed", __FUNCTION__);
        return 0;
    }

    // do dc calibrate
    retval = mixer_ctl_set_enum_by_string(mixer_ctl, DC_TRIM_CALIBRATING_STRING);
