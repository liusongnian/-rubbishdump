...
#define PCM_READ_MS (22500 / 1000.f)
...
void *AudioALSACaptureDataProviderBTCVSD::readThread(void *arg) {
    ...
    while (pDataProvider->mEnable == true) {
        ...
        if (pDataProvider->mPCMDumpFile) {
            ...
            ALOGV("%s, latency_in_us,%1.6lf,%1.6lf,%1.6lf,%1.6lf,%1.6lf", __FUNCTION__, pDataProvider->mTimerec[0], pDataProvider->mTimerec[1], pDataProvider->mTimerec[2], pDataProvider->mTimerec[3], pDataProvider->mTimerec[4]);
        }
    }
...
uint32_t AudioALSACaptureDataProviderBTCVSD::readDataFromBTCVSD(void *linear_buffer, bool *isMuteData, struct timespec *bufferTimeStamp) {
    ...
    *isMuteData = false;
    ...
    ASSERT(mPcm != NULL);
    if (mPcm == NULL) {                                                            
        ALOGE("%s(), mPcm = NULL, skip pcm read!!!", __FUNCTION__);
    } else {
        ...
        if (!mBTIrqReceived) {
            ...
            ALOGW("%s(), mBTIrqReceived = %d, returnMuteDataMs =%f",
                  __FUNCTION__, mBTIrqReceived, returnMuteDataMs);
        } else {
            // check if enough data in kernel
            struct timespec timeStamp;
            unsigned int avail;
            if (pcm_get_htimestamp(mPcm, &avail, &timeStamp) != 0) {
                ALOGV("%s(), pcm_get_htimestamp fail %s", __FUNCTION__, pcm_get_error(mPcm));
                bool btscoOn = AudioALSAHardware::GetInstance()->getBtscoOn();
                if (!btscoOn) {
                    ALOGD("%s(), btscoOn %d, pcm stop", __FUNCTION__, btscoOn);
                    pcm_stop(mPcm);
                    return 0;
                }
            } else {
                unsigned int numFrame = BTSCO_CVSD_RX_TEMPINPUTBUF_SIZE / (mConfig.channels * pcm_format_to_bits(mConfig.format) / 8);
                if (avail < numFrame) {
                    bool btscoOn = AudioALSAHardware::GetInstance()->getBtscoOn();
                    ALOGD("%s(), skip, wait next read: avail %u < %u  frames (%u bytes), btscoOn %d",
                          __FUNCTION__, avail, numFrame, BTSCO_CVSD_RX_TEMPINPUTBUF_SIZE, btscoOn);
                    if (btscoOn) {
            ...
            if (returnMuteDataMs == 0) {
                ...
                if (retval != 0) {
                    ...
                    ALOGE("%s(), pcm_read() error, retval = %d, fail due to %s",
                          __FUNCTION__, retval, pcm_get_error(mPcm));
                    returnMuteDataMs = BTSCO_CVSD_RX_INBUF_SIZE / 8.0f;
                } else {
                    ...
                    ALOGV("%s(), BTCVSD Rx Timeout Switch, rx_timeout %d, index %d",
                          __FUNCTION__, rx_timeout, index);
                    if (rx_timeout) {
