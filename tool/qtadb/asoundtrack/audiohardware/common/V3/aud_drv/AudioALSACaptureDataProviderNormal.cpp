...
status_t AudioALSACaptureDataProviderNormal::open() {
...

    {
        ...
        mConfig.rate = getInputSampleRate(mStreamAttributeSource.input_device,
    ...
    // Reset frames readed counter
        ...
        if (mConfig.channels == 4 && !bHDRRecord) {
        ...
        if (isNeedSyncPcmStart() == true) {
            ...
            maxSize = getPeriodBufSize(&mStreamAttributeSource, UPLINK_MAX_LATENCY_MS);
            if (checkSramSize > maxSize) {
                checkSramSize = maxSize;
                ALOGD("only allocate maxSize = %d of sram", checkSramSize);
            }
        ...
        if (mlatency == UPLINK_LOW_LATENCY_MS) { // fast use same kernel buffer size as normal
            mConfig.period_count = mConfig.period_count * UPLINK_NORMAL_LATENCY_MS / UPLINK_LOW_LATENCY_MS;
        }
#endif
    ...
    if (isAdspOptionEnable() &&
        mStreamAttributeSource.input_device == AUDIO_DEVICE_IN_WIRED_HEADSET &&
...
void *AudioALSACaptureDataProviderNormal::readThread(void *arg) {
    ...
    char nameset[32];
    if (snprintf(nameset, sizeof(nameset), "%s%d", __FUNCTION__, pDataProvider->mCaptureDataProviderType) < 0) {
        ALOGE("%s(), snprintf nameset fail!!", __FUNCTION__);
    }
    ...
    char *linear_buffer = NULL;
    uint32_t Read_Size = kReadBufferSize;
    pDataProvider->mNewBufSize = kReadBufferSize;

    linear_buffer = (char *)malloc(kReadBufferSize);
    if (linear_buffer == NULL) {
        ALOGE("%s(), create linear buffer fail!", __FUNCTION__);
    ...
    while (pDataProvider->mEnable == true) {
        ...
        if (kReadBufferSize != pDataProvider->mNewBufSize) {
            ...
            linear_buffer = (char *)malloc(kReadBufferSize);
            if (linear_buffer == NULL) {
                ALOGE("%s(), create linear buffer fail! return", __FUNCTION__);
                break;
            }
        }
        ...
        double logTimeout = (double)pDataProvider->mlatency / 1000;
