...
#endif
    ALOGV("AudioBTCVSDControl constructor");
...
void AudioBTCVSDControl::BT_SCO_CVSD_Init(void) {
    ...
    ALOGV("BT_SCO_CVSD_Init() allocate mBTSCOCVSDContext");
}
...
void AudioBTCVSDControl::BT_SCO_CVSD_DeInit(void) {
    if (mBTSCOCVSDContext) {
        ...
        mBTSCOCVSDContext = NULL;
        ALOGV("BT_SCO_CVSD_DeInit() release mBTSCOCVSDContext");
    }
}
...
void AudioBTCVSDControl::BT_SCO_TX_Open(void) {
    ...
    retTime = time(&rawtime);
    struct tm *timeinfo = (retTime != -1) ? localtime(&rawtime) : NULL;
    if (timeinfo != NULL) {
        char timeBufStr[128];
        retStrftime = strftime(timeBufStr, sizeof(timeBufStr), "CVSDTXOut_%Y_%m_%d_%H%M%S.pcm", timeinfo);
        if (retStrftime != 0) {
            dumpFileName.appendFormat("%s%s", audio_dump_path, timeBufStr);
        } else {
            dumpFileName.appendFormat("%s%s", audio_dump_path, "CVSDTXOut.pcm");
        }
    } else {
        dumpFileName.appendFormat("%s%s", audio_dump_path, "CVSDTXOut.pcm");
    }
    mTXSRCPCMDumpFile = NULL;
    mTXSRCPCMDumpFile = AudioOpendumpPCMFile(dumpFileName, streamout_propty);
...
void AudioBTCVSDControl::btsco_process_RX_CVSD(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize, uint8_t packetvalid) {
    ...
    *insize = consumed;
    ...
    if (mBTCVSDRXInDumpFile) {
        if (fwrite((void *)inbuf, 1, *insize, mBTCVSDRXInDumpFile) != *insize) {
            ALOGW("%s(), fwrite error", __FUNCTION__);
        }
    }
    ...
    if (mBTCVSDRXDumpFile) {
        if (fwrite((void *)outbuf, 1, *outsize, mBTCVSDRXDumpFile) != *outsize) {
            ALOGW("%s(), fwrite error", __FUNCTION__);
        }
    }
...
void AudioBTCVSDControl::btsco_process_RX_MSBC(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize,
    ...
    if ((mBTSCOCVSDContext->pRX->PacketBuf[index1][0] == 0x1) &&
        ...
        if (mBTCVSDRXInDumpFile) {
            if (fwrite((void *)mBTSCOCVSDContext->pRX->EntirePacket, 1, inBytePacket, mBTCVSDRXInDumpFile) != inBytePacket) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
        }
        ...
        if (mBTCVSDRXDumpFile) {
            if (fwrite((void *)workbuf, 1, iOutSample << 1, mBTCVSDRXDumpFile) != (iOutSample << 1)) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
        }
...
void AudioBTCVSDControl::btsco_process_TX_CVSD(void *inbuf, uint32_t *insize, void *outbuf, uint32_t *outsize, void *workbuf, const uint32_t workbufsize) {
    ...
    if (*insize != 0 && *outsize != 0) {
        ...
        if (mTXSRCPCMDumpFile) {
            if (fwrite((void *)workbuf, 1, src_outsize, mTXSRCPCMDumpFile) != src_outsize) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
        }
...
void AudioBTCVSDControl::BT_SCO_RX_Open(void) {
    ...
    time_t rawtime;
    String8 dumpFileName, dumpRxInFileName;
    retTime = time(&rawtime);
    struct tm *timeinfo = (retTime != -1) ? localtime(&rawtime) : NULL;
    if (timeinfo != NULL) {
        char timeBufStr[128];
        retStrftime = strftime(timeBufStr, sizeof(timeBufStr), "CVSDRXOut_%Y_%m_%d_%H%M%S.pcm", timeinfo);
        if (retStrftime != 0) {
            dumpFileName.appendFormat("%s%s", audio_dump_path, timeBufStr);
        } else {
            dumpFileName.appendFormat("%s%s", audio_dump_path, "CVSDRXOut.pcm");
        }
        retStrftime = strftime(timeBufStr, sizeof(timeBufStr), "CVSDRXIn_%Y_%m_%d_%H%M%S.dat", timeinfo);
        if (retStrftime != 0) {
            dumpRxInFileName.appendFormat("%s%s", audio_dump_path, timeBufStr);
        } else {
            dumpRxInFileName.appendFormat("%s%s", audio_dump_path, "CVSDRXIn.dat");
        }
    } else {
        dumpFileName.appendFormat("%s%s", audio_dump_path, "CVSDRXOut.pcm");
        dumpRxInFileName.appendFormat("%s%s", audio_dump_path, "CVSDRXIn.dat");
    ...
    mBTCVSDRXDumpFile = NULL;
    mBTCVSDRXDumpFile = AudioOpendumpPCMFile(dumpFileName, streamin_propty);
    mBTCVSDRXInDumpFile = NULL;
    mBTCVSDRXInDumpFile = AudioOpendumpPCMFile(dumpRxInFileName, streamin_propty);
...
void AudioBTCVSDControl::AudioExtMDCVSDThread::ExtMD_btsco_cvsd_UL_Write_main(void) {
    ...
    uint8_t *wtmpbuf = NULL, *wtmpbuf2 = NULL, *pbuf1 = NULL, *pbuf2 = NULL;
    uint16_t *pSrc = NULL, *pDst = NULL, threshold, samplecount;
...
void AudioBTCVSDControl::AudioExtMDCVSDThread::ExtMD_btsco_cvsd_DL_Write_main(void) {
    ...
    size_t outputSize = 0;
    uint8_t *outbuffer = NULL, *inbuf = NULL, *workbuf = NULL, i;
...
void AudioBTCVSDControl::AudioBTCVSDLoopbackRxThread::btsco_cvsd_RX_main(void) {
    ...
    offset = 0;
    dump_size = 0;
    uint8_t *pWriteBuffer = new uint8_t[outsize];
    do {
        packetvalid = *((char *)tempbuf + SCO_RX_PLC_SIZE + offset + i * BTSCO_CVSD_PACKET_VALID_SIZE); //parser packet valid info for each 30-byte packet
        //packetvalid   = 1; //force packvalid to 1 for test
        memcpy(inbuf + offset, tempbuf + offset + i * BTSCO_CVSD_PACKET_VALID_SIZE, SCO_RX_PLC_SIZE);
