...
AudioALSAFMController::AudioALSAFMController() :
    ...
    mPcm(NULL),
    mPcmUL(NULL),
    mOutput_device(AUDIO_DEVICE_NONE),
...
#if defined(MTK_AUDIODSP_SUPPORT) && defined(MTK_AUDIO_KS)
    if (isAdspOptionEnable()) {
        if ((AudioDspStreamManager::getInstance()->getDspVersion() == DSP_VER_SW_MIX) &&
            (AudioSmartPaController::getInstance()->isSwDspSpkProtect(AUDIO_DEVICE_OUT_SPEAKER))) {
            setFmAdspEnable(true);
        } else {
            setFmAdspEnable(false);
        }
    }
#endif
}
    ...
    if (mFmEnable == true) { // Open
        ...
        if (true == bForceControl && false == bForce2DirectConn) {
            ...
            if (isAdspOptionEnable() &&
                ...
                mFmAdspEnable == true && mIsFmAdspActive == false) {
                enableFmAdspTask(enable, output_device);
            } else
#endif
            {
                // acquire pmic clk
        ...
        if (mIsFmDirectConnectionMode == true && mIsFmAdspActive == false) {
...
#ifdef MTK_AUDIODSP_SUPPORT
        if (isAdspOptionEnable() && mFmAdspEnable == true && mIsFmAdspActive == true) {
            enableFmAdspTask(enable, output_device);
        } else
#endif
        {
...
status_t AudioALSAFMController::setFmAdspEnable(const bool enable) {
    AL_AUTOLOCK(mLock);
    ALOGD("%s(), mFmAdspEnable = %d to %d", __FUNCTION__, mFmAdspEnable, enable);
    mFmAdspEnable = enable;
    return NO_ERROR;
}
...
status_t AudioALSAFMController::routing(const audio_devices_t pre_device, const audio_devices_t new_device) {
    ...
    if (mIsFmDirectConnectionMode) {
        if (new_device & (AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_WIRED_HEADSET | AUDIO_DEVICE_OUT_WIRED_HEADPHONE)) {
            mHardwareResourceManager->stopOutputDevice();
#if defined(MTK_AUDIO_KS)
            setFmDirectConnection_l(false, true, pre_device);
            setFmDirectConnection_l(true, true, new_device);
#endif
            mHardwareResourceManager->startOutputDevice(new_device, getFmDownlinkSamplingRate());
        }
    }
    mOutput_device = new_device;
#else
    if (WCNChipController::GetInstance()->IsSupportFM() == false) {
        ALOGW("-%s(), Don't support FM in the platform", __FUNCTION__);
        return INVALID_OPERATION;
...
#ifdef MTK_AUDIODSP_SUPPORT
status_t AudioALSAFMController::enableFmAdspTask(const bool enable, const audio_devices_t output_device) {
    AL_AUTOLOCK(mEnableLock);
    ...
    if (enable == true) {
        ...
        mStreamAttributeSource.sample_rate = getFmUplinkSamplingRate();
        mStreamAttributeSource.output_devices = output_device;

        AudioALSAPlaybackHandlerFmAdsp::getInstance(&mStreamAttributeSource)->open();
    } else {
        AudioALSAPlaybackHandlerFmAdsp::getInstance(&mStreamAttributeSource)->close();
    }

    // Update Status
    mIsFmDirectConnectionMode = enable;
    mIsFmAdspActive = enable;

    // Update (HW_GAIN1) Volume
    setFmVolume_l(mFmVolume);
...
bool AudioALSAFMController::isFmAdspActive() {
    AL_AUTOLOCK(mEnableLock);
    return mIsFmAdspActive;
}
...
#ifdef MTK_ENABLE_HOSTLESS_FM
                card_index = AudioALSADeviceParser::getInstance()->GetCardIndexByString(keypcmFMHostless);
                pcm_index = AudioALSADeviceParser::getInstance()->GetPcmIndexByString(keypcmFMHostless);
