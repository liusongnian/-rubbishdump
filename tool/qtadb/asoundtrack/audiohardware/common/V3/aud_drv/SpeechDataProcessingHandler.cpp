...
static const char *PROPERTY_KEY_RECORD_DL_POSITION = "vendor.audiohal.phonecall.record.dl.position"; // 0 for property disable, so need to set enum value +1
...
SpeechDataProcessingHandler::SpeechDataProcessingHandler() {
    ...
    mTypeRecord.direction = RECORD_TYPE_MIX;
#ifdef MTK_PHONE_CALL_RECORD_VOICE_ONLY
    mTypeRecord.dlPosition = RECORD_POS_DL_AFTER_ENH;
#else
    mTypeRecord.dlPosition = RECORD_POS_DL_END;
#endif
    uint32 recordDlPos = get_uint32_from_property(PROPERTY_KEY_RECORD_DL_POSITION);
    if (recordDlPos > 0) {
        // 0 for property disable, so property will set enum value +1
        mTypeRecord.dlPosition = (RecordPositionDlType)(recordDlPos - 1);
    }
...
void SpeechDataProcessingHandler::getStreamAttributeSource(Caller caller, stream_attribute_t *streamAttributeSource) {
    if (streamAttributeSource != NULL) {
        ...
        case VOICE_MIX_CALLER:
            streamAttributeSource->audio_channel_mask = (audio_channel_mask_t)(
                    AUDIO_CHANNEL_IN_VOICE_UPLINK | AUDIO_CHANNEL_IN_VOICE_DNLINK);
        ...
        // switch Record DL position when entor screen call mode
        if (streamAttributeSource->audio_mode == AUDIO_MODE_CALL_SCREEN) {
            mTypeRecord.dlPosition = RECORD_POS_DL_HEAD;
        } else {
            mTypeRecord.dlPosition = RECORD_POS_DL_AFTER_ENH;
        }
    }
...
status_t SpeechDataProcessingHandler::recordOn() {
    ALOGD("+%s()\n", __FUNCTION__);
    AL_AUTOLOCK(speechDataProcessingHandlerLock);

    mUserCounter++;

    if (mTypeRecord.dlPosition >= RECORD_POS_DL_MAX || mTypeRecord.dlPosition < 0) {
        ALOGE("%s(), Wrong dlPosition: %d !! use default: %d\n",
              __FUNCTION__, mTypeRecord.dlPosition, RECORD_POS_DL_END);
        mTypeRecord.dlPosition = RECORD_POS_DL_END;
    }

    if (mUserCounter == 1) {
        SpeechDriverFactory::GetInstance()->GetSpeechDriver()->recordOn(mTypeRecord);
        ALOGD("%s(), First user, record on. dlPosition: %d\n", __FUNCTION__, mTypeRecord.dlPosition);
    } else {
...
status_t SpeechDataProcessingHandler::recordOff() {
    ALOGD("+%s()\n", __FUNCTION__);
    AL_AUTOLOCK(speechDataProcessingHandlerLock);
    ...
    if (mUserCounter == 0) {
        SpeechDriverFactory::GetInstance()->GetSpeechDriver()->recordOff(mTypeRecord);
...
status_t SpeechDataProcessingHandler::setRecDlPos(RecordPositionDlType recDlPos) {
    ALOGD("+%s(), speech rec dl position : %d -> %d\n", __FUNCTION__, mTypeRecord.dlPosition, recDlPos);
    AL_AUTOLOCK(speechDataProcessingHandlerLock);
    mTypeRecord.dlPosition = recDlPos;
    // 0 for property disable, so need to set enum value +1
    set_uint32_to_property(PROPERTY_KEY_RECORD_DL_POSITION, recDlPos + 1);
    return NO_ERROR;
}
...
status_t SpeechDataProcessingHandler::processSpeechPacket(char *pInputPacketBuf, uint32_t speechDataSize) {
    ...
    if (uULFreq != 0 && uULFreq != kTargetSampleRate) {
        ...
        if (uSrcOutputBufSize <= 0) {
            ALOGE("%s(), ERROR size! uSrcOutputBufSize : %d ", __FUNCTION__, uSrcOutputBufSize);
            return -EINVAL;
        }
    ...
    for (int recordType = RECORD_TYPE_UL; recordType <= RECORD_TYPE_MIX; recordType++) {
        ...
        case RECORD_TYPE_MIX:
            if (pULPcmInputBuf != NULL && pDLPcmInputBuf != NULL && AudioALSACaptureDataProviderVoiceMix::hasInstance()) {
                ...
                if (bufferSize <= 0) {
                    ALOGE("%s(), ERROR size! bufferSize : %d ", __FUNCTION__, bufferSize);
                    if (pULSrcOutputBuf != NULL) {
                        delete[] pULSrcOutputBuf;
                    }
                    if (pDLSrcOutputBuf != NULL) {
