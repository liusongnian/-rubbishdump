...
status_t AudioALSACaptureHandlerBase::setLowLatencyMode(bool mode, size_t kernel_buffer_size, size_t reduce_size, bool bforce) {
    ...
    if (attribute == NULL) {
        ALOGE("%s(), attribute is NULL!!", __FUNCTION__);
        return NO_ERROR;
    }
...
status_t AudioALSACaptureHandlerBase::updateVolumeIndex(int stream, int device, int index) {
    if (mCaptureDataClient == NULL) {
        return NO_ERROR;
    }

    return mCaptureDataClient->updateVolumeIndex(stream, device, index);
}
...
status_t AudioALSACaptureHandlerBase::updateInputDevice(const audio_devices_t input_device __unused) {
    return NO_ERROR;
}

status_t AudioALSACaptureHandlerBase::dynamicSetCaptureHandlerAudioDump() {
    if (mCaptureDataClient == NULL) {
        return INVALID_OPERATION;
    }

    if (mCaptureDataClient->getCaptureDataProvider()) {
        ALOGD("%s(), CaptureDataProviderType = %d, dynamicSetCaptureDataProviderAudioDump",
              __FUNCTION__, mCaptureDataClient->getCaptureDataProvider()->getCaptureDataProviderType());
        mCaptureDataClient->getCaptureDataProvider()->dynamicSetCaptureDataProviderAudioDump();
    }
