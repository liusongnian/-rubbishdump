...
uint32_t AudioALSAPlaybackHandlerNormal::UpdateKernelBufferSize(audio_devices_t outputdevice) {
    uint32_t target_size = AudioPlatformInfo::getInstance()->getPlatformInfoValue(KERNEL_BUFFER_SIZE_NORMAL);
    bool hifi_enable = mHardwareResourceManager->getHiFiStatus();
    bool device_support_hifi = DeviceSupportHifi(outputdevice);
...
status_t AudioALSAPlaybackHandlerNormal::open() {
    ...
    if (mStreamAttributeSource->dualDevice != AUDIO_DEVICE_NONE &&
        (mStreamAttributeSource->output_devices == AUDIO_DEVICE_OUT_WIRED_HEADSET ||
         mStreamAttributeSource->output_devices == AUDIO_DEVICE_OUT_WIRED_HEADPHONE)) {
        /* fast/deep will use sw mixer when spk+hs/hp s.t. DL2 is availible */
        ...
        String8 tmpPcmDeepBuffer = String8(AudioPlatformInfo::getInstance()->getPlatformInfoStr(DPCM_DEEP_BUFFER));
        ALOGD("%s(), isolated deep buffer keypcmDeepBuffer = %s", __FUNCTION__, tmpPcmDeepBuffer.string());

        pcmindex = AudioALSADeviceParser::getInstance()->GetPcmIndexByString(tmpPcmDeepBuffer);
        cardindex = AudioALSADeviceParser::getInstance()->GetCardIndexByString(tmpPcmDeepBuffer);

        // use playback 2
        if (tmpPcmDeepBuffer.compare(keypcmPlayback2) == 0) {
            playbackSeq = String8(AUDIO_CTL_PLAYBACK2);
        } else {
    ...
    if (mStreamAttributeSource->dualDevice != AUDIO_DEVICE_NONE &&
            (mStreamAttributeSource->output_devices == AUDIO_DEVICE_OUT_WIRED_HEADSET ||
             mStreamAttributeSource->output_devices == AUDIO_DEVICE_OUT_WIRED_HEADPHONE)) {
        pcmPath = keypcmDL1DATA2PLayback;
    } else if (isIsolatedDeepBuffer(mStreamAttributeSource->mAudioOutputFlags)) {
        pcmPath = keypcmDL1DATA2PLayback;
    } else {
        pcmPath = keypcmI2S0Dl1Playback;
    }
...
#define KERNEL_BUFFER_SIZE_WITH_DRE  (20 * 1024) /* 20KB for 16bit hal */
    ...
    if (isAdspOptionEnable() &&
        ((mStreamAttributeSource->output_devices & AUDIO_DEVICE_OUT_SPEAKER)  ||
         (mStreamAttributeSource->output_devices & AUDIO_DEVICE_OUT_ALL_A2DP)) &&
          AudioDspStreamManager::getInstance()->getDspVersion() == DSP_VER_HARDWARE_MIX &&
    ...
    if (mHardwareResourceManager->isSpkApSwMixSupport(mStreamAttributeSource->output_devices)) {
        isBypassAurisys = false; // playback normal smartpa effect after mix
    } else {
        isBypassAurisys = mStreamAttributeSource->isBypassAurisys;
    }
...
ssize_t AudioALSAPlaybackHandlerNormal::write(const void *buffer, size_t bytes) {
    ALOGD_IF((mlog_flag&AUD_OUT_WRITE_LOG),"+%s(), flag = %d, buffer = %p, bytes = %zu",
    ...
    if (mStreamAttributeSource->output_devices & AUDIO_DEVICE_OUT_WIRED_HEADSET) {
        bytesAfterDcRemoval = bytes;
        pBufferAfterDcRemoval = pBuffer; 
    } else {
