 ...
 *                     Callback Function
 *============================================================================*/
static void callbackIpcLpbk(int audioEventType, void *caller, void *arg) {
    uint16_t ipcLpbkRouting = *(uint16_t *)arg;
    LoopbackManager *loopbackManager = NULL;
    loopbackManager = static_cast<LoopbackManager *>(caller);

    ALOGD("%s(), audioEventType = %d, caller(%p), ipcLpbkRouting(%d)",
          __FUNCTION__, audioEventType, caller, ipcLpbkRouting);
    if (loopbackManager == NULL) {
        ALOGE("%s(), loopbackManager is NULL. return.", __FUNCTION__);
    } else {
        AL_AUTOLOCK(loopbackManager->mIpcLpbkLock);
        switch (ipcLpbkRouting) {
        case IPC_LOOPBACK_CASE_INVALID:
            loopbackManager->SetLoopbackOff();
            break;
        case IPC_LOOPBACK_CASE_HANDSET://RX: Handset, TX: Main Mic
            loopbackManager->SetLoopbackOn(MD_MAIN_MIC_ACOUSTIC_LOOPBACK, LOOPBACK_OUTPUT_RECEIVER);
            break;
        case IPC_LOOPBACK_CASE_HEADSET://RX: Headset, TX: Headset Mic
            loopbackManager->SetLoopbackOn(MD_HEADSET_MIC_ACOUSTIC_LOOPBACK, LOOPBACK_OUTPUT_EARPHONE);
            break;
        case IPC_LOOPBACK_CASE_SPK://RX: Speaker, TX: Main Mic
            if (AudioALSAHardwareResourceManager::getInstance()->getNumPhoneMicSupport() >= 2) {
                loopbackManager->SetLoopbackOn(MD_REF_MIC_ACOUSTIC_LOOPBACK, LOOPBACK_OUTPUT_SPEAKER);
            } else {
                loopbackManager->SetLoopbackOn(MD_MAIN_MIC_ACOUSTIC_LOOPBACK, LOOPBACK_OUTPUT_SPEAKER);
            }
            break;
        case IPC_LOOPBACK_CASE_HEADPHONE_MAINMIC://RX: Headphone, TX: Main Mic
            loopbackManager->SetLoopbackOn(MD_MAIN_MIC_ACOUSTIC_LOOPBACK, LOOPBACK_OUTPUT_EARPHONE);
            break;
        case IPC_LOOPBACK_CASE_HEADPHONE_REFMIC://RX: Headphone, TX: Ref Mic
            loopbackManager->SetLoopbackOn(MD_REF_MIC_ACOUSTIC_LOOPBACK, LOOPBACK_OUTPUT_EARPHONE);
            break;
        case IPC_LOOPBACK_CASE_HEADPHONE_3RDMIC://RX: Headphone, TX: 3rd Mic
            loopbackManager->SetLoopbackOn(MD_3RD_MIC_ACOUSTIC_LOOPBACK, LOOPBACK_OUTPUT_EARPHONE);
            break;
        case IPC_LOOPBACK_PCM_CASE_HANDSET:
            loopbackManager->SetLoopbackOn(AP_MAIN_MIC_AFE_LOOPBACK, LOOPBACK_OUTPUT_RECEIVER);
            break;
        case IPC_LOOPBACK_PCM_CASE_HEADSET:
            loopbackManager->SetLoopbackOn(AP_HEADSET_MIC_AFE_LOOPBACK, LOOPBACK_OUTPUT_EARPHONE);
            break;
        case IPC_LOOPBACK_PCM_CASE_SPK:
            if (AudioALSAHardwareResourceManager::getInstance()->getNumPhoneMicSupport() >= 2) {
                loopbackManager->SetLoopbackOn(AP_REF_MIC_AFE_LOOPBACK, LOOPBACK_OUTPUT_SPEAKER);
            } else {
                loopbackManager->SetLoopbackOn(AP_MAIN_MIC_AFE_LOOPBACK, LOOPBACK_OUTPUT_SPEAKER);
            }
            break;
        case IPC_LOOPBACK_PCM_CASE_HEADPHONE_MAINMIC:
            loopbackManager->SetLoopbackOn(AP_MAIN_MIC_AFE_LOOPBACK, LOOPBACK_OUTPUT_EARPHONE);
            break;
        default:
            ALOGD("%s(), Not supported ipcLpbkRouting = 0x%x", __FUNCTION__, ipcLpbkRouting);
            break;
        }
        loopbackManager->setIpcLpbkStatus(LOOPBACK_STATE_READY);

        ALOGD("%s(), ipcLpbkRouting(0x%x), process done", __FUNCTION__, ipcLpbkRouting);
    }
}
...
LoopbackManager::LoopbackManager() :
    ...
    mIpcLpbkStatus = LOOPBACK_STATE_READY;
    AudioEventThreadManager::getInstance()->registerCallback(AUDIO_EVENT_IPC_LOOPBACK, callbackIpcLpbk, this);
...
status_t LoopbackManager::SetLoopbackOn(loopback_t loopback_type, loopback_output_device_t loopback_output_device) {
    ...
    if (CheckIsModemLoopback(loopback_type) == true) {
        ...
        if (pSpeechDriver != NULL) {
            ...
            ALOGE("%s(), pSpeechDriver == NULL! mWorkingModemIndex: %d", __FUNCTION__, mWorkingModemIndex);
        }
    }
    ...
    if (loopback_type == MD_DUAL_MIC_ACOUSTIC_LOOPBACK_WITHOUT_DMNR ||
        ...
        SpeechDriverInterface *pSpeechDriver = SpeechDriverFactory::GetInstance()->GetSpeechDriverByIndex(mWorkingModemIndex);
        if (pSpeechDriver) {
            pSpeechDriver->SetSpeechEnhancementMask(mask);
        }
    }
    ...
    // BT CVSD
    if (loopback_type == AP_BT_LOOPBACK || loopback_type == AP_MAIN_MIC_BT_OUT_LOOPBACK) {
...
status_t LoopbackManager::SetLoopbackOff() {
    ...
    // BT CVSD
    if ((mLoopbackType == AP_BT_LOOPBACK) || (mLoopbackType == AP_BT_LOOPBACK_NO_CODEC) || (mLoopbackType == AP_MAIN_MIC_BT_OUT_LOOPBACK)) {
    ...
    if (mLoopbackType == MD_DUAL_MIC_ACOUSTIC_LOOPBACK_WITHOUT_DMNR ||
        ...
        SpeechDriverInterface *pSpeechDriver = SpeechDriverFactory::GetInstance()->GetSpeechDriverByIndex(mWorkingModemIndex);
        if (pSpeechDriver != NULL) {
            pSpeechDriver->SetSpeechEnhancementMask(mMaskCopy);
        } else {
            ALOGE("%s(), pSpeechDriver == NULL! mWorkingModemIndex: %d", __FUNCTION__, mWorkingModemIndex);
        }
    }
...
status_t LoopbackManager::CheckLoopbackTypeIsValid(loopback_t loopback_type) {
    ...
    case AP_BT_LOOPBACK_NO_CODEC:
    case MD_BT_LOOPBACK_NO_CODEC:
    case AP_MAIN_MIC_BT_OUT_LOOPBACK:
        retval = NO_ERROR;
        break;
    case AP_REF_MIC_AFE_LOOPBACK:
    case MD_DUAL_MIC_ACOUSTIC_LOOPBACK_WITHOUT_DMNR:
...
audio_devices_t LoopbackManager::GetOutputDeviceByLoopbackType(loopback_t loopback_type, loopback_output_device_t loopback_output_device) {
    ...
    if (loopback_type == AP_BT_LOOPBACK ||
        ...
        loopback_type == AP_MAIN_MIC_BT_OUT_LOOPBACK) { // BT
...
int LoopbackManager::setIpcLpbkStatus(const loopback_ipc_state_t ipcLpbkStatus) {
    int retval = NO_ERROR;
    ALOGD("%s(), mIpcLpbkStatus(0x%x ->0x%x)", __FUNCTION__, mIpcLpbkStatus, ipcLpbkStatus);
    mIpcLpbkStatus = ipcLpbkStatus;
    return retval;
}

loopback_ipc_state_t LoopbackManager::getIpcLpbkStatus(void) {
    ALOGD("%s(), mIpcLpbkStatus = 0x%x", __FUNCTION__, mIpcLpbkStatus);
    return mIpcLpbkStatus;
}
