...
#endif
{
    ...
    mAudioThrottleTimeControlInstance = NULL;
    mIsTthrottling = 0;
}
...
AudioALSAPlaybackHandlerBTCVSD::~AudioALSAPlaybackHandlerBTCVSD() {
    ALOGD("%s()", __FUNCTION__);
    if (mAudioThrottleTimeControlInstance != NULL) {
...
status_t AudioALSAPlaybackHandlerBTCVSD::open() {
...
#ifdef MTK_AURISYS_FRAMEWORK_SUPPORT
    if (get_aurisys_on() && !mStreamAttributeSource->isBypassAurisys) {
    ...
    mDataProviderEchoRefBTCVSD->attachPlaybackHandler(&mStreamAttributeTarget);
    if (AudioALSADriverUtility::getInstance()->GetPropertyValue("vendor.audiohal.btcvsd_throttle_off") == 1) {
        mIsTthrottling = 0;
    } else {
        mIsTthrottling = 1;
    }
    if (mIsTthrottling) {
        mAudioThrottleTimeControlInstance = new AudioThrottleTimeControl(true);
        mAudioThrottleTimeControlInstance->resetTimeStampInfo();
    }

    ALOGD("-%s(), mStreamAttributeTarget, ch=%d, sr=%d, mStreamAttributeTargetEchoRef, ch=%d, sr=%d, throttle=%d",
        __FUNCTION__,
...
status_t AudioALSAPlaybackHandlerBTCVSD::close() {
    ...
    mDataProviderEchoRefBTCVSD->detachPlaybackHandler();
...
ssize_t AudioALSAPlaybackHandlerBTCVSD::write(const void *buffer, size_t bytes) {
...
#endif
    ALOGD_IF((mlog_flag & AUD_OUT_WRITE_LOG), "+%s(), buffer = %p, bytes = %zu", __FUNCTION__, buffer, bytes);
    ...
    if (mDataPendingForceUse) {
        ...
        if (bytesAfterDataPending < mDataAlignedSize) {
            ALOGD("%s(), bytesAfterDataPending %u, return", __FUNCTION__, bytesAfterDataPending);
            return bytes;
        }
    } else {
    ...
    do {
        ...
        if (total_outsize > 0) {
            ...
            while (maxTimeoutTrial >= 0) {
                ...
                int retHts = pcm_get_htimestamp(mPcm, &avail, &timeStamp);
                if (retHts != 0) {
                    ALOGW("%s(), pcm_get_htimestamp fail %s, ret %d, mWrittenFrame %u, totalOutFrame %u\n",
                          __FUNCTION__, pcm_get_error(mPcm), retHts, mWrittenFrame, totalOutFrame);
                ...
                if (needSkipWrite) { //only retry when mInitWrite
                    skipWrite = true;
                    if (mInitWrite) {
    ...
    if (mIsTthrottling) {
        mAudioThrottleTimeControlInstance->adaptiveSleepUs(bytes, *mStreamAttributeSource);
    }
    ...
    mTotalWriteBytes += bytes;
    ALOGD_IF((mlog_flag & AUD_OUT_WRITE_LOG), "-%s(), buffer = %p, bytes = %zu, mIsTthrottling = %d",
             __FUNCTION__, buffer, bytes, mIsTthrottling);
    return bytes;
}

status_t AudioALSAPlaybackHandlerBTCVSD::getHardwareBufferInfo(time_info_struct_t *HWBuffer_Time_Info) {
    stream_attribute_t *mStreamAttributeSourcePtr = (stream_attribute_t *)mStreamAttributeSource;
    calculateTimeStampByBytes(mEchoRefStartTime, mTotalWriteBytes, *mStreamAttributeSourcePtr, &HWBuffer_Time_Info->timestamp_get);

    ALOGV("%s(), mStartTime %ld.%09ld, mTotalWriteBytes %d, timestamp_get %ld.%09ld",
          __FUNCTION__, mEchoRefStartTime.tv_sec, mEchoRefStartTime.tv_nsec, mTotalWriteBytes, HWBuffer_Time_Info->timestamp_get.tv_sec, HWBuffer_Time_Info->timestamp_get.tv_nsec);
    return NO_ERROR;
}
