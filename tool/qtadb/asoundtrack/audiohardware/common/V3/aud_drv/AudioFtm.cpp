...
AudioFtm::AudioFtm() :
    ...
    isFactoryMode = InFactoryMode();

    ALOGD("%s(), mMixer: %p, factory mode: %d", __FUNCTION__, mMixer, isFactoryMode);
    ASSERT(mMixer != NULL);

    if (isAdspOptionEnable() && isFactoryMode) {
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_primary_default_en"), 0, false)) {
            ALOGW("%s(), dsp_primary disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_deepbuf_default_en"), 0, false)) {
            ALOGW("%s(), dsp_deepbuf disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_voipdl_default_en"), 0, false)) {
            ALOGW("%s(), dsp_voipdl disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_playback_default_en"), 0, false)) {
            ALOGW("%s(), dsp_playback disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_captureul1_default_en"), 0, false)) {
            ALOGW("%s(), dsp_captureul1 disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_dataprovider_default_en"), 0, false)) {
            ALOGW("%s(), dsp_dataprovider disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_fast_default_en"), 0, false)) {
            ALOGW("%s(), dsp_fast disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "dsp_call_final_default_en"), 0, false)) {
            ALOGW("%s(), dsp_call_final disable fail", __FUNCTION__);
        }
        if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "swdsp_smartpa_process_enable"), 0, false)) {
            ALOGW("%s(), swdsp_smartpa disable fail", __FUNCTION__);
        }
    }
}
...
int AudioFtm::RecieverTest(char receiver_test) {
    ...
    if (receiver_test) { // enable
        mHardwareResourceManager->openAddaOutput(32000, AUDIO_DEVICE_OUT_EARPIECE);
...
int AudioFtm::LouderSPKTest(char left_channel, char right_channel) {
    ...
    if (left_channel != false || right_channel != false) { // enable
        mHardwareResourceManager->openAddaOutput(48000, AUDIO_DEVICE_OUT_SPEAKER);
        mHardwareResourceManager->startOutputDevice(AUDIO_DEVICE_OUT_SPEAKER, 48000);
        mHardwareResourceManager->setSgenMode(SGEN_MODE_I05_I06);
        mHardwareResourceManager->setSgenSampleRate(SGEN_MODE_SAMPLERATE_48000HZ);
    } else { // disable
        mHardwareResourceManager->setSgenMode(SGEN_MODE_DISABLE);
        mHardwareResourceManager->stopOutputDevice();
        mHardwareResourceManager->closeAddaOutput();
...
int AudioFtm::EarphoneTest(char bEnable) {
    ...
    if (bEnable) { // enable
        ...
        mHardwareResourceManager->openAddaOutput(32000, AUDIO_DEVICE_OUT_WIRED_HEADPHONE);
...
uint32_t AudioFtm::GetVibSpkCalibrationStatus() {
    ...
    // init audioParam.bes_loudness_WS_Gain_Max
    audioParam.bes_loudness_WS_Gain_Max = 0;
    // init audioParam.bes_loudness_WS_Gain_Min
    audioParam.bes_loudness_WS_Gain_Min = 0;

    getAudioCompFltCustParam(AUDIO_COMP_FLT_VIBSPK, &audioParam);
    ALOGD("%s(), bes_loudness_WS_Gain_Max %d, bes_loudness_WS_Gain_Min %d",
          __FUNCTION__, audioParam.bes_loudness_WS_Gain_Max, audioParam.bes_loudness_WS_Gain_Min);
...
bool AudioFtm::ftmWavePlaybackCalib(void) {
    int ret;
    ...
    return ret == SPK_RICHTEK_RT5509 ? true : false;
}
