...
void *AudioBitTrueTest::verifyThread(void *arg) {
    ...
    // clean data
    memset(readData, 0, readDataSize);
...
FILE *AudioBitTrueTest::testDumpOpen(const char *name, const char *property) {
    ...
    if (snprintf(dumpFileName, sizeof(dumpFileName), "%s.%s.%u.pid%d.tid%d.48000.16bit.2ch.pcm",
                 TEST_DUMP_FILE_PREFIX, name, dumpFileCount, getpid(), gettid()) < 0) {
        ALOGE("%s(), snprintf dumpFileName fail!!", __FUNCTION__);
    } else {
        file = NULL;
