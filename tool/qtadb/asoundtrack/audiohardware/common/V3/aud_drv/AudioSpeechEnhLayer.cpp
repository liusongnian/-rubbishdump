...
SPELayer::~SPELayer() {
    ...
    if (mfpInDL) {
        if (fclose(mfpInDL) != 0) {
            ALOGW("%s(), fclose error", __FUNCTION__);
        }
        mfpInDL = NULL;
    }
    if (mfpInUL) {
    ...
    if (mfpOutDL) {
        if (fclose(mfpOutDL) != 0) {
            ALOGW("%s(), fclose error", __FUNCTION__);
        }
        mfpOutDL = NULL;
    }
    if (mfpOutUL) {
        if (fclose(mfpOutUL) != 0) {
            ALOGW("%s(), fclose error", __FUNCTION__);
        }
        mfpOutUL = NULL;
    }
    if (mfpProcessedDL) {
        if (fclose(mfpProcessedDL) != 0) {
            ALOGW("%s(), fclose error", __FUNCTION__);
        }
        mfpProcessedDL = NULL;
    }
    if (mfpProcessedUL) {
        if (fclose(mfpProcessedUL) != 0) {
            ALOGW("%s(), fclose error", __FUNCTION__);
        }
        mfpProcessedUL = NULL;
    }
...
bool SPELayer::PrepareProcessData() {
    ...
    if (mDLNewStart || (DLdataPrepareCount > 0)) { //the first queue downlink buffer is not ready to play yet, only need to process uplink data, so add zero data on downlink
        ...
        if (mNeedJitterBuffer && (mJitterSampleCount != 0)) { //the first DL buffer, add the jitter buffer at the first downlink buffer queue and downlink delay buffer queue
            ...
            if (newJitterBuffer->pBufBase == NULL) {
                delete newJitterBuffer;
                ASSERT(0);
                return false;
            }
        ...
        if (mNeedDelayLatency && (mLatencySampleCount != 0)) { //the first DL buffer, add the delay time buffer as first delay buffer queue
            ...
            if (mLatencyDir == true) {
                ...
                if (newDelayBuffer->pBufBase == NULL){
...
int SPELayer::Process_VoIP(short *inBuf, int  inBufLength) {
    ...
    if (mULInBufQLenTotal >= mSPEProcessBufSize) {    //&&(mDLInBufQLenTotal>= mSPEProcessBufSize/2))
        while (mULInBufQLenTotal >= mSPEProcessBufSize) { //&&(mDLInBufQLenTotal>= mSPEProcessBufSize/2))  //TODO:fixme!!! has problem
            ...
            if (newULOutBuffer->pBufBase == NULL) {
                delete newULOutBuffer;
                ASSERT(0);
                break;
            }
...
void *DumpThread(void *arg) {
    ...
    while (1) {
        ...
        if (pSPEL->mDumpDLInBufferQ.size() > 0) {
            if (fwrite(pSPEL->mDumpDLInBufferQ[0]->pBufBase, pSPEL->mDumpDLInBufferQ[0]->BufLen, 1, pSPEL->mfpInDL) != 1) {
        ...
        if (pSPEL->mDumpDLOutBufferQ.size() > 0) {
            if (fwrite(pSPEL->mDumpDLOutBufferQ[0]->pBufBase, pSPEL->mDumpDLOutBufferQ[0]->BufLen, 1, pSPEL->mfpOutDL) != 1) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
            pSPEL->DumpMutexLock();
            if (pSPEL->hDumpThread != 0) {
        ...
        if (pSPEL->mDumpULInBufferQ.size() > 0) {
            if (fwrite(pSPEL->mDumpULInBufferQ[0]->pBufBase, pSPEL->mDumpULInBufferQ[0]->BufLen, 1, pSPEL->mfpInUL) != 1) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
            pSPEL->DumpMutexLock();
            if (pSPEL->hDumpThread != 0) {
        ...
        if (pSPEL->mDumpULOutBufferQ.size() > 0) {
            if (fwrite(pSPEL->mDumpULOutBufferQ[0]->pBufBase, pSPEL->mDumpULOutBufferQ[0]->BufLen, 1, pSPEL->mfpOutUL) != 1) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
            pSPEL->DumpMutexLock();
            if (pSPEL->hDumpThread != 0) {
        ...
        if (pSPEL->mDumpEPLBufferQ.size() > 0) {
            if (fwrite(pSPEL->mDumpEPLBufferQ[0]->pBufBase, pSPEL->mDumpEPLBufferQ[0]->BufLen, 1, pSPEL->mfpEPL) != 1) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
            pSPEL->DumpMutexLock();
...
void SPELayer::Dump_Enalbe_Check(void) {
    int ret;
    char Buf[10];
    if (snprintf(Buf, sizeof(Buf), "%d.pcm", DumpFileNum) < 0) {
        ALOGE("%s(), snprintf dumpFileNum fail!!", __FUNCTION__);
    }
    char value[PROPERTY_VALUE_MAX] = {0};
    ...
    if (bflag || bflag1) {
        ...
        if (snprintf(Buf, sizeof(Buf), "%d.EPL", DumpFileNum) < 0) {
            ALOGE("%s(), snprintf dumpFileNum fail!!", __FUNCTION__);
        }
...
void SPELayer::Dump_PCM_Process(SPE_DATA_DIRECTION dir, void *buffer, int bytes) {
    ...
    if (dir == UPLINK) {
        if (mfpProcessedUL != NULL) {
            if (fwrite(buffer, bytes, count, mfpProcessedUL) != count) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
        }
    } else {
        if (mfpProcessedDL != NULL) {
            if (fwrite(buffer, bytes, count, mfpProcessedDL) != count) {
                ALOGW("%s(), fwrite error", __FUNCTION__);
            }
        }
    }
        ...
        if (mfpVM != NULL) {
            if (mSph_Enh_ctrl.sample_rate == 48000) {
                ...
                */
                ...
                if (fwrite(mVM, MaxVMSize * sizeof(short), count, mfpVM) != count) {
                    ALOGW("%s(), fwrite error", __FUNCTION__);
                }
                ...
                */
                ...
                if (fwrite(mVM, 642 * sizeof(short), count, mfpVM) != count) {
                    ALOGW("%s(), fwrite error", __FUNCTION__);
                }
            }
        } else {
