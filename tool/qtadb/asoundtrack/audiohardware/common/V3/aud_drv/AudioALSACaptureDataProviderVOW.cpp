...
// need to define MTK_VOW_DUAL_MIC_SUPPORT first for phase 2.5 dual channel data
...
status_t AudioALSACaptureDataProviderVOW::close() {
    ...
    if (drop_flag == false) {
        ...
        ALOGD("%s(), bypass VOWControlCmd_DisableDebug set and pth_join", __FUNCTION__);
        ClosePCMDump();
    }
    ALOGD("-%s()", __FUNCTION__);
    return NO_ERROR;
...
void *AudioALSACaptureDataProviderVOW::readThread(void *arg) {
    ...
    while (pDataProvider->mEnable == true) {
        ...
        if (pDataProvider->mEnable_debug == false) {
            ALOGI("%s(), VOWControlCmd_EnableDebug set, ret = %d", __FUNCTION__, ret);
            ret = ::ioctl(pDataProvider->mFd, VOW_SET_CONTROL, (unsigned int)VOWControlCmd_EnableDebug);
            if (ret != 0) {
                ALOGE("%s(), VOWControlCmd_EnableDebug error, ret = %d", __FUNCTION__, ret);
            }
            pDataProvider->mEnable_debug = true;
        }
        int retval = ::ioctl(pDataProvider->mFd, VOW_READ_VOICE_DATA, vow_info);
        if (retval != 0) {
