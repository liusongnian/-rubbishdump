...
status_t AudioALSAPlaybackHandlerAAudio::open() {
    ...
    int min_framecount = MMAP_DL_PERIOD_SIZE * MIN_MMAP_DL_PERIOD_COUNT;
    ...
    mConfig.period_size = MMAP_DL_PERIOD_SIZE;
    mConfig.period_count = (mMin_size_frames - 1) / MMAP_DL_PERIOD_SIZE + 1;
    ...
    ALOGD("%s(), mConfig: channels = %d, rate = %d, period_size = %d, period_count = %d, format = %d, "
          "avail_min = %d, start_threshold = %d, mMin_size_frames %d",
          __FUNCTION__, mConfig.channels, mConfig.rate, mConfig.period_size, mConfig.period_count,
          mConfig.format, mConfig.avail_min, mConfig.start_threshold, mMin_size_frames);
