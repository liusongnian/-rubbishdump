...
void audio_messenger_ipi_init(bool ipiDmaEn, bool adspRecoveryEn, bool ipiServiceEn)
{
    int ret = 0;
    int i = 0;
    ...
    if (adspRecoveryEn && ipiServiceEn) {
...
void audio_messenger_ipi_deinit(bool ipiDmaEn, bool adspRecoveryEn, bool ipiServiceEn)
{
    if (ipiDmaEn) {
        /* dma */
...
uint8_t get_audio_controller_task_by_id(const uint8_t adsp_id) {
    switch (adsp_id) {
    case AUDIO_OPENDSP_USE_CM4_A:
        return TASK_SCENE_AUDIO_CONTROLLER_CM4;
    case AUDIO_OPENDSP_USE_HIFI3_A:
        return TASK_SCENE_AUDIO_CONTROLLER_HIFI3_A;
    case AUDIO_OPENDSP_USE_HIFI3_B:
        return TASK_SCENE_AUDIO_CONTROLLER_HIFI3_B;
    default:
        AUD_LOG_E("%s(), opendsp_id %u not support", __FUNCTION__, adsp_id);
        return TASK_SCENE_INVALID;
    }
}
