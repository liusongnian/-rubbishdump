    ...
    if (bt_device_on == true) {
        ...
        if (WCNChipController::GetInstance()->BTUseCVSDRemoval() == true) {
            if (!mUseBtCodec) {
                ...
                uint32_t sampleRate = WCNChipController::GetInstance()->GetBTCurrentSamplingRateNumber();

                streamOutput = AudioALSAStreamManager::getInstance()->openOutputStream(output_device, &format,
                                                                                       &channels, &sampleRate,
                                                                                       &status, AUDIO_OUTPUT_FLAG_FAST);
                ALOGD("+%s(), CVSD AP loopback with codec, fast flag, output_device=0x%x, streamOutput: %p", __FUNCTION__, output_device, streamOutput);

                uint32_t channels_in = AUDIO_CHANNEL_IN_MONO;
                streamInput = AudioALSAStreamManager::getInstance()->openInputStream(input_device, &format, &channels_in, &sampleRate, &status, (audio_in_acoustics_t)0);
                ALOGD("+%s(), CVSD AP loopback with codec, input_device=0x%x, streamInput: %p", __FUNCTION__, input_device, streamInput);
    ...
    if (bt_device_on) {
        if (WCNChipController::GetInstance()->BTUseCVSDRemoval() == true) {
            if (mBtLoopbackWithoutCodec) {
                ...
                AudioALSAStreamManager::getInstance()->closeInputStream(streamInput);
...
bool AudioALSALoopbackController::AudioMTKLoopbackThread::threadLoop() {
    #define BT_LOOPBACK_BUFFER_SIZE (640) // for 20ms, 16kHz/16bit/1ch
    uint32_t uReadByte = 0, uWriteDataToBT = 0;
    uint8_t btloopback_buffer[BT_LOOPBACK_BUFFER_SIZE];
    memset(btloopback_buffer, 0, BT_LOOPBACK_BUFFER_SIZE);

    uint32_t sampleRate = WCNChipController::GetInstance()->GetBTCurrentSamplingRateNumber();
    ASSERT(sampleRate == 8000 || sampleRate == 16000);
    ssize_t buf_size = sampleRate / 1000 * 20 * 2; // 20ms, 16bit, 1ch

    while (!(exitPending() == true)) {
        ALOGD("BT_SW_CVSD AP new loopback threadLoop(+)");
        uReadByte = streamInput->read(btloopback_buffer, buf_size);
        ALOGD("BT_SW_CVSD AP new loopback streamin read: %d", uReadByte);
        uWriteDataToBT = streamOutput->write(btloopback_buffer, buf_size);
        ALOGD("BT_SW_CVSD AP new loopback threadLoop(-), uReadByte: %d, uWriteDataToBT: %d", uReadByte, uWriteDataToBT);
    }
    ALOGD("BT_SW_CVSD AP new loopback threadLoop exit");
    return false;
}
