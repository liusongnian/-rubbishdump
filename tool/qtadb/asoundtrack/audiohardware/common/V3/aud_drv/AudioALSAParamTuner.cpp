...
#ifndef DEVICE_VOLUME_STEP
#define DEVICE_VOLUME_STEP (256) //141~330 Hz
#endif
...
#endif
#ifndef DEVICE_AMP_MAX_VOLUME
#define DEVICE_AMP_MAX_VOLUME (15)
#endif
#ifndef DEVICE_AMP_MIN_VOLUME
...
static void *DMNR_Play_Rec_ApSide_Routine(void *arg) {
    ...
    if (pInFp) { //open output stream for playback
        ...
        if (streamOutput == NULL) {
            ALOGE("%s(), NULL streamOutput!", __FUNCTION__);
            return 0;
        }
...
AudioALSAParamTuner::AudioALSAParamTuner() {
    ...
    mPlay2WayInstance = 0;
    mRec2WayInstance = 0;
    mDualMicTool_receiverGain = 0;
    mDualMicTool_headsetGain = 0;
...
void AudioALSAParamTuner::updataOutputFIRCoffes(AudioTasteTuningStruct *pCustParam) {
    ...
    ALOGD("%s(), mode:%d, mMode:%d, isPlaying:%d,", __FUNCTION__, mode, mMode, isPlaying());

    if (isPlaying() && mode == mMode) {
...
status_t AudioALSAParamTuner::enableDMNRAtApSide(bool bEnable, bool bWBMode, unsigned short outputDevice, unsigned short workMode) {
    ...
    if (bEnable && (isPlaying() == false)) {
        ...
        ALOGD("%s(), The DMNR Tuning State is error, bEnable=%d, playing=%d", __FUNCTION__, bEnable, isPlaying());
        return BAD_VALUE;
    }
