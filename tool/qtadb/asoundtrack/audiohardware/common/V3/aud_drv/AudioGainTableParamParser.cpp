...
void GainTableParamParser::updateMicUnit(GainTableMicUnit *micUnit, Param *param, int device) {
    int ch = 0;
    char *ul_gains = (char *)param->data;
    int *gain = (int *)param->data; /* for foward compatible */
    int size = param->arraySize;

    if (param->paramInfo->dataType == TYPE_INT) {
        /* for foward compatible */
        if (*gain > mSpec.micIdxMax[device])
            *gain = mSpec.micIdxMax[device];
        if (*gain < mSpec.micIdxMin[device])
            *gain = mSpec.micIdxMin[device];

        micUnit->gain[MIC_IN_CH0] = *gain;
        size = 0;
    }

    if (size > TOTAL_MIC_IN_CHANNEL) {
        ALOGW("error, param->arraySize %d exceed ul_gain array size %d",
              size, TOTAL_MIC_IN_CHANNEL);
        size = TOTAL_MIC_IN_CHANNEL;
    }

    for (ch = 0; ch < TOTAL_MIC_IN_CHANNEL; ch++) {
        char *gain = &micUnit->gain[ch];

        if (ch < size) {
            *gain = ul_gains[ch];

            if (*gain > mSpec.micIdxMax[device])
                *gain = (char)mSpec.micIdxMax[device];
            if (*gain < mSpec.micIdxMin[device])
                *gain = (char)mSpec.micIdxMin[device];
        } else {
            /* xml not defined the channel, use ch0 as default */
            *gain = micUnit->gain[MIC_IN_CH0];
        }
        ALOGV("\t%s(), device = %d, gain[%d] = %d\n", __FUNCTION__, device, ch, *gain);
    }
}
    ...
    for (int net = 0; net < NUM_GAIN_SPEECH_NETWORK; net++) {
        for (int band = 0; band < NUM_GAIN_SPEECH_BAND; band++) {
            for (int device = 0; device < NUM_GAIN_DEVICE; device++) {
                ...
                // Uplink gain
                Param *param;
                param = appOps->paramUnitGetParamByName(paramUnit, paramUlName);
                if (!param) {
                    ALOGW("error: get param_ul_gain fail");
                    continue;
                }

                updateMicUnit(&_gainTable->nonSceneGain.speechMicGain[band][net][device],
                              param,
                              device);
    ...
    for (int scene = 0; scene < (int)(sceneList->size()); scene++) {
        for (int mode = 0; mode < NUM_GAIN_MIC_MODE; mode++) {
            for (int device = 0; device < NUM_GAIN_DEVICE; device++) {
                ...
                // Uplink gain
                Param *param;
                param = appOps->paramUnitGetParamByName(paramUnit, "ul_gain");
                if (!param) {
                    ALOGW("error: get param_ul_gain fail");
                    continue;
                }

                updateMicUnit(&_gainTable->sceneGain[scene].micGain[mode][device],
                              param,
                              device);
            }
        }
...
status_t GainTableParamParser::updateVoIPVol(GainTableParam *_gainTable, std::vector<std::string> *sceneList) {
    ...
    for (int scene = 0; scene < (int)(sceneList->size()); scene++) {
        for (int device = 0; device < NUM_GAIN_DEVICE; device++) {
            ...
            updateMicUnit(&_gainTable->sceneGain[scene].micGain[GAIN_MIC_VOICE_COMMUNICATION][device],
                          param,
                          device);
...
status_t GainTableParamParser::updateRingbackVol(GainTableParam *_gainTable) {
    ...
    unsigned int streamDevice = 0;
    for (unsigned int device = 0; device < NUM_GAIN_DEVICE; device++) {
...
status_t GainTableParamParser::loadGainTableSpec() {
    ...
    getParam(paramUnit, &mSpec.audioBufMixerName, "audio_buffer_mixer_name");
    ALOGD("%s(), mSpec.audioBufferGainPreferMaxIdx = %d, audioBufMixerName = %s",
          __FUNCTION__,
          mSpec.audioBufferGainPreferMaxIdx,
          mSpec.audioBufMixerName.c_str());
#else
...
#if defined(GAIN_TABLE_USING_TLV_DB)
    getParam(paramUnit, &mSpec.spkMixerName, "spk_mixer_name");
    ALOGD("%s(), mSpec.spkAnaType = %d, spkMixerName = %s",
          __FUNCTION__, mSpec.spkAnaType, mSpec.spkMixerName.c_str());
#else
    ...
    getParam(paramUnit, &mSpec.ulPgaMixerName[MIC_IN_CH0], "ul_pga_l_mixer_name");
    getParam(paramUnit, &mSpec.ulPgaMixerName[MIC_IN_CH1], "ul_pga_r_mixer_name");
    getParam(paramUnit, &mSpec.ulPgaMixerName[MIC_IN_CH2], "ul_pga_3_mixer_name");
    ALOGD("%s(), ulPgaMixerName = ch0:%s, ch1:%s, ch2:%s",
          __FUNCTION__,
          mSpec.ulPgaMixerName[MIC_IN_CH0].c_str(),
          mSpec.ulPgaMixerName[MIC_IN_CH1].c_str(),
          mSpec.ulPgaMixerName[MIC_IN_CH2].c_str());
