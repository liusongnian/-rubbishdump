...
status_t AudioALSAPlaybackHandlerVoice::getHardwareBufferInfo(time_info_struct_t *HWBuffer_Time_Info) {
    calculateTimeStampByBytes(mStartTime, mTotalWriteBytes, *((stream_attribute_t *)mStreamAttributeSource), &HWBuffer_Time_Info->timestamp_get);

    ALOGV("%s(), mStartTime %ld.%09ld, mTotalWriteBytes %d, timestamp_get %ld.%09ld",
          __FUNCTION__, mStartTime.tv_sec, mStartTime.tv_nsec, mTotalWriteBytes, HWBuffer_Time_Info->timestamp_get.tv_sec, HWBuffer_Time_Info->timestamp_get.tv_nsec);

    return NO_ERROR;
}

status_t AudioALSAPlaybackHandlerVoice::updateStartTimeStamp() {
    if (mStartTime.tv_sec == 0 && mStartTime.tv_nsec == 0) {
        if (clock_gettime(CLOCK_MONOTONIC, &mStartTime) == 0) {
            ALOGD("%s(), Set start timestamp (%ld.%09ld), mTotalWriteBytes = %d",
                  __FUNCTION__,
                  mStartTime.tv_sec,
                  mStartTime.tv_nsec,
                  mTotalWriteBytes);
        ...
        ALOGV("%s(), Set start timestamp (%ld.%09ld), mTotalWriteBytes = %d",
              __FUNCTION__,
              mStartTime.tv_sec,
              mStartTime.tv_nsec,
              mTotalWriteBytes);
...
ssize_t AudioALSAPlaybackHandlerVoice::write(const void *buffer, size_t bytes) {
    ...
    doBitConversion(targetBuf, bytes, &pBufferAfterBitConvertion, &bytesAfterBitConvertion);
...
void AudioALSAPlaybackHandlerVoice::OpenPCMDump(const char *className) {
    ALOGV("%s()", __FUNCTION__);
