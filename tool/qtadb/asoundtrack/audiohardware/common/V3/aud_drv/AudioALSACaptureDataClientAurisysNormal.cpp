...
#include "AudioSystemLibUtil.h" /*  for ANDROID_PRIORITY_AUDIO */
...
AudioALSACaptureDataClientAurisysNormal::AudioALSACaptureDataClientAurisysNormal(
    ...
    if (audio_is_usb_in_device(mStreamAttributeSource->input_device) && (getPcmStatus() != true)) {
        hProcessThread = 0;
    ...
    ret = pthread_create(&hProcessThread, NULL,
                         ...
                         (void *)this);
    ASSERT(ret == 0);
    ALOGD("%s(-), drop_ms = %d, mDropPopSize = %d", __FUNCTION__, drop_ms, mDropPopSize);
}
...
uint32_t AudioALSACaptureDataClientAurisysNormal::copyCaptureDataToClient(RingBuf pcm_read_buf) {
    ...
    if (IsAECEnable() == false) {
        ...
        if (mIsEchoRefDataSync == false) {
            if (isNeedSkipSyncEchoRef() == true) {
                ALOGD("%s(), mPlaybackEnabled == false, no need to sync EchoRefData", __FUNCTION__);
                audio_ringbuf_copy_from_ringbuf_all(&mRawDataBuf, &pcm_read_buf_wrap);
            } else {
        ...
        if ((mIsEchoRefDataSync == true || isNeedSkipSyncEchoRef() == true) &&
...
void *AudioALSACaptureDataClientAurisysNormal::processThread(void *arg) {
    ...
    while (client->mEnable == true) {
        ...
        LATENCY_LOG("1.1");
        if (audio_is_usb_in_device(client->mStreamAttributeSource->input_device) && (client->getPcmStatus() != true)) {
            ALOGD("%s, PCM Open/Read Fail...USB Device is unplugged ?", __FUNCTION__);
...
bool AudioALSACaptureDataClientAurisysNormal::IsLowLatencyCapture(void) {
    ...
    bool is_usb = audio_is_usb_in_device(mStreamAttributeSource->input_device);
    uint32_t latency = mCaptureDataProvider->getLatencyTime();

#ifdef UPLINK_LOW_LATENCY
    if (voip_on == false && aec_on == false &&
        ((mStreamAttributeTarget->mAudioInputFlags & AUDIO_INPUT_FLAG_FAST) ||
         (is_usb && latency == UPLINK_LOW_LATENCY_MS))) {
        low_latency_on = true;
    }
#endif
...
status_t AudioALSACaptureDataClientAurisysNormal::updateVolumeIndex(int stream, int device, int index) {
    if (mManagerConfig == NULL || mCaptureDataProvider == NULL) {
        ALOGD("%s(), mManagerConfig = %p, mCaptureDataProvider = %p, ignore", __FUNCTION__, mManagerConfig, mCaptureDataProvider);
        return NO_ERROR;
    }
    ...
    setupCustomInfoStr(mManagerConfig->custom_info, MAX_CUSTOM_INFO_LEN, mStreamAttributeSource->mCustScene,
        ...
        audio_is_bluetooth_sco_device((audio_devices_t)(mManagerConfig->task_config.input_device_info.devices)) ? AudioALSAStreamManager::getInstance()->GetBtCodec() : -1);

#if defined(NON_SCENE_3GVT_VOWIFI_SUPPORT)
        const char* voipModeStr = NULL;
        if (mStreamAttributeTarget->mIs3GVT) {
            voipModeStr = "SetVoIPMode=3GVT;";
        } else if (mStreamAttributeTarget->mIsVoWifi && mStreamAttributeTarget->mIsVoWifiWbAmr) {
...
void AudioALSACaptureDataClientAurisysNormal::syncEchoRefData(
    ...
    if (mIsEchoRefDataSync && (mCaptureDataProviderEchoRef->getPlaybackEnabled() == false)) { // compensate data will add latency, drop it
...
void AudioALSACaptureDataClientAurisysNormal::CreateAurisysLibManager() {
    ...
    if (IsVoIPEnable() ||
        ...
        mAurisysScenario = AURISYS_SCENARIO_HDR_RECORD;
        sample_rate = 48000;
    ...
    if (audio_is_bluetooth_in_sco_device(mStreamAttributeTarget->input_device)) {
        ulDigitalGain = 0;
        ulAnalogGain = 0;
        ALOGD("BT path set Digital MIC gain = 0");
    } else if (audio_is_usb_in_device(mStreamAttributeTarget->input_device)) {
        ulAnalogGain = 0;
...
void AudioALSACaptureDataClientAurisysNormal::InitBufferConfig(struct aurisys_lib_manager_t *manager) {
    ...
    if (IsAECEnable()) {
        ...
        if ((mAudioPoolBufUlAec != NULL) && (mStreamAttributeSourceEchoRef != NULL)) {
            ...
            ALOGE("%s(), create pool buf ref fail!", __FUNCTION__);
        }
    }
...
void AudioALSACaptureDataClientAurisysNormal::UpdateAurisysConfig() {
    ALOGD("%s()", __FUNCTION__);
...
bool AudioALSACaptureDataClientAurisysNormal::isNeedSkipSyncEchoRef() {
    bool ret = false;
    /* For USB/BT device, echoref compensates mute data when mPlaybackEnabled false to prevent UL hang.
       No need to syncEchoRefData if echoref is only mute data, we only syncEchoRefData once DL data comes */
    if ((audio_is_usb_in_device(mStreamAttributeSource->input_device) ||
        audio_is_bluetooth_in_sco_device(mStreamAttributeSource->input_device)) &&
        mCaptureDataProviderEchoRef->getPlaybackEnabled() == false) {
        ret = true;
