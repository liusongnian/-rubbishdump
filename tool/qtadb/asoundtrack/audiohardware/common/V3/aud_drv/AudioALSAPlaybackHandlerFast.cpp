...
status_t AudioALSAPlaybackHandlerFast::open() {
    ...
    if (isAdspOptionEnable() &&
        AudioDspStreamManager::getInstance()->isAdspPlayback(mStreamAttributeTarget.mAudioOutputFlags,
...
#ifdef MTK_AUDIODSP_SUPPORT
    if (isAdspOptionEnable() &&
        ((AudioDspStreamManager::getInstance()->isAdspPlayback(mStreamAttributeTarget.mAudioOutputFlags, mStreamAttributeTarget.output_devices)) ||
