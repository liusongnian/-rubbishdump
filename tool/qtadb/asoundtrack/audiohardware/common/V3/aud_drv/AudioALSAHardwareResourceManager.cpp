...
#define A2DP_DEFAULT_LANTENCY (120)
...
AudioALSAHardwareResourceManager::AudioALSAHardwareResourceManager() :
    ...
    // if defined MTK_AUDIO_MIC_INVERSE, default invert main & ref mic
    ...
    mNumSpeakerSupport = setNumSpkSupport();
    mSmartPaController->setNumSmartPaSupport(mNumSpeakerSupport);
    setSpkApSwMixSupport();
...
status_t AudioALSAHardwareResourceManager::stopOutputDeviceWithoutNotify() {
    ALOGD("+%s(), mOutputDevices = 0x%x, mStartOutputDevicesCount = %d", __FUNCTION__, mOutputDevices, mStartOutputDevicesCount);
    ...
    if (mStartOutputDevicesCount > 1) {
        ...
        if (mStartOutputDevicesCount == 1 && mOutputDevices != AUDIO_DEVICE_NONE) {
            stopOutputDeviceWithoutNotify_l();
            mOutputDevices = AUDIO_DEVICE_NONE;
        } else {
            ASSERT(0);
            if (mStartOutputDevicesCount < 0) {
...
status_t AudioALSAHardwareResourceManager::startOutputDevice_l(const audio_devices_t new_devices, const uint32_t SampleRate) {
    ...
    if (isAdspOptionEnable()) {
        AudioDspStreamManager::getInstance()->startDevice(mOutputDevices);
    }
#endif

    notifyOutputDeviceStatusChange(mOutputDevices, DEVICE_STATUS_ON, SampleRate);
    return NO_ERROR;
}
...
status_t AudioALSAHardwareResourceManager::stopOutputDevice_l() {
    stopOutputDeviceWithoutNotify_l();
    notifyOutputDeviceStatusChange(mOutputDevices, DEVICE_STATUS_OFF, 0);
    mOutputDevices = AUDIO_DEVICE_NONE;
    return NO_ERROR;
}

status_t AudioALSAHardwareResourceManager::stopOutputDeviceWithoutNotify_l() {
...
#if defined(MTK_AUDIODSP_SUPPORT)
    if (isAdspOptionEnable()) {
        AudioDspStreamManager::getInstance()->stopDevice(mOutputDevices);
    }
#endif
...
#endif
    // Set mOutputDevices in stopOutputDeviceWithoutNotify for notify correct device in stopOutputDevice_l
    return NO_ERROR;
}
...
bool AudioALSAHardwareResourceManager::isSharedOutDevice(audio_devices_t device) {
    ...
    audio_devices_t sharedDevice = (audio_devices_t)(AUDIO_DEVICE_OUT_EARPIECE |
    ...
    if (isUsbSpkDevice(device)) {
        return true;
    }

    if (isEarphoneSpkDevice(device)) {
        return true;
    }

    if (isa2dpSpkDevice(device)) {
        return true;
    }
...
int AudioALSAHardwareResourceManager::setI2sSequenceHD(bool enable, const char *i2sStr) {
    ...
    if (strncmp(i2sStr, "I2S0", strlen(i2sStr)) == 0 ||
        ...
        i2sNum = AUDIO_I2S3;
    } else if (strncmp(i2sStr, "I2S5", strlen(i2sStr)) == 0 ||
               strncmp(i2sStr, "TINYCONN_I2S5", strlen(i2sStr)) == 0) {
        i2sNum = AUDIO_I2S5;
    } else {
...
bool AudioALSAHardwareResourceManager::is32BitI2sSupport(void) {
    String8 i2sIn = mSmartPaController->getI2sNumSequence(true);
    if (strstr(i2sIn.string(), "TINYCONN")) {
        return true;
    }
...
status_t AudioALSAHardwareResourceManager::applySettingSequence(String8 sequence) {
    int ret = NO_ERROR;
    ALOGV("%s(), sequence: %s", __FUNCTION__, sequence.string());

    if (!sequence.isEmpty()) {
        ret = mDeviceConfigManager->ApplyDeviceSettingByName(sequence);
        ASSERT(ret == NO_ERROR);
    } else {
        ret = -1;
    }
...
String8 AudioALSAHardwareResourceManager::getMicRecordPath(unsigned int channels) {
    ...
    if (mHDRRecordOn) {
        if (channels == 2) {
            ...
            ALOGE("%s(), Not support channel: %d of HRD record!", __FUNCTION__, channels);
            ASSERT(0);
            str = String8("HDR") + AUDIO_CTL_ADDA_TO_CAPTURE1_4CH;
        }
        return str;
    }
...
status_t AudioALSAHardwareResourceManager::startInputDevice(const audio_devices_t new_device) {
    ...
    if (!isPmicInputDevice(new_device)) {
    ...
    case AUDIO_DEVICE_IN_BUILTIN_MIC:
        ...
        default:
            if (mNumPhoneMicSupport == 2) {
                ...
                if (mHDRRecordOn) {
                    specificMic = String8("HDR_") + specificMic;
                }
    ...
    case AUDIO_DEVICE_IN_WIRED_HEADSET:
        ...
        if (mHDRRecordOn) {
            deviceName = deviceName + String8("_HDR");
        }
        break;
    default:
    ...
    if (AudioALSADriverUtility::getInstance()->mixerCtrlGetValueByName("DMic Used", 0) > 0) {
        mDeviceConfigManager->ApplyDeviceSettingByName(AUDIOMIC_TYPE_DMICMODE);
    }
    ...
    mInputDevice = (audio_devices_t)(mInputDevice | new_device);
    mStartInputDeviceCount++;
...
status_t AudioALSAHardwareResourceManager::stopInputDevice(const audio_devices_t stop_device) {
    ...
    if (!isPmicInputDevice(stop_device)) {
        ALOGD("%s(), no need to config analog part", __FUNCTION__);
        return NO_ERROR;
    }

    if (mStartInputDeviceCount == 0) {
        ALOGD("%s(), already stop", __FUNCTION__);
        return NO_ERROR;
    }
    ...
    mInputDevice = (audio_devices_t)(mInputDevice & ((~stop_device) | AUDIO_DEVICE_BIT_IN));
...
status_t AudioALSAHardwareResourceManager::setHWGain2DigitalGain(const uint32_t gain, int hwGainNum) {
    ALOGD("%s(), gain = 0x%x, hwGainNum: %d", __FUNCTION__, gain, hwGainNum);

#if defined(HW_POSITIVE_GAIN_SUPPORT)
    const uint32_t kMaxAudioHWGainValue = 0x8E432E; // +25 db
#else
    const uint32_t kMaxAudioHWGainValue = 0x80000; // 0 db
#endif
    String8 hwGainNumStr = (hwGainNum == AUDIO_HW_GAIN_1) ? String8("HW Gain 1") : String8("HW Gain 2");
    ...
    int retval = 0;
#if defined(MTK_AUDIO_KS)
    retval = mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, hwGainNumStr.string()), 0, gain);
#else
...
status_t AudioALSAHardwareResourceManager::openAddaOutput(const uint32_t sample_rate,
    ...
    String8 sequence = getOutputTurnOnSeq(devices, false, AUDIO_CTL_PLAYBACK1);
    enableTurnOnSequence(sequence);
#else
    UNUSED(devices);
...
#if defined(MTK_AUDIO_KS)
    String8 sequence = getOutputTurnOnSeq(mOutputDevices, false, AUDIO_CTL_PLAYBACK1);
    disableTurnOnSequence(sequence);
#endif
...
status_t AudioALSAHardwareResourceManager::setSgenMode(const sgen_mode_t sgen_mode) {
    if (sgen_mode < 0)  {
        ALOGE("%s(), invalid sgen_mode %d", __FUNCTION__, sgen_mode);
        return UNKNOWN_ERROR;
    }
...
#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
bool AudioALSAHardwareResourceManager::setOutputDevicesChangeCb(OutputDevicesChangeCb cb) {
    mOutputDevicesChangeCb = cb;
    return true;
}
#endif

bool AudioALSAHardwareResourceManager::notifyOutputDeviceStatusChange(const audio_devices_t device, const DeviceStatus status, const uint32_t sampleRate) {
    bool ret = true;

    if (device & AUDIO_DEVICE_OUT_SPEAKER) {
        AL_AUTOLOCK(mLockSpeakerCount);
        ALOGD("%s(), device = %d, status = %d, sampleRate = %d, mOpenSpeakerPathCount = %d", __FUNCTION__, device, status, sampleRate, mOpenSpeakerPathCount);
        if (mSpeakerStatusChangeCb != NULL) {
            // mOpenSpeakerPathCount is updated after speaker is on/off
            if (status == DEVICE_STATUS_OFF) {
                // notify speaker status off when the last speaker off comes
                if (mOpenSpeakerPathCount == 0) {
                    ret = mSpeakerStatusChangeCb(status, sampleRate);
                }
            } else {
                // notify speaker status on when the first speaker on comes
                if (mOpenSpeakerPathCount == 1) {
                    ret = mSpeakerStatusChangeCb(status, sampleRate);
                }
            }
        }
...
#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
    if (mOutputDevicesChangeCb != NULL) {
        mOutputDevicesChangeCb(device, status);
    }
#endif
...
bool AudioALSAHardwareResourceManager::getMicInverse(void) {
    if (appIsFeatureOptionEnabled("MTK_AUDIO_MIC_INVERSE")) {
        ALOGD("%s(), return = %d, mMicInverse = %d", __FUNCTION__, !mMicInverse, mMicInverse);
        return !mMicInverse;
    } else {
        ALOGD("%s(), return = %d, mMicInverse = %d", __FUNCTION__, mMicInverse, mMicInverse);
        return mMicInverse;
...
int AudioALSAHardwareResourceManager::setNonSmartPAType() {
    const char *spkType = appGetFeatureOptionValue("MTK_AUDIO_SPEAKER_PATH");
    if (!spkType) {
        ALOGE("%s(), Error: cannot get MTK_AUDIO_SPEAKER_PATH", __FUNCTION__);
        return -ENOENT;
    }
...
void AudioALSAHardwareResourceManager::setA2dpDeviceLatency(int latency)
{
    if (!latency)
        mA2dpDeviceLatency = A2DP_DEFAULT_LANTENCY;
    else
        mA2dpDeviceLatency = latency;
}
...
int AudioALSAHardwareResourceManager::getTVOutFileDescriptor(void) {
#if defined(MTK_AUDIO_TVOUT_SUPPORT)
    AL_AUTOLOCK(mGetTVFdLock);

    if (mTVOutFd < 0) {
        mTVOutFd = open(DRM_HDMI_DEV_DRV, O_RDWR);
        ALOGD("%s(), open %s, mTVOutFd %d", __FUNCTION__, DRM_HDMI_DEV_DRV, mTVOutFd);
        if (mTVOutFd < 0) {
            ALOGE("%s(), open(%s) fail, fd = %d, errno: %d",
                  __FUNCTION__, DRM_HDMI_DEV_DRV, mTVOutFd, errno);
        }
    }
    ASSERT(mTVOutFd >= 0);
#endif
...
void AudioALSAHardwareResourceManager::setTVOutFileDescriptor(int fd) {
    AL_AUTOLOCK(mGetTVFdLock);
    mTVOutFd = fd;
}

int AudioALSAHardwareResourceManager::setNumSpkSupport() {
    // default set speaker num as 1
    int spkNum = 1;
    const char *spkNumStr = appGetFeatureOptionValue("MTK_AUDIO_NUMBER_OF_SPEAKER");
    if (!spkNumStr) {
        ALOGE("%s(), parse spkNumStr error, %s", __FUNCTION__, spkNumStr);
        return spkNum;
    }

    if (sscanf(spkNumStr, "%d", &spkNum) <= 0) {
        ALOGE("%s(), spkNumStr convert to digital error, %s", __FUNCTION__, spkNumStr);
    }

    ALOGD("%s() spk number: %d", __FUNCTION__, spkNum);

    return spkNum;
}

int AudioALSAHardwareResourceManager::getSpkNum() {
    return mNumSpeakerSupport;
}

void AudioALSAHardwareResourceManager::setSpkApSwMixSupport() {
    if (appIsFeatureOptionEnabled("MTK_SPK_AP_SWMIX_PROCESS")) {
        mSpkApSwMixUsed = true;
    } else {
        mSpkApSwMixUsed = false;
    }
}

bool AudioALSAHardwareResourceManager::isSpkApSwMixSupport(audio_devices_t devices) {
    if (devices & AUDIO_DEVICE_OUT_SPEAKER) {
        return mSpkApSwMixUsed;
    }

    return false;
}

void AudioALSAHardwareResourceManager::setHDRRecord(bool enable) {
    mHDRRecordOn = enable;
#ifdef MTK_NEW_VOL_CONTROL
    if (enable) {
        AudioMTKGainController::getInstance()->setScene("HDR_RECORD");
    } else {
        AudioMTKGainController::getInstance()->setScene("Default");
    }
#endif
}

#ifdef MTK_ULTRASOUND_PROXIMITY_SUPPORT
status_t AudioALSAHardwareResourceManager::startForceUseReceiver(const uint32_t SampleRate) {
    AL_AUTOLOCK(mLock);
    if (mForceUseReceiver != false) {
        ALOGD("%s() should not start twice: %d", __FUNCTION__, mForceUseReceiver);
        AUD_ASSERT(0);
    } else {
        OpenReceiverPath(SampleRate);
        mForceUseReceiver = true;
        ALOGD("%s() sample rate=%d", __FUNCTION__, SampleRate);
    }
    return NO_ERROR;
}

status_t AudioALSAHardwareResourceManager::stopForceUseReceiver() {
    AL_AUTOLOCK(mLock);
    if (mForceUseReceiver == false) {
        ALOGD("%s() should not stop without start: %d", __FUNCTION__, mForceUseReceiver);
        AUD_ASSERT(0);
    } else {
        CloseReceiverPath();
        mForceUseReceiver = false;
...
bool AudioALSAHardwareResourceManager::getForceUseReceiver() {
    AL_AUTOLOCK(mLock);
    return mForceUseReceiver;
}
#endif
