...
void AudioALSADeviceParser::getCardName() {
    ...
    if (doc.LoadFile(AUDIO_DEVICE_EXT_CONFIG_FILE) == XML_SUCCESS) {
        XMLElement *root = doc.FirstChildElement("root");

        if (root) {
            XMLElement *card = root->FirstChildElement("card");
        ...
        ALOGE("%s(), LoadFile fail, %s err(%d) %s", __FUNCTION__, AUDIO_DEVICE_EXT_CONFIG_FILE,
...
void AudioALSADeviceParser::ParseCardIndex() {
    ...
    if (mCardFile) {
        ...
        while (!feof(mCardFile)) {
            if (fgets(tempbuffer, PROC_READ_BUFFER_SIZE, mCardFile) == NULL) {
                goto release_source;
            }
            ...
            if (strchr(tempbuffer, '[')) {  // this line contain '[' character
                ...
                if (strstr(Rch, mCardName.string())) {
...
void AudioALSADeviceParser::SetPcmCapability(AudioDeviceDescriptor *Descriptor, const char *Buffer) {
    if (Buffer == NULL) {
        return;
    }
...
void AudioALSADeviceParser::GetAllPcmAttribute(void) {
    ...
    if (mPcmFile) {
        ...
        while (!feof(mPcmFile)) {
            if (fgets(tempbuffer, PROC_READ_BUFFER_SIZE, mPcmFile) == NULL) {
                ALOGE("%s fget error", __func__);
                break;
            }
            else {
    ...
    if (mPcmFile) {
        if (fclose(mPcmFile) != 0) {
            ALOGD("%s() fclose mPcmFile fail", __FUNCTION__);
        }
    }
}
...
void AudioALSADeviceParser::GetAllCompressAttribute(void) {
    ...
    if (result < 0) {
        ALOGD("%s(), Soundcard path fail result = %d", __FUNCTION__, result);
        return;
    }
    ...
    while ((de = readdir(d)) != NULL) {
        if (strncmp(de->d_name, "compr", 5) == 0) {
            ...
            if (compress_file) {
                ...
                while (fgets(tempbuffer, PROC_READ_BUFFER_SIZE, compress_file) != NULL) {
                    ...
                    if (Rch  != NULL) {
                        if (strncmp(Rch, "card", 4) == 0) {
                            ...
                            Rch = strtok_r(NULL, " ", &rest_of_str);
                            mAudioDeviceDescriptor->mStreamName = String8(Rch);
                        }
                    }
                    memset((void *)tempbuffer, 0, PROC_READ_BUFFER_SIZE);
                }
                mAudioComprDevVector.push(mAudioDeviceDescriptor);
                if (fclose(compress_file) != 0) {
                    ALOGD("%s() fclose compress_file fail", __FUNCTION__);
                }
            } else {
