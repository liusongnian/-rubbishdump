...
AudioSpeechEnhanceInfo::AudioSpeechEnhanceInfo() {
    ...
    if (snprintf(property_default_value, sizeof(property_default_value), "0x%x", VOIP_SPH_ENH_DYNAMIC_MASK_ALL) < 0) {
        ALOGE("%s(), snprintf property_default_value fail!!", __FUNCTION__);
    }
    ...
    if (sscanf(property_value, "0x%x", &mVoIPSpeechEnhancementMask.dynamic_func) != 1) {
        ALOGE("%s(), sscanf fail!!", __FUNCTION__);
    }
    ...
    //debug purpose++
    mDebugflag = false;
    int bflag = AudioALSADriverUtility::getInstance()->GetPropertyValue(streamin_log_propty);
    if (bflag) {
        mDebugflag = true;
    }
    //debug purpose--
...
void AudioSpeechEnhanceInfo::UpdateDynamicSpeechEnhancementMask(const voip_sph_enh_mask_struct_t &mask) {
    ...
    if (feature_support & (SUPPORT_DMNR_3_0 | SUPPORT_VOIP_ENHANCE)) {
        ...
        if (snprintf(property_value, sizeof(property_value), "0x%x", mask.dynamic_func) < 0) {
            ALOGE("%s(), snprintf property_value fail!!", __FUNCTION__);
        }
