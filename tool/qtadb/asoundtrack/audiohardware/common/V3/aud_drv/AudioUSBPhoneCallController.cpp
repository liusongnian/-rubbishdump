...
int AudioUSBPhoneCallController::enable(unsigned int speechRate) {
    ...
    while (!mAudioHWReady) {
        ...
        if (sleepTotal_us >= 3 * 1000 * 1000) {
            ...
            ALOGE("%s(), speechDLThread mDebugLine = %d, speechULThread mDebugLine = %d", __FUNCTION__, mUSBOutStream.mDebugLine, mUSBInStream.mDebugLine);
            ASSERT(0);
            break;
        }
        sleepTotal_us += 500;
...
int AudioUSBPhoneCallController::setUSBOutConnectionState(audio_devices_t devices, bool connect, int card, int device) {
    ...
    if (audio_is_usb_out_device(devices)) {
        ...
        if (connect) {
            ...
            mOutputDevice = (audio_devices_t)devices;
        }
    }
...
int AudioUSBPhoneCallController::setUSBInConnectionState(audio_devices_t devices, bool connect, int card, int device) {
    ...
    if (audio_is_usb_in_device(devices)) {
        if (connect) {
            ...
            mInputDevice = (audio_devices_t)devices;
        }
        ...
        if (usbInConnectedAfterEnable && ((mDebugType & USB_DBG_USE_DL_ONLY) == 0)) {
            ...
            // update speech mode
            pSpeechDriver->SetSpeechMode(mInputDevice, mOutputDevice);
        ...
        if (usbInConnectedAfterEnable && ((mDebugType & USB_DBG_USE_DL_ONLY) == 0)) {
            ...
            volumeController->setVoiceVolume(volumeController->getVoiceVolume(), AUDIO_MODE_IN_CALL, mOutputDevice);
        }
    }
...
audio_devices_t AudioUSBPhoneCallController::getUSBCallInDevice() {
    ...
    return mUSBInConnected ? mInputDevice : USB_SPH_PRIMARY_IN_DEVICE;
}
...
void *AudioUSBPhoneCallController::speechDLThread(void *arg) {
    ...
    int ret = 0;
    bool usbReady = true;
    controller->mUSBOutStream.mDebugLine = __LINE__;
...
void *AudioUSBPhoneCallController::speechULThread(void *arg) {
    ...
    int ret = 0;
    bool usbReady = true;
    controller->mUSBInStream.mDebugLine = __LINE__;
    audio_sched_setschedule(0, SCHED_RR, sched_get_priority_min(SCHED_RR));
    ...
    while (controller->isEnable()) {
        ...
        if (controller->mLpbkType) {
            if (controller->mLpbkStart && controller->mLpbkType == USB_LPBK_NORMAL) {
                ...
                clock_gettime(CLOCK_MONOTONIC, &controller->mLpbkNewTime);
            ...
            if (!controller->mLpbkStart && controller->mLpbkTime > 0.1f) {
                ...
                *(tmpBuffer + 1) = 0x7fff;
                ...
                clock_gettime(CLOCK_MONOTONIC, &controller->mLpbkOldTime);
    ...
    pcmDumpClose(pcmULAWSOUTDumpFile);
    pcmDumpClose(pcmULSRCOUTDumpFile);
    // close audio hw
...
FILE *AudioUSBPhoneCallController::pcmDumpOpen(const char *name, const char *property, unsigned int rate, audio_format_t fmt, unsigned int chnum) {
    ...
    if (name == NULL) {
        ALOGE("%s name = NULL", __FUNCTION__);
        return NULL;
    }
    if (property == NULL) {
        ALOGE("%s property = NULL", __FUNCTION__);
        return NULL;
    }
    if (strlen(name) > 128) {
        ALOGE("%s name: %s, strlen=%zu, too long", __FUNCTION__, name, strlen(name));
        return NULL;
    }

    int ret = snprintf(dumpFileName, sizeof(dumpFileName), "%s.%s.%u.pid%d.tid%d.%d.%s.%dch.pcm", USB_SPH_DUMP_FILE_PREFIX, name, dumpFileCount, getpid(), gettid(), rate, transferAudioFormatToDumpString(fmt), chnum);
    if (ret < 0 || ret >= sizeof(dumpFileName)) {
        ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d", __FUNCTION__,
              dumpFileName, sizeof(dumpFileName), ret);
        return NULL;
    }
...
status_t AudioUSBPhoneCallController::initBliSrc(struct USBStream *stream) {
    ...
    if (stream->blisrc == NULL) {
        ALOGE("%s(), stream->blisrc = NULL", __FUNCTION__);
        return NO_MEMORY;
    }
    stream->blisrc->open();
...
bool AudioUSBPhoneCallController::getLpbkTime(unsigned int idx, void *buffer, unsigned int bufferSize,
    ...
    // lpbk test
    if (mLpbkStart) {
        clock_gettime(CLOCK_MONOTONIC, &mLpbkNewTime);
