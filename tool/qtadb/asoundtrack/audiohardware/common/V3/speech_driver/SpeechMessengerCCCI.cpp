...
int32_t SpeechMessengerCCCI::SendMessage(const ccci_buff_t &ccci_buff) {
    ...
    if (b_need_ack == true) {
        ...
        ALOGD("%s(), message: 0x%x, b_need_ack: %d, mWaitAckMessageID is 0x%x",
              __FUNCTION__, ccci_buff.message, b_need_ack, mWaitAckMessageID);
    }
...
void *SpeechMessengerCCCI::CCCIReadThread(void *arg) {
    ...
    while (pCCCI->CCCIEnable) {
        ...
        default: {
            ALOGD("%s(), Read modem message(0x%x), don't care. (or no such message), mWaitAckMessageID is 0x%x",
                  __FUNCTION__, ccci_buff.message, pCCCI->mWaitAckMessageID);
            sph_msleep(10);
            break;
        }
        }
        ...
        if (pCCCI->JudgeAckOfMsg(a2m_message_id_of_m2a_ack) == MESSAGE_NEED_ACK) {
            if (a2m_message_id_of_m2a_ack == pCCCI->mWaitAckMessageID) {
                ...
                ALOGW("%s(), Message(0x%x) might double ack!! The current mWaitAckMessageID is 0x%x",
                      __FUNCTION__, ccci_buff.message, pCCCI->mWaitAckMessageID);
            }
        }
    }
...
bool SpeechMessengerCCCI::A2MBufLock() {
    ...
    int rc = AL_LOCK_MS_NO_ASSERT(mA2MShareBufMutex, kA2MBufLockTimeout);
    if (rc != 0) {
...
bool SpeechMessengerCCCI::SpeechParamLock() {
    ...
    ALOGD("%s()", __FUNCTION__);
    int rc = AL_LOCK_MS_NO_ASSERT(mSetSpeechParamMutex, kSphParamLockTimeout);
    if (rc != 0) {
