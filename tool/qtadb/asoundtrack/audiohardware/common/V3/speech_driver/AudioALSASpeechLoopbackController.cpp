...
status_t AudioALSASpeechLoopbackController::open(const audio_devices_t output_devices,
    ...
    modem_index_t mdIndex = mSpeechDriverFactory->GetActiveModemIndex();
...
#endif
    {
        ...
        if (AudioSmartPaController::getInstance()->isSwDspSpkProtect(output_devices)) {
...
#if defined(MTK_AUDIODSP_SUPPORT)
        if (AudioDspCallFinal::getInstance()->isOutputDevCallFinalSupport(output_devices)) {
            AudioDspCallFinal::getInstance()->initCallFinalTask(input_device, output_devices, sample_rate, mdIndex);
        } else
#endif
        {

            mApTurnOnSequence = mHardwareResourceManager->getOutputTurnOnSeq(output_devices, false,
            ...
            if ((output_devices & AUDIO_DEVICE_OUT_SPEAKER) &&
                ...
                popcount(output_devices) > 1) {
                mApTurnOnSequence2 = mHardwareResourceManager->getOutputTurnOnSeq(output_devices, true,
                                                                                  mdIndex == MODEM_1 ? AUDIO_CTL_MD1 : AUDIO_CTL_MD2);
            }
    ...
    mHardwareResourceManager->startInputDevice(input_device);
#endif
    usleep(50 * 1000); //Add delay to avoid mic pop when loopback on
...
#else
    sample_rate  = (bt_device_on == true) ? 8000 : 16000;
#endif
