 ...
 * lib related
 */
enum ADSP_CALL_ENH_CONFIG_T{
	ADSP_CALL_ENH_CONFIG_ENABLE     = 1 << 0,
	ADSP_CALL_ENH_CONFIG_CORE_APART = 1 << 1,
};
...
#define DUMP_DSP_PCM_DATA_PATH "/data/vendor/audiohal/audio_dump/"
#ifdef MTK_AUDIODSP_SUPPORT
#define EPL_DUMP_ON 3
#define EPL_DUMP_OFF 0
#endif
...
SpeechDriverOpenDSP *SpeechDriverOpenDSP::GetInstance(
...
SpeechDriverOpenDSP::SpeechDriverOpenDSP(modem_index_t modem_index) :
#if defined(MTK_AUDIO_IPI_SUPPORT)
    ...
    mEnableDump = false;
#ifdef MTK_AUDIODSP_SUPPORT
    mEnableEPLDump = false;
#endif
    mSuperVolumeEnable = false;
    ret = snprintf(mScenekeyword, sizeof("Default"), "Default");
    if ((ret)<0) {
        ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
    }
    ret = snprintf(mBtcodec, sizeof("cvsd"), "cvsd");
    if ((ret)<0) {
        ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
    }
...
#if defined(MTK_COMBO_MODEM_SUPPORT)/* SWB */
    ...
    mAdspPhoneCallConfig = 0;
    mUsipMemoryHandler = open(kUsipDeviceName, O_RDWR);
    if (mUsipMemoryHandler < 0) {
        ALOGE("%s(), adspPhoneCallConfig=%llx", __FUNCTION__, mAdspPhoneCallConfig);
    } else {
        ::ioctl(mUsipMemoryHandler, GET_USIP_ADSP_PHONE_CALL_ENH_CONFIG, &mAdspPhoneCallConfig);
        ALOGD("%s(), adspPhoneCallConfig=%llx", __FUNCTION__, mAdspPhoneCallConfig);

        close(mUsipMemoryHandler);
    }

    /* Network callback */
    AudioEventThreadManager::getInstance()->registerCallback(AUDIO_EVENT_SPEECH_NETWORK_CHANGE, callbackSpeechNetworkChange, this);
...
void SpeechDriverOpenDSP::SetArsiCustInfo(){
    ...
    if (mSuperVolumeEnable) {
        ret = snprintf(pManagerConfig->custom_info, MAX_CUSTOM_INFO_LEN, "vol_level=extra");
    } else {
        ret = snprintf(pManagerConfig->custom_info, MAX_CUSTOM_INFO_LEN, "vol_level=%d", mVolumeIndex);
    }
    if (ret < 0) {
        ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
    }
    ...
    if (appOps && appOps->appHandleIsFeatureOptionEnabled(appOps->appHandleGetInstance(), "VIR_SCENE_CUSTOMIZATION_SUPPORT")) {
        ret = snprintf(temp, sizeof(temp), ",SetAudioCustomScene=%s", mScenekeyword);
        if (ret < 0) {
            ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
        }
        strcat(pManagerConfig->custom_info, temp);
    }
#endif
    if (mIsLoopBack){
        ret = snprintf(temp, sizeof(temp), ",lpbk_switch=%d,lpbk_type=%d,lpbk_device=%d", mIsLoopBack, mIpcLpbkSwitch, mIpcPath);
        if (ret < 0) {
            ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
        }
        strcat(pManagerConfig->custom_info, temp);
        pManagerConfig->task_config.network_info = 0;
    } else if(ipc_cust_info && !mIsLoopBack){
        ret = snprintf(temp, sizeof(temp), ",lpbk_switch=%d", mIsLoopBack);
        if (ret < 0) {
            ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
        }
        strcat(pManagerConfig->custom_info, temp);
    ...
    if (audio_is_bluetooth_sco_device(mOutputDevices)){
        ret = snprintf(temp, sizeof(temp), ",bt_codec=%s", mBtcodec);
        if (ret < 0) {
            ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
        }
        strcat(pManagerConfig->custom_info, temp);
...
void SpeechDriverOpenDSP::CreateAurisysLibManager() {
    ...
    if (mAurisysDspConfig == NULL) {
        ALOGE("%s(), %p get NULL!!", __FUNCTION__, mAurisysDspConfig);
        return;
    }
    ...
    ALOGD("manager config: aurisys_scenario %u, core_type %d, " \
          ...
          pManagerConfig->task_config.network_info,
    ...
    if (configAndParam == NULL) {
        ALOGE("%s(), fail!! configAndParam is NULL", __FUNCTION__);
    } else {

        if (isAdspPhoneCallCoreSeparate()) {
            pManagerConfig->arsi_process_type = ARSI_PROCESS_TYPE_UL_ONLY;
        }

        memcpy(configAndParam, mAurisysDspConfig, sizeof(struct aurisys_dsp_config_t));
        if (paramList.p_buffer == NULL) {
            ALOGE("%s(), only config but no paramList", __FUNCTION__);
        } else {
        ...
        if (isAdspPhoneCallCoreSeparate()) {
            ...
            retval = mAudioMessengerIPI->sendIpiMsg(
                    &msg,
                    TASK_SCENE_PHONE_CALL_SUB, AUDIO_IPI_LAYER_TO_DSP,
                    AUDIO_IPI_DMA, AUDIO_IPI_MSG_NEED_ACK,
                    IPI_MSG_A2D_AURISYS_INIT,
                    sizeof(struct aurisys_dsp_config_t) + paramList.data_size,
                    0,
...
void SpeechDriverOpenDSP::UpdateAurisysConfig(const bool is_enh_need_reset) {
    ...
    /* task config */
    SetArsiTaskConfig(&pManagerConfig->task_config);
    ALOGD("%s network_info=%d, custom_info:%s", __FUNCTION__, pManagerConfig->task_config.network_info, pManagerConfig->custom_info);
    ...
    if (is_enh_need_reset) {
        ...
        if (configAndParam == NULL) {
            ...
            if (paramList.p_buffer == NULL) {
                ALOGE("%s(), only config but no paramList skip sending!!", __FUNCTION__);
            } else {
        ...
        if (configAndParam == NULL) {
            ...
            if (paramList.p_buffer == NULL) {
                ALOGE("%s(), no paramList skip sending!!", __FUNCTION__);
            } else {
...
#ifdef MTK_AUDIODSP_SUPPORT
void SpeechDriverOpenDSP::EnableEPLDUmp() {
    sph_info_t sphInfo;

    memset(&sphInfo, 0, sizeof(sph_info_t));
    SpeechConfig::getInstance()->getDriverParam(DRIVER_PARAM_COMMON_PAR, sphInfo.drv_common_param);
    if (SpeechVMRecorder::getInstance()->getVmConfig() == SPEECH_VM_SPEECH) {
        SpeechConfig::getInstance()->getDriverParam(DRIVER_PARAM_DEBUG_INFO, sphInfo.drv_debug_info);
        if (mEnableEPLDump == false && sphInfo.drv_debug_info[0] == EPL_DUMP_ON) {
            mEnableEPLDump = true;
            aurisys_set_parameter("DSP,PHONE_CALL,LIB_ALL,ENABLE_LIB_DUMP,1=SET"); // open 3rd party lib dump
        } else if (mEnableEPLDump == true && sphInfo.drv_debug_info[0] == EPL_DUMP_OFF) {
            mEnableEPLDump = false;
            aurisys_set_parameter("DSP,PHONE_CALL,LIB_ALL,ENABLE_LIB_DUMP,0=SET"); // close 3rd party lib dump
        }
    } else if (mEnableEPLDump == true) {
        mEnableEPLDump = false;
        aurisys_set_parameter("DSP,PHONE_CALL,LIB_ALL,ENABLE_LIB_DUMP,0=SET"); // close 3rd party lib dump
    }
}
...
static void putTimestampAndEnableToParam(uint32_t *param, const bool enable) {
    ...
    uint8_t time_value_h = 0;
    uint8_t time_value_m = 0;
    uint8_t time_value_s = 0;

    time_t rawtime;
    struct tm *timeinfo;

    if (time(&rawtime) == ((time_t) - 1)) {
        ALOGE("%s(), unknown time", __FUNCTION__);
    } else {
        ...
        if (timeinfo == NULL) {
            ...
            if (strftime(time_h, 3, "%H", timeinfo)) {
                time_value_h = (uint8_t)atoi(time_h);
            }
            if (strftime(time_m, 3, "%M", timeinfo)) {
                time_value_m = (uint8_t)atoi(time_m);
            }
            if (strftime(time_s, 3, "%S", timeinfo)) {
...
void SpeechDriverOpenDSP::createDumpFiles() {
    ...
    if (time(&rawtime) == ((time_t) - 1)) {
        ...
        if (timeinfo == NULL) {
            ...
            if (strftime(string_time, 100, "%Y_%m_%d_%H%M%S_", timeinfo) == 0) {
                ALOGE("%s(), no time info in pcm name", __FUNCTION__);;
            }
        }
    }

    if (sprintf(file_path_ul_in1, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "ul_in1.pcm") < 0) {
        isError = true;
    }
    if (sprintf(file_path_ul_in2, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "ul_in2.pcm") < 0) {
        isError = true;
    }
    if (sprintf(file_path_ul_in3, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "ul_in3.pcm") < 0) {
        isError = true;
    }
    if (sprintf(file_path_ul_ref1, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "ul_ref1.pcm") < 0) {
        isError = true;
    }
    if (sprintf(file_path_ul_ref2, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "ul_ref2.pcm") < 0) {
        isError = true;
    }
    if (sprintf(file_path_ul_out, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "ul_out.pcm") < 0) {
        isError = true;
    }
    if (sprintf(file_path_dl_in, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "dl_in.pcm") < 0) {
        isError = true;
    }
    if (sprintf(file_path_dl_out, "%s%s%s", DUMP_DSP_PCM_DATA_PATH, string_time, "dl_out.pcm") < 0) {
        isError = true;
    }
    if (isError) {
        ALOGE("%s(), unknown time info", __FUNCTION__);
...
status_t SpeechDriverOpenDSP::ScpSpeechOn() {
    ...
    if (isAdspPhoneCallCoreSeparate()) {
        mAudioMessengerIPI->registerAdspFeature(VOICE_CALL_SUB_FEATURE_ID);
        mAudioMessengerIPI->registerDmaCbk(
            TASK_SCENE_PHONE_CALL_SUB,
            0x10000,
            0x10000,
...
status_t SpeechDriverOpenDSP::ScpSpeechOff() {
    ...
    if (isAdspPhoneCallCoreSeparate()) {
        mAudioMessengerIPI->deregisterDmaCbk(TASK_SCENE_PHONE_CALL_SUB);
        mAudioMessengerIPI->deregisterAdspFeature(VOICE_CALL_SUB_FEATURE_ID);
    }
...
status_t SpeechDriverOpenDSP::TelephonyTxOn(uint16_t pcmTelephonyTxType) {
    return pSpeechDriverInternal->TelephonyTxOn(pcmTelephonyTxType);
}


status_t SpeechDriverOpenDSP::TelephonyTxConfig(uint16_t pcmTelephonyTxType) {
    return pSpeechDriverInternal->TelephonyTxConfig(pcmTelephonyTxType);
}
...
status_t SpeechDriverOpenDSP::SetDynamicDownlinkMute(bool mute_on) {
    ALOGD("%s(), mute_on = %d, old mDynamicDownlinkMuteOn = %d", __FUNCTION__, mute_on, mDynamicDownlinkMuteOn);
    mDynamicDownlinkMuteOn = mute_on;
    return pSpeechDriverInternal->SetDynamicDownlinkMute(mute_on);
}
...
void SpeechDriverOpenDSP::setMuteInfo(uint32_t muteInfo) {
    ALOGD("%s(), mMuteInfo: %d => %d",
          __FUNCTION__, mMuteInfo, muteInfo);
    mMuteInfo = muteInfo;
    pSpeechDriverInternal->setMuteInfo(muteInfo);
}
...
void SpeechDriverOpenDSP::setBtHeadsetName(const char *bt_headset_name) {
    return pSpeechDriverInternal->setBtHeadsetName(bt_headset_name);
}
...
int SpeechDriverOpenDSP::updateSpeechFeature(const SpeechFeatureType featureType, const bool flagOn) {
    ...
    } else if (SPEECH_FEATURE_BT_WB == featureType){
        if (flagOn) {
            snprintf(mBtcodec, SCENEKEYWORDLLENGTH, "msbc");
            ALOGD("%s(), SPEECH_FEATURE_BT_WB(%d)=%s", __FUNCTION__, flagOn, mBtcodec);
        }
        else {
            snprintf(mBtcodec, SCENEKEYWORDLLENGTH, "cvsd");
            ALOGD("%s(), SPEECH_FEATURE_BT_WB(%d)=%s", __FUNCTION__, flagOn, mBtcodec);
        }
        UpdateAurisysConfig(false);
    }
...
int SpeechDriverOpenDSP::updateCustScene(const char *custScene) {
    int ret = 0;
    if (strcmp(mScenekeyword, custScene) == 0) {
        ALOGD("%s(), custScene(%s), return",
              __FUNCTION__, custScene);
        return -ENOSYS;
    } else {
#if (defined(MTK_AUDIO_HIERARCHICAL_PARAM_SUPPORT) && (MTK_AUDIO_TUNING_TOOL_V2_PHASE >= 2))
        AppOps *appOps = appOpsGetInstance();
        if (appOps && appOps->appHandleIsFeatureOptionEnabled(appOps->appHandleGetInstance(), "VIR_SCENE_CUSTOMIZATION_SUPPORT")) {
            ret = snprintf(mScenekeyword, sizeof(custScene), "%s", custScene);
            if (ret < 0) {
                ALOGE("%s(), snprintf task_name fail!!", __FUNCTION__);
            }
            UpdateAurisysConfig(false);
            return pSpeechDriverInternal->updateCustScene(custScene);
        } else {
        ALOGD("%s(), custScene(%s), return (!appOps)",
              __FUNCTION__, custScene);
            return -ENOSYS;
        }
#endif
...
int SpeechDriverOpenDSP::setParameter(const char *keyParameter) {
    return pSpeechDriverInternal->setParameter(keyParameter);
}

const char *SpeechDriverOpenDSP::getParameter(const char *keyParameter) {
    return pSpeechDriverInternal->getParameter(keyParameter);
}

int SpeechDriverOpenDSP::updateCustInfo(const char *custInfo) {
    int ret = 0, value = 0;
    char custInfoKeyword[128] = {0};
    AudioParameter param = AudioParameter(String8(custInfo));

    ALOGD("%s(), custInfo: %s", __FUNCTION__, custInfo);
    if (param.getInt(String8("NbQuality"), value) == NO_ERROR) {
        if (value == 1) {
            ret = snprintf(custInfoKeyword, sizeof("NbQuality"), "NbQuality");
        } else {
            ret = snprintf(custInfoKeyword, sizeof("Default"), "Default");
        }

        if (strcmp(mScenekeyword, custInfoKeyword) == 0) {
            ALOGD("%s(), the same custInfoKeyword: %s, return", __FUNCTION__, custInfoKeyword);
            return -ENOSYS;
        } else {
#if (defined(MTK_AUDIO_HIERARCHICAL_PARAM_SUPPORT) && (MTK_AUDIO_TUNING_TOOL_V2_PHASE >= 2))
            AppOps *appOps = appOpsGetInstance();
            ret = snprintf(mScenekeyword, sizeof(mScenekeyword), "%s", custInfoKeyword);
            if (ret < 0) {
                ALOGE("%s(), snprintf task_name fail!!ret = %d", __FUNCTION__, ret);
            }
            UpdateAurisysConfig(false);
            return pSpeechDriverInternal->updateCustInfo(custInfo);
#endif
        }
    }
    return NO_ERROR;
...
bool SpeechDriverOpenDSP::isAdspPhoneCallEnhOn() {
    return (mAdspPhoneCallConfig & ADSP_CALL_ENH_CONFIG_ENABLE);
}

bool SpeechDriverOpenDSP::isAdspPhoneCallCoreSeparate() {
    return (mAdspPhoneCallConfig & ADSP_CALL_ENH_CONFIG_CORE_APART) == ADSP_CALL_ENH_CONFIG_CORE_APART;
}

void SpeechDriverOpenDSP::setAdspPhoneCallEnh(int value) {
    int mUsipMemoryHandler = ::open(kUsipDeviceName, O_RDWR);
    if (mUsipMemoryHandler < 0) {
        ALOGE("%s(), open fail to set adspPhoneCallConfig=%llx", __FUNCTION__, mAdspPhoneCallConfig);
    } else {
        mAdspPhoneCallConfig = value;
        ::ioctl(mUsipMemoryHandler, SET_USIP_ADSP_PHONE_CALL_ENH_CONFIG, &mAdspPhoneCallConfig);
        ALOGD("%s(), adspPhoneCallConfig=%llx", __FUNCTION__, mAdspPhoneCallConfig);
...
void SpeechDriverOpenDSP::callbackSpeechParamChange(int audioEventType, void *caller, void *arg) {
    ALOGD("%s(), audioEventType = %d, caller(%p), arg(%p)",
          __FUNCTION__, audioEventType, caller, arg);
 ...
 *                     IPC Control
 *============================================================================*/
int SpeechDriverOpenDSP::sendIpcCmd(int cmd) {
    return pSpeechDriverInternal->sendIpcCmd(cmd);
}
