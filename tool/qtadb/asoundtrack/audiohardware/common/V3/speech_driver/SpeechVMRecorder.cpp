...
static char gVMFileName[kMaxSizeOfVMFileName] = {0};
static uint32_t gSizeOfVMFileName = 0;
...
static vm_file_info_t *gVMFileList = NULL;
...
static int getVMFileList(const char *path, const struct stat *sb, int typeflag __unused) {
    if (strncmp(path, gVMFileName, gSizeOfVMFileName) != 0) {
        return 0;
    }
...
SpeechVMRecorder::SpeechVMRecorder() {
    ...
    ret = snprintf(property_value, sizeof(property_value), "%d", (uint8_t) sphParamNB.uAutoVM);
...
#endif
    ret = snprintf(gVMFileName, sizeof(gVMFileName), "%s%s", kFolderOfVMFile, "VMLog");
    if (ret < 0 || ret >= sizeof(gVMFileName)) {
        ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d",
              __FUNCTION__, gVMFileName, sizeof(gVMFileName), ret);
    }
    gSizeOfVMFileName = strlen(gVMFileName) - 1;
...
int SpeechVMRecorder::configVm(const uint8_t vmConfig) {
    ...
    char property_value[PROPERTY_VALUE_MAX] = {0};
    int ret = snprintf(property_value, sizeof(property_value), "%d", vmConfig);
    if (ret < 0 || ret >= sizeof(property_value)) {
        ALOGE("%s(), snprintf %s fail!! sz %zu, ret %d", __FUNCTION__, property_value, sizeof(property_value), ret);
    }
    property_set(PROPERTY_KEY_VM_CFG, property_value);
...
void *SpeechVMRecorder::dumpVMRecordDataThread(void *arg) {
    ...
    while (1) {
        ...
        if (!sd_card_buf) {
            ALOGE("%s(), sd_card_buf NULL", __FUNCTION__);
            AL_UNLOCK(pSpeechVMRecorder->mMutex);
            break;
        }
        if (!(pSpeechVMRecorder->mRingBuf.pRead) ||
            !(pSpeechVMRecorder->mRingBuf.pWrite) ||
            !(pSpeechVMRecorder->mRingBuf.pBufBase)) {
            ALOGE("%s(), mRingBuf NULL", __FUNCTION__);
            AL_UNLOCK(pSpeechVMRecorder->mMutex);
            break;
        }
    ...
    if (vm_recycle_on == true) {
        AL_LOCK(pSpeechVMRecorder->mThreadRecycleMutex);
        AUDIO_ALLOC_STRUCT_ARRAY(vm_file_info_t, kMaxNumOfVMFiles, gVMFileList);
        ...
        while ((gNumOfVMFiles > kMinKeepNumOfVMFiles && gTotalSizeOfVMFiles > kMaxSizeOfAllVMFiles) || gNumOfVMFiles >= kMaxNumOfVMFiles) {
            ALOGD("%s(), gNumOfVMFiles = %u, gTotalSizeOfVMFiles = %u",
                  __FUNCTION__, gNumOfVMFiles, gTotalSizeOfVMFiles);

            ALOGD("%s(), remove file, index = %u, path(%s), size = %u",
                  __FUNCTION__, index_vm_file_list, gVMFileList[index_vm_file_list].path, gVMFileList[index_vm_file_list].size);
        ...
        AUDIO_FREE_POINTER(gVMFileList);
        AL_UNLOCK(pSpeechVMRecorder->mThreadRecycleMutex);
    }
...
FILE *SpeechVMRecorder::openFile() {
    ...
    ret = snprintf(vm_file_path, sizeof(vm_file_path), "%s_%u.vm", gVMFileName, vm_file_number++);
    if (ret < 0 || ret >= sizeof(vm_file_path)) {
        ALOGE("%s(), snprintf %s fail!! sz %d, ret %d",
              __FUNCTION__, vm_file_path, sizeof(vm_file_path), ret);
    } else {
        gSizeOfVMFileName = strlen(gVMFileName) - 1;
    }

    ret = snprintf(property_value, "%u", vm_file_number & 0x7);
    if (ret < 0 || ret >= PROPERTY_VALUE_MAX) {
        ALOGE("%s(), snprintf %s fail!! sz %d, ret %d", __FUNCTION__, property_value, PROPERTY_VALUE_MAX, ret);
    }
    ...
    if (timeinfo != NULL) {
        audio_strncpy(vm_file_path, gVMFileName, kMaxSizeOfVMFileName);
        ret = strftime(vm_file_path + gSizeOfVMFileName, kMaxSizeOfVMFileName - gSizeOfVMFileName - 1,
                       "_%Y_%m_%d_%H%M%S.vm", timeinfo);
        if (ret == 0) {
            ALOGE("%s(), CtmUlIn strftime() fail!!", __FUNCTION__);
            return NULL;
        }
    } else {
        char property_value[PROPERTY_VALUE_MAX] = {0};
        ...
        uint8_t vm_file_number = atoi(property_value);
        ret = snprintf(vm_file_path, kMaxSizeOfVMFileName, "%s_%u.vm", gVMFileName, vm_file_number++);
        if (ret < 0 || ret >= kMaxSizeOfVMFileName) {
            ALOGE("%s(), snprintf %s fail!! sz %d, ret %d", __FUNCTION__, vm_file_path, kMaxSizeOfVMFileName, ret);
        }

        ret = snprintf(property_value, PROPERTY_VALUE_MAX, "%u", vm_file_number & 0x7);
        if (ret < 0 || ret >= PROPERTY_VALUE_MAX) {
            ALOGE("%s(), snprintf %s fail!! sz %d, ret %d", __FUNCTION__, property_value, PROPERTY_VALUE_MAX, ret);
        }
        property_set(PROPERTY_KEY_VM_INDEX, property_value);
    ...
    ret = AudiocheckAndCreateDirectory(vm_file_path);
    if (ret < 0) {
...
int SpeechVMRecorder::startCtmDebug() {
    ...
    int ret;
    ret = strftime(ctm_file_path_UlIn, kMaxPathLength,
                   "/data/vendor/audiohal/audio_dump/%Y_%m_%d_%H%M%S_CtmUlIn.pcm", timeinfo);
    if (ret == 0) {
        ALOGE("%s(), CtmUlIn strftime() fail!!", __FUNCTION__);
        return UNKNOWN_ERROR;
    }
    ret = strftime(ctm_file_path_DlIn, kMaxPathLength,
                   "/data/vendor/audiohal/audio_dump/%Y_%m_%d_%H%M%S_CtmDlIn.pcm", timeinfo);
    if (ret == 0) {
        ALOGE("%s(), CtmDlIn strftime() fail!!", __FUNCTION__);
        return UNKNOWN_ERROR;
    }
    ret = strftime(ctm_file_path_UlOut, kMaxPathLength,
                   "/data/vendor/audiohal/audio_dump/%Y_%m_%d_%H%M%S_CtmUlOut.pcm", timeinfo);
    if (ret == 0) {
        ALOGE("%s(), CtmUlOut strftime() fail!!", __FUNCTION__);
        return UNKNOWN_ERROR;
    }
    ret = strftime(ctm_file_path_DlOut, kMaxPathLength,
                   "/data/vendor/audiohal/audio_dump/%Y_%m_%d_%H%M%S_CtmDlOut.pcm", timeinfo);
    if (ret == 0) {
        ALOGE("%s(), CtmDlOut strftime() fail!!", __FUNCTION__);
        return UNKNOWN_ERROR;
    }
...
int SpeechVMRecorder::stopCtmDebug() {
    ...
    int ret = fclose(pCtmDumpFileUlIn);
    if (ret != 0) {
        ALOGE("fclose UlIn file error %d", ret);
    }
    ret = fclose(pCtmDumpFileDlIn);
    if (ret != 0) {
        ALOGE("fclose DlIn file error %d", ret);
    }
    ret = fclose(pCtmDumpFileUlOut);
    if (ret != 0) {
        ALOGE("fclose UlOut file error %d", ret);
    }
    ret = fclose(pCtmDumpFileDlOut);
    if (ret != 0) {
        ALOGE("fclose DlOut file error %d", ret);
    }
    return true;
}
...
void SpeechVMRecorder::getCtmDebugDataFromModem(RingBuf ul_ring_buf, FILE *pFile) {
    ...
    if (ShareBuf_dataCnt >= 0) {
        ...
        uint32_t write_bytes = fwrite(linear_buffer, sizeof(char), ShareBuf_dataCnt, pFile);
        if (write_bytes != ShareBuf_dataCnt) {
            ALOGE("%s(), fwrite fail! data_count: %d, write_bytes: %d", __FUNCTION__, ShareBuf_dataCnt, write_bytes);
        }
    }
}
