...
AudioALSAHandlerKtv::AudioALSAHandlerKtv() :
    ...
    for (uint16_t i = 0; i < DEBUG_PCMDUMP_NUM; i++) {
...
void AudioALSAHandlerKtv::get_task_pcmdump_info(uint16_t task_id, uint16_t param, void **pcm_dump) {
    *pcm_dump = pcmin_dump_array[param];
    ALOGV("%s() %p %d %d\n", __FUNCTION__, *((FILE **)pcm_dump), task_id, param);
}

void AudioALSAHandlerKtv::set_task_pcmdump_info(uint16_t task_id, uint16_t param, void *pcm_dump) {
...
#define MAX_TASKNAME_LEN (128)
    ...
    if (ktv_dump_on) {
        ...
        for (i = 0; i < DEBUG_PCMDUMP_NUM; i++) {
            ...
            if (pcm_dump == NULL) {
                ret = sprintf(mDumpFileName, "%s%s.%d.%d.%d.%s_point%d.pcm",
...
void AudioALSAHandlerKtv::setktvDspShareMem(bool enable) {
    if (mixer_ctl_set_value(mixer_get_ctl_by_name(mMixer, "adsp_ktv_sharemem_scenario"), 0, enable)) {
        ALOGW("%s(), enable sharemem fail", __FUNCTION__);
    }
}

void AudioALSAHandlerKtv::startKtvTask(const stream_attribute_t *attribute) {
    ...
    setktvDspShareMem(true);

    memset((void *)&mKtvUlConfig, 0, sizeof(struct pcm_config));
    memset((void *)&mKtvDlConfig, 0, sizeof(struct pcm_config));
    memset((void *)&mDspKtvConfig, 0, sizeof(struct pcm_config));

    memcpy(&attributeKtv, attribute, sizeof(stream_attribute_t));
    attributeKtv.num_channels = 2;
    attributeKtv.output_devices = AUDIO_DEVICE_OUT_WIRED_HEADSET;
    unsigned int multiplier = (attributeKtv.sample_rate / 96000) + 1;

    mKtvUlConfig.rate = attributeKtv.sample_rate;
    ...
    mKtvDlConfig.rate = attributeKtv.sample_rate;
    ...
    mDspKtvConfig.rate = attributeKtv.sample_rate;
    ...
    if (mDspKtvPcm == NULL) {
        ...
        pcm_close(mDspKtvPcm);
        mDspKtvPcm = NULL;
    } else if (pcm_start(mDspKtvPcm) != 0) {
    ...
    AudioALSAHardwareResourceManager::getInstance()->startOutputDevice(AUDIO_DEVICE_OUT_WIRED_HEADSET, mKtvDlConfig.rate); //TODO only headset?
}
...
void AudioALSAHandlerKtv::stopKtvTask(void) {
    ...
    AudioALSAHardwareResourceManager::getInstance()->stopOutputDevice(); //TODO only headset?
    if (mDspKtvPcm != NULL) {
        pcm_stop(mDspKtvPcm);
        pcm_close(mDspKtvPcm);
        mDspKtvPcm = NULL;
