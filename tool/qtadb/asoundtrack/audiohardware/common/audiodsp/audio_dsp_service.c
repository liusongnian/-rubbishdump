...
#define UEVENT_ADSP_ONLINE  "online@/devices/virtual/misc/adsp"
#define UEVENT_ADSP_OFFLINE "offline@/devices/virtual/misc/adsp"

static int open_adsp_uevent_socket(void) {
    int sock;
    ...
    if (sock < 0) {
        AUD_LOG_W("%s, error getting socket", __func__);
	return -1;
    }
...
void *read_adsp_calbak_thread(void *arg  __unused) {
    ...
    int sock = open_adsp_uevent_socket();

    pthread_detach(pthread_self());

    while (g_read_adsp_thread_enable && sock > 0) {
        n = recv(sock, msg, UEVENT_MSG_LEN, 0);

	if (n <= 0)
		continue;

	msg[UEVENT_MSG_LEN - 1] = '\0';

	if (!strncmp(msg, UEVENT_ADSP_ONLINE, UEVENT_MSG_LEN)) {
	    /* Call StreamManager API */
            ...
            AUD_LOG_D("%s, event: %s", __FUNCTION__, msg);
        } else if (!strncmp(msg, UEVENT_ADSP_OFFLINE, UEVENT_MSG_LEN)) {
            /* Call StreamManager API */
            for (i = 0; i < ADSP_NUM_FEATURE_ID; i++) {
