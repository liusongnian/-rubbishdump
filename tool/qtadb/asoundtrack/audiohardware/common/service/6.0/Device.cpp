...
std::tuple<Result, sp<IStreamOut>> Device::openOutputStreamImpl(int32_t ioHandle,
    ...
    status_t convertStatus = HidlUtils::audioConfigFromHal(halConfig, false, suggestedConfig);
...
std::tuple<Result, sp<IStreamIn>> Device::openInputStreamImpl(
    ...
    status_t convertStatus = HidlUtils::audioConfigFromHal(halConfig, true, suggestedConfig);
    ALOGW_IF(convertStatus != OK, "%s: suggested config with incompatible fields", __func__);
...
Result Device::doClose() {
    ALOGD("%s mIsClosed %d, mOpenedStreamsCount %d", __func__, mIsClosed, mOpenedStreamsCount);
...
Return<Result> Device::addDeviceEffect(AudioPortHandle device, uint64_t effectId) {
    ...
    ALOGI("%s(), halEffect = %p, effectId = %" PRIu64 "", __func__, halEffect, effectId);
...
int Device::audioParameterChangedCallback(const char *param, void *cookie) {
    ...
    if (self->mAudioParameterChangedCallback != NULL) {
...
Return<Result> Device::setAudioParameterChangedCallback(const sp<IAudioParameterChangedCallback>& callback) {
    ALOGV("Device::%s(), callback = %p, callback.get() = %p", __FUNCTION__, &callback, callback.get());
    if (mDevice->set_audio_parameter_changed_callback == NULL) return Result::NOT_SUPPORTED;
    if (callback.get() == NULL) {
        ALOGW("Device::%s(), callback.get() == NULL, return Result::INVALID_ARGUMENTS", __FUNCTION__);
        return Result::INVALID_ARGUMENTS;
    }
...
Return<Result> Device::clearAudioParameterChangedCallback() {
    ...
    Vector<sp<Device>>::iterator iter = gCallbackDeviceList.begin();
    if (iter == NULL) {
        ALOGE("%s(), iter == NULL, return", __FUNCTION__);
        return Result::INVALID_ARGUMENTS;
    }

    // Remove the device from sp list
    for (; iter != gCallbackDeviceList.end();) {
        if (*iter == this) {
