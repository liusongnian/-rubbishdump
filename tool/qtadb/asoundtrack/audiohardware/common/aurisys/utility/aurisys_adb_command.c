...
int parse_adb_cmd(const char *key_value_pair, aurisys_adb_command_t *adb_cmd) {
    char key_value_pair_copy[MAX_ADB_CMD_LEN] = {0};
    char *current = NULL;
    int ret = 0;
    ...
    if (adb_cmd == NULL || key_value_pair == NULL) {
        AUD_LOG_E("%s() fail! adb_cmd %p, key_value_pair %p",
                  __FUNCTION__, adb_cmd, key_value_pair);
        return -1;
    }
    ...
    ret = snprintf(key_value_pair_copy, MAX_ADB_CMD_LEN, "%s", key_value_pair);
    if (ret < 0) {
        AUD_LOG_E("%s() get key_value_pair fail!", __FUNCTION__);
        return -1;
    }
...
static void parse_adb_cmd_aurisys_scenario(char **current, aurisys_adb_command_t *adb_cmd) {
    char aurisys_scenario_str[MAX_ADB_CMD_LEN] = {0};
    ...
    *current = end + 1;
    ...
    if (!strncmp(scenario_prefix, scenario, strlen(scenario_prefix))) {
        ...
        ret = snprintf(aurisys_scenario_str, MAX_ADB_CMD_COPY_LEN, "%s", scenario);
    } else {
        AUD_LOG_V("scenario_suffix: %s", scenario_suffix);
        ret = snprintf(aurisys_scenario_str, MAX_ADB_CMD_COPY_LEN, "%s%s", scenario_prefix, scenario_suffix);
    }

    if (ret < 0) {
        AUD_LOG_E("%s(), get scenario fail, ret = %d", __FUNCTION__, ret);
        return;
    }
...
static void parse_adb_cmd_type_and_data(char **current, aurisys_adb_command_t *adb_cmd) {
    ...
    if (!strncmp(adb_cmd_str, ADB_CMD_STR_PARAM_FILE, strlen(ADB_CMD_STR_PARAM_FILE))) {
        ...
        if (adb_cmd->direction == AURISYS_SET_PARAM) {
            ...
            if (sscanf(data, "%x,%x", &adb_cmd->addr_value_pair.addr, &adb_cmd->addr_value_pair.value) == 2) {
            ...
            AUD_LOG_V("%s: *0x%x = 0x%x", ADB_CMD_STR_ADDR_VALUE,
                ...
                AUD_LOG_W("%s error!!, direction: %d", ADB_CMD_STR_ADDR_VALUE, adb_cmd->direction);
            }
        } else if (adb_cmd->direction == AURISYS_GET_PARAM) {
            data = adb_cmd_str + strlen(ADB_CMD_STR_ADDR_VALUE) + 1; /* +1: skip ',' */
            if (sscanf(data, "%x", &adb_cmd->addr_value_pair.addr) == 1) {
                adb_cmd->adb_cmd_type = ADB_CMD_ADDR_VALUE;
        ...
        if (adb_cmd->direction == AURISYS_SET_PARAM) {
            ...
            if (comma == NULL) {
                AUD_LOG_E("%s error key value incomplete!!, direction: %d",
                          ADB_CMD_STR_KEY_VALUE, adb_cmd->direction);
                return;
            }
            *comma = '=';
        }
