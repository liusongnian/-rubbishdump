...
void create_pool_buf(
    ...
    if (pool_buf->buf == NULL) {
        AUD_LOG_E("%s(), data_buf_type %d pool_buf->buf alloc fail!",
                  __FUNCTION__, audio_buf_pattern->data_buf_type);
        return;
    }
...
void audio_pool_buf_formatter_process(audio_pool_buf_formatter_t *formatter) {
    ...
    frame_size = AUDIO_BYTES_PER_SAMPLE(formatter->pool_source->buf->audio_format) *
                 formatter->pool_source->buf->num_channels;
    data_count = (audio_ringbuf_count(rb_in) / frame_size) * frame_size; // align ch, format
    AUD_LOG_VV("%s(+) data_count = %d, data_count_raw = %d", __FUNCTION__,
...
void audio_pool_buf_formatter_deinit(audio_pool_buf_formatter_t *formatter) {
    ...
    aud_fmt_conv_hal_destroy(formatter->aud_fmt_conv_hdl);
