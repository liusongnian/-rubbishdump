...
enum { /* audio_support_format_mask_t */
    ...
    AUDIO_SUPPORT_FORMAT_PCM_24_BIT_PACKED_MASK = AUDIO_FORMAT_TO_MASK(AUDIO_FORMAT_PCM_24_BIT_PACKED)
};
...
static const string_to_enum_pair_t g_s2e_aurisys_scenario[] = {
    ...
    STRING_TO_ENUM("AURISYS_SCENARIO_RECORD_IEM", AURISYS_SCENARIO_RECORD_IEM),
    ...
    STRING_TO_ENUM("AURISYS_SCENARIO_PLAYBACK_SMARTPA", AURISYS_SCENARIO_PLAYBACK_SMARTPA),
    STRING_TO_ENUM("AURISYS_SCENARIO_HDR_RECORD", AURISYS_SCENARIO_HDR_RECORD),
...
static const string_to_enum_pair_t g_s2e_aurisys_scenario_dsp[] = {
    ...
    STRING_TO_ENUM("AURISYS_SCENARIO_DSP_HDR_RECORD", AURISYS_SCENARIO_DSP_HDR_RECORD),
...
static const string_to_enum_pair_t g_s2e_audio_format[] = {
    ...
    STRING_TO_ENUM("AUDIO_FORMAT_PCM_24_BIT_PACKED", AUDIO_FORMAT_PCM_24_BIT_PACKED)
};
...
uint8_t map_aurisys_scenario_to_task_scene(
    ...
    if (core_type == AURISYS_CORE_HAL) {
        ...
        case AURISYS_SCENARIO_RECORD_IEM:
        case AURISYS_SCENARIO_HDR_RECORD:
        ...
        case AURISYS_SCENARIO_PHONE_CALL:
            ...
            break;
        case AURISYS_SCENARIO_PLAYBACK_SMARTPA:
        ...
        case AURISYS_SCENARIO_DSP_HDR_RECORD:
            task_scene = TASK_SCENE_RECORD;
            break;
        ...
        case AURISYS_SCENARIO_DSP_KTV:
            ...
            break;
        case AURISYS_SCENARIO_DSP_MUSIC:
            task_scene = TASK_SCENE_MUSIC;
            break;
...
static void match_support_format_mask(uint32_t *p_mask, const char *format, const uint32_t string_size) {
    ...
    } else if (strncmp(format, "AUDIO_FORMAT_PCM_24_BIT_PACKED", string_size) == 0) {
        *p_mask |= AUDIO_SUPPORT_FORMAT_PCM_24_BIT_PACKED_MASK;
    }
}
...
uint32_t parse_segment_in_string(const char *string, match_mask_by_string_fp_t match_func) {
    ...
    if (copy_string == NULL) {
        AUD_LOG_W("%s(), copy_string alloc fail!! return 0", __FUNCTION__);
        return 0;
    }

    ret = snprintf(copy_string, copy_size + 1, "%s", string);
    if (ret < 0) {
        AUD_LOG_W("%s(), copy_string fail!! return 0", __FUNCTION__);
        return 0;
    }
...
uint32_t get_format_from_mask(const uint32_t mask) {
    ...
    } else if (mask & AUDIO_SUPPORT_FORMAT_PCM_24_BIT_PACKED_MASK) {
...
uint32_t get_dedicated_format_from_mask(const uint32_t masks, const uint32_t the_audio_format) {
    ...
    if (the_audio_format != AUDIO_FORMAT_PCM_16_BIT &&
        ...
        the_audio_format != AUDIO_FORMAT_PCM_24_BIT_PACKED) {
...
void get_current_timestamp(char *timep_str, uint32_t timep_str_size)
{
    time_t rawtime;
    struct tm *time_info;

    if (time(&rawtime) == ((time_t)-1)) {
        AUD_LOG_E("%s(), get timep failed\n", __FUNCTION__);
    } else {
