...
#define AURISYS_DUMMY_LIB_NAME    "aurisys_demo"
...
typedef struct aurisys_xml_handler_t {
    void      *dlopen_handle;
    xmlDocPtr  (*xmlParseFile)(const char *filename);
    xmlNode   *(*xmlDocGetRootElement)(xmlDocPtr doc);
    xmlChar   *(*xmlGetProp)(const xmlNode *node, const xmlChar *name);
    void       (*xmlFree)(void *mem);
    void       (*xmlFreeDoc)(xmlDocPtr cur);
    void       (*xmlCleanupParser)(void);
    int        (*xmlStrcmp)(const xmlChar *str1, const xmlChar *str2);
    void       (*xmlMemGet)(xmlFreeFunc *freeFunc, xmlMallocFunc *mallocFunc,
                            xmlReallocFunc *reallocFunc, xmlStrdupFunc *strdupFunc);
} aurisys_xml_handler_t;
...
static aurisys_xml_handler_t *gXmlHandler = NULL;
...
#define GET_PROP(node, prop) ((char *)gXmlHandler->xmlGetProp(node, XML_CHAR_CAST(prop)))
#define GET_INT_BY_PROP(int_type, node, prop) \
    ({ \
        ...
        gXmlHandler->xmlFree(__prop_string); \
...
aurisys_config_t *parse_aurisys_config(const uint8_t core_type) {
    ...
    if (gXmlHandler == NULL) {
        AUD_LOG_E("%s(), xml handler create fail!", __FUNCTION__);
        return NULL;
    }
    ...
    ret = snprintf(gProductInfoBuf, sizeof(gProductInfoBuf),
                   "platform=%s,device=%s,model=%s", platform, device, model);
    if (ret < 0) {
        memset(gProductInfoBuf, 0, sizeof(gProductInfoBuf));
        AUD_LOG_W("%s(), set product info fail, ret = %d", __FUNCTION__, ret);
    }
    ...
    doc = gXmlHandler->xmlParseFile(xml_file);
    if (doc == NULL) {
        AUD_LOG_E("xmlParseFile %s fail", xml_file);
        return NULL;
    }

    root = gXmlHandler->xmlDocGetRootElement(doc);
    if (root == NULL) {
    ...
    if (uplink_library_name_list) {
        HASH_ITER(hh, uplink_library_name_list, itor_library_name, tmp_library_name) {
            ...
            if (new_library_name != NULL) {
                ...
                AUD_LOG_E("%s(), add ul lib name %s fail!", __FUNCTION__, itor_library_name->name);
            }
    ...
    if (downlink_library_name_list) {
        HASH_ITER(hh, downlink_library_name_list, itor_library_name, tmp_library_name) {
            ...
            if (new_library_name != NULL) {
                ...
                AUD_LOG_E("%s(), add dl lib name %s fail!", __FUNCTION__, itor_library_name->name);
            }
            new_library_name = NULL;
        }
    }
...
static aurisys_scene_lib_table_t *parse_scenarios(
    ...
    while (node_aurisys_scenario != NULL) {
        ...
        gXmlHandler->xmlFree(prop_string);
        ...
        HASH_ITER(hh, new_scene_lib_table->uplink_library_name_list, itor_library_name, tmp_library_name) {
            if (!strncmp(AURISYS_DUMMY_LIB_NAME, itor_library_name->name, strlen(AURISYS_DUMMY_LIB_NAME))) {
                new_scene_lib_table->bypassed_mask |= (1 << ARSI_PROCESS_TYPE_UL_ONLY);
                break;
            }
        }

        HASH_ITER(hh, new_scene_lib_table->downlink_library_name_list, itor_library_name, tmp_library_name) {
            if (!strncmp(AURISYS_DUMMY_LIB_NAME, itor_library_name->name, strlen(AURISYS_DUMMY_LIB_NAME))) {
                new_scene_lib_table->bypassed_mask |= (1 << ARSI_PROCESS_TYPE_DL_ONLY);
                break;
            }
        }
...
static void dump_scene_lib_table(aurisys_scene_lib_table_t *scene_lib_table) {
    ...
    HASH_ITER(hh, scene_lib_table, itor_scene_lib_table, tmp_scene_lib_table) {
        ...
        AUD_LOG_D("bypassed_mask = %u", itor_scene_lib_table->bypassed_mask);
    }
}
#endif
...
static aurisys_library_config_t *parse_library_config(
    ...
    while (node_library != NULL) {
        ...
        if (new_library_config != NULL) {
            ...
            AUD_LOG_E("%s(), new_library_config is NULL!", __FUNCTION__);
        }
...
static void dump_library_config(aurisys_library_config_t *library_config) {
    ...
    HASH_ITER(hh, library_config, itor_lib, tmp_lib) {
        AUD_LOG_V("%s(), library name %s", __FUNCTION__, itor_lib->name);
...
static int parse_xlink_bufs(
    ...
    if (!node_buf || !buf_in) {
        AUD_LOG_E("%s(), parse buf in error, node_buf = %p, buf_in = %p",
                  __FUNCTION__, node_buf, buf_in);
        goto AURISYS_XLINK_BUF_PARSE_FAIL;
    }
    ...
    *in = buf_in;
    ...
    if (!node_buf || !buf_out) {
        AUD_LOG_E("%s(), parse buf out error, node_buf = %p, buf_in = %p",
                  __FUNCTION__, node_buf, buf_in);
        goto AURISYS_XLINK_BUF_PARSE_FAIL;
    }
    prop_string = get_prop_string_by_prop(node_buf, "data_buf_type");
    ...
    *out = buf_out;
    ...
    if (node_buf) {
        ...
        if (buf_refs == NULL) {
             AUD_LOG_E("%s(), parse buf ref error", __FUNCTION__);
             goto AURISYS_XLINK_BUF_PARSE_FAIL;
        }
    ...
    return retval;
AURISYS_XLINK_BUF_PARSE_FAIL:
    AUDIO_FREE_POINTER(buf_in);
    AUDIO_FREE_POINTER(buf_out);
    AUDIO_FREE_POINTER(buf_refs);
    return -1;
}
 ...
 */
static void add_library_config(aurisys_library_config_t **hash_head, aurisys_library_config_t *new_library_config) {
    if (!new_library_config->name) {
        AUD_LOG_E("%s(), lib name is NULL!", __FUNCTION__);
        AUD_ASSERT(new_library_config->name != NULL);
        return;
    }
...
static xmlNodePtr get_neighbor_node_by_name(xmlNodePtr node, const char *name) {
    ...
    for (iterator = node; iterator != NULL; iterator = iterator->next) {
        ...
        if (gXmlHandler->xmlStrcmp(iterator->name, XML_CHAR_CAST(name)) == 0) {
            break;
        }
    }
...
void init_aurisys_lib_api(
    ...
    string_buf_t arsi_version;
    string_buf_t param_file_path;
    struct timespec ts_before_dlopen;
    struct timespec ts_before_dlsym;
    struct timespec ts_after_dlsym;

    if (library_config == NULL) {
        AUD_LOG_E("%s, lib_config is NULL", __FUNCTION__);
        return;
    }

    /* dlopen */
#if defined(__LP64__)
    dlopen_lib_path = library_config->lib64_path;
#else
    dlopen_lib_path = library_config->lib_path;
#endif
    audio_get_timespec_monotonic(&ts_before_dlopen);
    library_config->dlopen_handle = dlopen(dlopen_lib_path, RTLD_NOW);
    if (library_config->dlopen_handle == NULL) {
        ...
        //return NULL;
#if defined(__LP64__)
        library_config->dlopen_handle = dlopen(AURISYS_DUMMY_LIB64_PATH, RTLD_NOW);
#else
        library_config->dlopen_handle = dlopen(AURISYS_DUMMY_LIB_PATH, RTLD_NOW);
#endif
        if (library_config->dlopen_handle == NULL) {
            AUD_ASSERT(library_config->dlopen_handle != NULL);
            return;
        }
    }

    audio_get_timespec_monotonic(&ts_before_dlsym);
    dynamic_link_arsi_assign_lib_fp = (dynamic_link_arsi_assign_lib_fp_t)dlsym(
                                          library_config->dlopen_handle,
    ...
    audio_get_timespec_monotonic(&ts_after_dlsym);

    AUDIO_ALLOC_STRUCT(AurisysLibInterface, library_config->api);
    dynamic_link_arsi_assign_lib_fp(library_config->api);
    if ((manager_config->core_type == AURISYS_CORE_HAL) && (library_config->api != NULL)) {
        AUD_ASSERT(library_config->api->arsi_create_handler != NULL);
    }

    AUD_LOG_D("%s(), dlopen time(ms): %lf, dlsym time(ms): %lf", __FUNCTION__,
              ((double)get_time_diff_ns(&ts_before_dlopen, &ts_before_dlsym) / 1000000),
              ((double)get_time_diff_ns(&ts_before_dlsym, &ts_after_dlsym) / 1000000));

    if (library_config->api &&
        library_config->api->arsi_get_lib_version != NULL) {
        ...
        if (library_config->api->arsi_get_lib_version(&lib_version) == 0) {
            AUD_LOG_D("%s(), lib_name: %s lib_version: \"%s\"",
                      __FUNCTION__, library_config->name, lib_version.p_string);
        } else {
            AUD_LOG_W("%s(), lib_name: %s, unknown lib_version",
                      __FUNCTION__, library_config->name);
        }
        AUDIO_FREE_POINTER(lib_version.p_string);
    } else {
        AUD_LOG_W("%s(), lib_name: %s, unknown lib_version", __FUNCTION__, library_config->name);
    }

    if (library_config->api &&
        library_config->api->arsi_get_arsi_version != NULL) {
        arsi_version.memory_size = 128;
        arsi_version.string_size = 0;
        arsi_version.p_string = NULL;
        AUDIO_ALLOC_CHAR_BUFFER(arsi_version.p_string, arsi_version.memory_size);
        if (library_config->api->arsi_get_arsi_version(&arsi_version) == 0) {
            if(strncmp(arsi_version.p_string, MTK_ARSI_API_VERSION, sizeof(MTK_ARSI_API_VERSION))) {
                AUD_LOG_W("%s(), lib_name: %s, arsi_version %s not match current version!",
                          __FUNCTION__, library_config->name, arsi_version.p_string);
            }
        } else {
            AUD_LOG_W("%s(), lib_name: %s, unknown arsi_version",
                      __FUNCTION__, library_config->name);
        }
        AUDIO_FREE_POINTER(arsi_version.p_string);
    } else {
        AUD_LOG_W("%s(), lib_name: %s, arsi_get_arsi_version not implement!", __FUNCTION__, library_config->name);
    }

    if (library_config->api &&
        library_config->api->arsi_load_param != NULL) {
        char_to_string(&param_file_path, library_config->param_path,
                       strlen(library_config->param_path) + 1);
...
void init_aurisys_xml_handler() {
    AUDIO_ALLOC_STRUCT(aurisys_xml_handler_t, gXmlHandler);
    gXmlHandler->dlopen_handle = dlopen("libxml2.so", RTLD_NOW);
    if (gXmlHandler->dlopen_handle == NULL) {
        AUD_LOG_E("%s(), dlopen fail", __FUNCTION__);
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    gXmlHandler->xmlParseFile = dlsym(gXmlHandler->dlopen_handle, "xmlParseFile");
    if (gXmlHandler->xmlParseFile == NULL) {
        AUD_LOG_E("%s(), xmlParseFile dlsym fail(%s)", __FUNCTION__, dlerror());
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    gXmlHandler->xmlDocGetRootElement = dlsym(gXmlHandler->dlopen_handle, "xmlDocGetRootElement");
    if (gXmlHandler->xmlDocGetRootElement == NULL) {
        AUD_LOG_E("%s(), xmlDocGetRootElement dlsym fail(%s)", __FUNCTION__, dlerror());
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    gXmlHandler->xmlGetProp = dlsym(gXmlHandler->dlopen_handle, "xmlGetProp");
    if (gXmlHandler->xmlGetProp == NULL) {
        AUD_LOG_E("%s(), xmlGetProp dlsym fail(%s)", __FUNCTION__, dlerror());
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    gXmlHandler->xmlMemGet = dlsym(gXmlHandler->dlopen_handle, "xmlMemGet");
    if (gXmlHandler->xmlMemGet == NULL) {
        AUD_LOG_E("%s(), xmlMemGet dlsym fail(%s)", __FUNCTION__, dlerror());
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    gXmlHandler->xmlMemGet(&gXmlHandler->xmlFree, NULL, NULL, NULL);
    if (gXmlHandler->xmlFree == NULL) {
        AUD_LOG_E("%s(), get xmlFree dlsym fail", __FUNCTION__);
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    gXmlHandler->xmlFreeDoc = dlsym(gXmlHandler->dlopen_handle, "xmlFreeDoc");
    if (gXmlHandler->xmlFreeDoc == NULL) {
        AUD_LOG_E("%s(), xmlFreeDoc dlsym fail(%s)", __FUNCTION__, dlerror());
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    gXmlHandler->xmlStrcmp = dlsym(gXmlHandler->dlopen_handle, "xmlStrcmp");
    if (gXmlHandler->xmlStrcmp == NULL) {
        AUD_LOG_E("%s(), xmlStrcmp dlsym fail(%s)", __FUNCTION__, dlerror());
        goto INIT_AURISYS_XML_HANDLER_FAIL;
    }
    return;
INIT_AURISYS_XML_HANDLER_FAIL:
    if (gXmlHandler->dlopen_handle != NULL) {
        dlclose(gXmlHandler->dlopen_handle);
    }
    AUDIO_FREE_POINTER(gXmlHandler);
    return;
}

void deinit_aurisys_xml_handler() {
    if (gXmlHandler == NULL || gXmlHandler->dlopen_handle == NULL) {
        return;
    }
    dlclose(gXmlHandler->dlopen_handle);
    AUDIO_FREE_POINTER(gXmlHandler);
