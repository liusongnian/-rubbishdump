...
#define AURISYS_LIB_ALL_KEY "LIB_ALL"
...
static bool g_dsp_enable;
/*
 * =============================================================================
 *                     private function declaration
 * =============================================================================
 */
static int aurisys_set_parameter_cont(
...
static void arsi_lib_printf(const char *message, ...) {
    ...
    if (ret > 0) {
        ...
        AUD_LOG_W("%s(), lib print log fail! err = %d", __FUNCTION__, ret);
    }
...
int init_aurisys_controller(bool adspEn) {
...
int wrap_config_to_dsp(void) {
    ...
    for (adsp_id = AUDIO_OPENDSP_USE_HIFI3_A; adsp_id <= AUDIO_OPENDSP_USE_HIFI3_B; adsp_id++) {
        audio_send_ipi_msg(&ipi_msg,
                           get_audio_controller_task_by_id(adsp_id),
...
aurisys_lib_manager_t *create_aurisys_lib_manager(
...
#endif
        {
            AUD_LOG_E("%s(), not support adsp!!", __FUNCTION__);
            return NULL;
        }
    } else {
    ...
    if (new_manager != NULL) {
        ...
        AUD_LOG_E("%s(), fail! return NULL lib manager!", __FUNCTION__);
    }
    UNLOCK_ALOCK(g_controller->lock);
...
int aurisys_set_parameter(const char *key_value_pair) {
    ...
    if (!strcmp(adb_cmd.adb_cmd_key, AURISYS_LIB_ALL_KEY)) {
        if (adb_cmd.aurisys_scenario != scenario_all) {
            HASH_FIND_INT(
                aurisys_config->scene_lib_table_hh,
                &adb_cmd.aurisys_scenario,
                the_scene_lib_table);

            if (the_scene_lib_table == NULL ||
                the_scene_lib_table->all_library_name_list == NULL) {
                AUD_LOG_E("%s get scene table fail!!", adb_cmd.adb_cmd_key);
                ret = -1;
                goto AURISYS_SET_PARAM_EXIT;
            }

            HASH_ITER(hh, the_scene_lib_table->all_library_name_list, itor_library_name, tmp_library_name) {
                HASH_FIND_STR(aurisys_config->library_config_hh,
                    itor_library_name->name,
                    library_config);
                if (library_config == NULL) {
                    AUD_LOG_W("%s get lib config fail!!", adb_cmd.adb_cmd_key);
                    ret = -1;
                    goto AURISYS_SET_PARAM_EXIT;
                }
                if (aurisys_set_parameter_cont(library_config, &adb_cmd)) {
                    ret = -1;
                }
            }
        } else {
            HASH_ITER(hh, aurisys_config->library_config_hh, library_config, tmp_library_config) {
                if (aurisys_set_parameter_cont(library_config, &adb_cmd)) {
                    ret = -1;
                }
            }
        ...
        ret = aurisys_set_parameter_cont(library_config, &adb_cmd);
    }

AURISYS_SET_PARAM_EXIT:
#ifdef MTK_AUDIODSP_SUPPORT
    if (g_dsp_enable && ret == 0 && adb_cmd.target == ADB_CMD_TARGET_DSP) {
...
static int aurisys_set_parameter_cont(
    ...
    if ((library_config == NULL) || (adb_cmd == NULL)) {
        AUD_LOG_E("%s fail!, library_config: %p, adb_cmd: %p", __FUNCTION__, library_config, adb_cmd);
        return -1;
    }
    ...
    if (adb_cmd->target == ADB_CMD_TARGET_HAL) {
        ...
        AUD_LOG_W("target not support, target: %d", adb_cmd->target);
        return -1;
    }
...
static int aurisys_set_parameter_to_component(
    ...
    case ADB_CMD_ENABLE_RAW_DUMP:
        if (component->enable_raw_dump == adb_cmd->enable_raw_dump) {
            set_adb_done = true;
            ret = 0;
            break;
        }
        ...
        break;
    case ADB_CMD_ENABLE_LIB_DUMP:
        if (component->enable_lib_dump == adb_cmd->enable_lib_dump) {
            set_adb_done = true;
            ret = 0;
            break;
        }
...
#ifdef MTK_AUDIODSP_SUPPORT
        if (g_dsp_enable && adb_cmd->target == ADB_CMD_TARGET_DSP) {
            adb_cmd->dsp_mask |= (1 << audio_get_dsp_id(itor_lib_handler->dsp_task_scene));
        }
#endif
    }
...
char *aurisys_get_parameter(const char *key) {
    ...
    aurisys_library_config_t *library_config = NULL;
    aurisys_component_t *component = NULL;
    aurisys_component_t *first_component = NULL;
        ...
        if (adb_cmd.aurisys_scenario != scenario_all) {
            ...
            HASH_ITER(hh, the_scene_lib_table->all_library_name_list, itor_library_name, tmp_library_name) {
                HASH_FIND_STR(aurisys_config->library_config_hh,
                    ...
                    library_config);
                if (library_config != NULL) {
                    AUD_LOG_W("%s get lib all, only get lib %s!!", adb_cmd.adb_cmd_key, itor_library_name->name);
                    break;
                }
            }
        } else {
            HASH_ITER(hh, aurisys_config->library_config_hh, library_config, tmp_library_config) {
                if (library_config != NULL) {
                    AUD_LOG_W("%s get lib all, only get lib %s!!", adb_cmd.adb_cmd_key, library_config->name);
                    break;
                }
            }
        }
        ...
        for (scenario_idx = 0; scenario_idx < scenario_size; scenario_idx++) {
            ...
            if (component != NULL &&
                ...
                AUD_LOG_W("%s get scenario all!! only get aurisys_scenario %u...",
                          library_config->name, adb_cmd.aurisys_scenario);
                break;
            } else if ((component != NULL) && (first_component == NULL)) {
    ...
    case ADB_CMD_KEY_VALUE:
        get_adb_done = false;
        if (HASH_CNT(hh_component, component->lib_handler_list_for_adb_cmd) == 0) {
            AUD_LOG_W("%s(), adb_cmd->adb_cmd_type %d fail, need active handler!", __FUNCTION__, adb_cmd.adb_cmd_type);
            ret = -1;
        }
        break;
    default:
    ...
    if (component == NULL) {
        AUD_LOG_E("%s cannot find component, return fail!!", adb_cmd.adb_cmd_key);
        ret = -1;
        goto AURISYS_GET_PARAM_EXIT;
    }
...
static int aurisys_run_parameter_to_dsp(struct aurisys_adb_command_t *adb_cmd) {
    ...
    for (adsp_id = AUDIO_OPENDSP_USE_HIFI3_A; adsp_id <= AUDIO_OPENDSP_USE_HIFI3_B; adsp_id++) {
        if (adb_cmd->dsp_mask & (1 << adsp_id)) {
            ret_tmp = audio_send_ipi_msg(&ipi_msg,
                                         get_audio_controller_task_by_id(adsp_id),
                                         AUDIO_IPI_LAYER_TO_DSP,
                                         AUDIO_IPI_DMA,
                                         AUDIO_IPI_MSG_NEED_ACK,
...
bool is_bypass_aurisys_lib(
    const uint8_t core_type,
    const uint32_t scenario,
    const uint8_t process_type) {
    aurisys_scene_lib_table_t *the_scene_lib_table = NULL;
    aurisys_config_t *aurisys_config = NULL;
    bool ret = false;
...
#endif
        {
            AUD_LOG_E("%s(), not suppot DSP!!!", __FUNCTION__);
            return false;
        }
    } else {
        AUD_LOG_E("%s(), undefined core type : %d!!!", __FUNCTION__, core_type);
        return false;
    }

    if (aurisys_config == NULL) {
        AUD_LOG_E("%s(), aurisys_config is NULL!!!", __FUNCTION__);
        return false;
    }
    ...
    if (the_scene_lib_table == NULL) {
        AUD_LOG_E("%s(), scene: %d table is NULL!!!", __FUNCTION__, scenario);
        UNLOCK_ALOCK(g_controller->lock);
        return false;
    }

    ret = ((the_scene_lib_table->bypassed_mask & (1 << process_type)) != 0) ? true : false;
    AUD_LOG_V("%s(), ret = %d", __FUNCTION__, ret);
    UNLOCK_ALOCK(g_controller->lock);
    return ret;
}
