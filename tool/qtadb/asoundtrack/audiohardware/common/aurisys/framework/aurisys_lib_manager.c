...
aurisys_lib_manager_t *new_aurisys_lib_manager(
    ...
    aurisys_library_name_t *lib_name_list = NULL;
    ...
    if (manager_config->arsi_process_type == ARSI_PROCESS_TYPE_UL_AND_DL) {
        lib_name_list = the_scene_lib_table->all_library_name_list;
    } else if (manager_config->arsi_process_type == ARSI_PROCESS_TYPE_UL_ONLY) {
        lib_name_list = the_scene_lib_table->uplink_library_name_list;
    } else { // dl only
        lib_name_list = the_scene_lib_table->downlink_library_name_list;
    }
...
audio_pool_buf_t *create_audio_pool_buf(
    ...
    if (pool_buf == NULL) {
        AUD_LOG_E("%s(), pool_buf create fail!", __FUNCTION__);
        return NULL;
    }
    AUDIO_ALLOC_STRUCT(audio_buf_t, pool_buf->buf);
    if (pool_buf->buf == NULL) {
        AUD_LOG_E("%s(), pool_buf->buf create fail!", __FUNCTION__);
...
int aurisys_pool_buf_formatter_init(aurisys_lib_manager_t *manager) {
    ...
    if (manager->num_uplink_library_hanlder != 0) {
        if ((ul_in == NULL) || (ul_out == NULL)) {
            AUD_ASSERT(ul_in != 0);
            AUD_ASSERT(ul_out != 0);
            goto AURISYS_FORMATTER_INIT_FAIL;
        } else if ((ul_in->buf == NULL) || (ul_out->buf == NULL)) {
    ...
    if (manager->num_downlink_library_hanlder != 0) {
        if ((dl_in == NULL) || (dl_out == NULL)) {
            AUD_ASSERT(dl_in != 0);
            AUD_ASSERT(dl_out != 0);
            goto AURISYS_FORMATTER_INIT_FAIL;
        } else if ((dl_in->buf == NULL) || (dl_out->buf == NULL)) {
