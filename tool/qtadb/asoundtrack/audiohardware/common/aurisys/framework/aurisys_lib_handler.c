...
static void aurisys_open_raw_dump(aurisys_lib_handler_t *lib_handler);
static void aurisys_open_lib_dump(aurisys_lib_handler_t *lib_handler);
static void aurisys_close_raw_dump(aurisys_lib_handler_t *lib_handler);
static void aurisys_close_lib_dump(aurisys_lib_handler_t *lib_handler);
...
int aurisys_arsi_parsing_param_file(aurisys_lib_handler_t *lib_handler) {
    ...
    ret = snprintf(custom_info_string, MAX_CUSTOM_INFO_LEN, "%s", lib_handler->custom_info);
    if (ret < 0) {
        AUD_LOG_E("%s(), get custom_info fail!", __FUNCTION__);
    }
...
int aurisys_arsi_run_adb_cmd(aurisys_lib_handler_t *lib_handler, aurisys_adb_command_t *adb_cmd) {
    ...
    case ADB_CMD_APPLY_PARAM:
        if (adb_cmd->direction == AURISYS_SET_PARAM) {
            ...
            /* apply */
            if (arsi_handler && arsi_api->arsi_update_param) {
                ...
                if (lib_name_param == NULL) {
                    AUD_LOG_E("%s() lib: %s, APPLY_PARAM get fail", __FUNCTION__, lib_handler->lib_name);
                    retval = NO_MEMORY;
                    break;
                }
    ...
    case ADB_CMD_ENABLE_RAW_DUMP:
        if (adb_cmd->direction == AURISYS_SET_PARAM) {
            if (adb_cmd->enable_raw_dump == true) {
                aurisys_open_raw_dump(lib_handler);
                mDumpFileNum++;
    ...
    case ADB_CMD_ENABLE_LIB_DUMP:
        if (adb_cmd->direction == AURISYS_SET_PARAM) {
            if (adb_cmd->enable_lib_dump == true) {
                aurisys_open_lib_dump(lib_handler);
                mDumpFileNum++;
                mDumpFileNum %= MAX_DUMP_NUM;
                if (adb_cmd->target == ADB_CMD_TARGET_DSP) {
                    run_adb_done = true;
                } else if (arsi_handler &&
    ...
    if (manager_config->arsi_process_type == ARSI_PROCESS_TYPE_UL_ONLY ||
        ...
        if (src->p_ul_buf_in != NULL) {
            ...
            if (des->p_ul_buf_in == NULL) {
                goto AURISYS_CLONE_LIB_CONFIG_FAIL;
            }
...
static void aurisys_open_raw_dump(aurisys_lib_handler_t *lib_handler) {
    ...
    if (*lib_handler->enable_raw_dump) {
        get_current_timestamp(timep_str, sizeof(timep_str));
        lib_handler->raw_dump_enabled = true;
        if ((lib_handler->pcm_dump_ul_in == NULL && lib_handler->pcm_dump_ul_out == NULL) &&
            (lib_handler->arsi_process_type == ARSI_PROCESS_TYPE_UL_ONLY ||
             lib_handler->arsi_process_type == ARSI_PROCESS_TYPE_UL_AND_DL)) {
            p_audio_buf = lib_handler->lib_config.p_ul_buf_in;
            AUDIO_ALLOC_STRUCT(PcmDump_t, lib_handler->pcm_dump_ul_in);
            ret = snprintf(mDumpFileName, sizeof(mDumpFileName), "%s/%s.%d.%d.%d.ul_in.%d.%s.%dch.%s.pcm",
                           audio_dump, lib_handler->lib_name, mDumpFileNum, getpid(), gettid(),
                           p_audio_buf->sample_rate_buffer,
                           audio_format_to_dump_string(p_audio_buf->audio_format),
                           p_audio_buf->num_channels, timep_str);
            if (ret < 0) {
                AUD_LOG_W("%s(), set ul_in dump name fail, ret = %d", __FUNCTION__, ret);
            }
            if (lib_handler->pcm_dump_ul_in) {
                InitPcmDump_t(lib_handler->pcm_dump_ul_in, 32768);
                lib_handler->pcm_dump_ul_in->AudioOpendumpPCMFile(
                    lib_handler->pcm_dump_ul_in, mDumpFileName);
            }
            p_audio_buf = lib_handler->lib_config.p_ul_buf_out;
            AUDIO_ALLOC_STRUCT(PcmDump_t, lib_handler->pcm_dump_ul_out);
            ret = snprintf(mDumpFileName, sizeof(mDumpFileName), "%s/%s.%d.%d.%d.ul_out.%d.%s.%dch.%s.pcm",
                           audio_dump, lib_handler->lib_name, mDumpFileNum, getpid(), gettid(),
                           p_audio_buf->sample_rate_buffer,
                           audio_format_to_dump_string(p_audio_buf->audio_format),
                           p_audio_buf->num_channels, timep_str);
            if (ret < 0) {
                AUD_LOG_W("%s(), set ul_out dump name fail, ret = %d", __FUNCTION__, ret);
            }
            if (lib_handler->pcm_dump_ul_out) {
                InitPcmDump_t(lib_handler->pcm_dump_ul_out, 32768);
                lib_handler->pcm_dump_ul_out->AudioOpendumpPCMFile(
                    lib_handler->pcm_dump_ul_out, mDumpFileName);
            }
            if ((lib_handler->p_ul_ref_buf_aec != NULL) && (lib_handler->pcm_dump_aec == NULL)) {
                p_audio_buf = lib_handler->lib_config.p_ul_ref_bufs;
                AUDIO_ALLOC_STRUCT(PcmDump_t, lib_handler->pcm_dump_aec);
                ret = snprintf(mDumpFileName, sizeof(mDumpFileName), "%s/%s.%d.%d.%d.aec.%d.%s.%dch.%s.pcm",
                               audio_dump, lib_handler->lib_name, mDumpFileNum, getpid(), gettid(),
                               p_audio_buf->sample_rate_buffer,
                               audio_format_to_dump_string(p_audio_buf->audio_format),
                               p_audio_buf->num_channels, timep_str);
                if (ret < 0) {
                    AUD_LOG_W("%s(), set ul_ref dump name fail, ret = %d", __FUNCTION__, ret);
                }
                if (lib_handler->pcm_dump_aec) {
                    InitPcmDump_t(lib_handler->pcm_dump_aec, 32768);
                    lib_handler->pcm_dump_aec->AudioOpendumpPCMFile(
                        lib_handler->pcm_dump_aec, mDumpFileName);
                }
            }
        }

        if ((lib_handler->pcm_dump_ul_in == NULL && lib_handler->pcm_dump_ul_out == NULL) &&
            (lib_handler->arsi_process_type == ARSI_PROCESS_TYPE_DL_ONLY ||
             lib_handler->arsi_process_type == ARSI_PROCESS_TYPE_UL_AND_DL)) {
            p_audio_buf = lib_handler->lib_config.p_dl_buf_in;
            AUDIO_ALLOC_STRUCT(PcmDump_t, lib_handler->pcm_dump_dl_in);
            ret = snprintf(mDumpFileName, sizeof(mDumpFileName), "%s/%s.%d.%d.%d.dl_in.%d.%s.%dch.%s.pcm",
                           audio_dump, lib_handler->lib_name, mDumpFileNum, getpid(), gettid(),
                           p_audio_buf->sample_rate_buffer,
                           audio_format_to_dump_string(p_audio_buf->audio_format),
                           p_audio_buf->num_channels, timep_str);
            if (ret < 0) {
                AUD_LOG_W("%s(), set dl_in dump name fail, ret = %d", __FUNCTION__, ret);
            }
            if (lib_handler->pcm_dump_dl_in) {
                InitPcmDump_t(lib_handler->pcm_dump_dl_in, 32768);
                lib_handler->pcm_dump_dl_in->AudioOpendumpPCMFile(
                    lib_handler->pcm_dump_dl_in, mDumpFileName);
            }
            p_audio_buf = lib_handler->lib_config.p_dl_buf_out;
            AUDIO_ALLOC_STRUCT(PcmDump_t, lib_handler->pcm_dump_dl_out);
            ret = snprintf(mDumpFileName, sizeof(mDumpFileName), "%s/%s.%d.%d.%d.dl_out.%d.%s.%dch.%s.pcm",
                           audio_dump, lib_handler->lib_name, mDumpFileNum, getpid(), gettid(),
                           p_audio_buf->sample_rate_buffer,
                           audio_format_to_dump_string(p_audio_buf->audio_format),
                           p_audio_buf->num_channels, timep_str);
            if (ret < 0) {
                AUD_LOG_W("%s(), set dl_out dump name fail, ret = %d", __FUNCTION__, ret);
            }
            if (lib_handler->pcm_dump_dl_out) {
                InitPcmDump_t(lib_handler->pcm_dump_dl_out, 32768);
                lib_handler->pcm_dump_dl_out->AudioOpendumpPCMFile(
                    lib_handler->pcm_dump_dl_out, mDumpFileName);
            }
        }
    }
}
...
static void aurisys_open_lib_dump(aurisys_lib_handler_t *lib_handler) {
    ...
    if ((*lib_handler->enable_lib_dump) && (lib_handler->lib_dump == NULL)) {
        lib_handler->lib_dump_enabled = true;
        get_current_timestamp(timep_str, sizeof(timep_str));
        AUDIO_ALLOC_STRUCT(PcmDump_t, lib_handler->lib_dump);
        if (!strcmp(lib_handler->lib_dump_path, "AUTO")) {
            ret = snprintf(mDumpFileName, sizeof(mDumpFileName), "%s/%s.%d.%d.%d.lib_dump.%s.bin",
                           audio_dump, lib_handler->lib_name, mDumpFileNum, getpid(), gettid(), timep_str);
        } else {
            ret = snprintf(mDumpFileName, sizeof(mDumpFileName), "%s", lib_handler->lib_dump_path);
        }

        if (ret < 0) {
            AUD_LOG_W("%s(), set lib dump name fail, ret = %d", __FUNCTION__, ret);
        }
        if (lib_handler->lib_dump) {
            InitPcmDump_t(lib_handler->lib_dump, 32768);
            lib_handler->lib_dump->AudioOpendumpPCMFile(
                lib_handler->lib_dump, mDumpFileName);
#ifdef MTK_AUDIODSP_SUPPORT
            if (lib_handler->core_type == AURISYS_CORE_HIFI3) {
                add_aurisys_dsp_dump(lib_handler->lib_dump, lib_handler->dsp_task_scene, lib_handler->lib_order);
                lib_handler->lib_dump->isDspDump = true;
            }
#endif
        }
    }
...
static void aurisys_close_raw_dump(aurisys_lib_handler_t *lib_handler) {
    if (lib_handler->raw_dump_enabled) {
        /* UL in */
        if (lib_handler->pcm_dump_ul_in != NULL &&
            lib_handler->pcm_dump_ul_in->mFilep != NULL) {
            lib_handler->pcm_dump_ul_in->AudioCloseDumpPCMFile(lib_handler->pcm_dump_ul_in);
        }
        AUDIO_FREE_POINTER(lib_handler->pcm_dump_ul_in);
        /* UL out */
        if (lib_handler->pcm_dump_ul_out != NULL &&
            lib_handler->pcm_dump_ul_out->mFilep != NULL) {
            lib_handler->pcm_dump_ul_out->AudioCloseDumpPCMFile(lib_handler->pcm_dump_ul_out);
        }
        AUDIO_FREE_POINTER(lib_handler->pcm_dump_ul_out);
        /* AEC */
        if (lib_handler->pcm_dump_aec != NULL &&
            lib_handler->pcm_dump_aec->mFilep != NULL) {
            lib_handler->pcm_dump_aec->AudioCloseDumpPCMFile(lib_handler->pcm_dump_aec);
        }
        AUDIO_FREE_POINTER(lib_handler->pcm_dump_aec);
        /* DL in */
        if (lib_handler->pcm_dump_dl_in != NULL &&
            lib_handler->pcm_dump_dl_in->mFilep != NULL) {
            lib_handler->pcm_dump_dl_in->AudioCloseDumpPCMFile(lib_handler->pcm_dump_dl_in);
        }
        AUDIO_FREE_POINTER(lib_handler->pcm_dump_dl_in);
        /* DL out */
        if (lib_handler->pcm_dump_dl_out != NULL &&
            lib_handler->pcm_dump_dl_out->mFilep != NULL) {
            lib_handler->pcm_dump_dl_out->AudioCloseDumpPCMFile(lib_handler->pcm_dump_dl_out);
        }
        AUDIO_FREE_POINTER(lib_handler->pcm_dump_dl_out);
    }
}

static void aurisys_close_lib_dump(aurisys_lib_handler_t *lib_handler) {
    if (lib_handler->lib_dump_enabled) {
        if (lib_handler->lib_dump != NULL &&
            lib_handler->lib_dump->mFilep != NULL) {
#ifdef MTK_AUDIODSP_SUPPORT
            if (lib_handler->lib_dump->isDspDump) {
                delete_aurisys_dsp_dump(lib_handler->lib_dump, lib_handler->dsp_task_scene, lib_handler->lib_order);
            }
#endif
            lib_handler->lib_dump->AudioCloseDumpPCMFile(lib_handler->lib_dump);
        }
        AUDIO_FREE_POINTER(lib_handler->lib_dump);
...
void aurisys_create_lib_handler_list_xlink(
    ...
    HASH_ITER(hh, name_list, itor_library_name, tmp_library_name) {
        ...
        *num_library_hanlder = (*num_library_hanlder) + 1;
        ...
        new_lib_handler->core_type = manager_config->core_type;
        ...
        /* init aurisys interface */
        if (the_library_config->api == NULL) {
            init_aurisys_lib_api(manager_config, the_library_config);
        }
        ...
        aurisys_open_raw_dump(new_lib_handler);
        aurisys_open_lib_dump(new_lib_handler);
...
void aurisys_destroy_lib_handler_list_xlink(aurisys_lib_handler_t **handler_list) {
    ...
    HASH_ITER(hh_all, *handler_list, itor_lib_hanlder, tmp_lib_hanlder) {
        ...
        aurisys_close_lib_dump(itor_lib_hanlder);
        aurisys_close_raw_dump(itor_lib_hanlder);
