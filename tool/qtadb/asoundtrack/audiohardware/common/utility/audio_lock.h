...
#define LOCK_ALOCK_NO_TIMEOUT(alock) \
    do { \
        alock_lock_no_timeout((alock), #alock, \
                              get_filename(__FILE__), \
                              (const char *)__FUNCTION__, \
                              __LINE__); \
    } while(0)
...
#define WAIT_ALOCK(alock) \
    do { \
        alock_wait_no_timeout((alock), #alock, \
                              get_filename(__FILE__), \
                              (const char *)__FUNCTION__, \
                              __LINE__); \
    } while(0)
...
/* ceiling us to ms due to the unit of pdMS_TO_TICKS() */
#define WAIT_ALOCK_US(alock, us) \
    ({ \
        int __ret = alock_wait_ms((alock), #alock, \
                                  (((us) + 999) / 1000), \
                                  get_filename(__FILE__), \
                                  (const char *)__FUNCTION__, \
                                  __LINE__); \
