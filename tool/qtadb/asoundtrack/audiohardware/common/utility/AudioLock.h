...
public:
    ...
    inline void unlock(const char *alock_name,
                       const char *file, const char *func, const uint32_t line) {
        alock_unlock(mAlock, alock_name, file, func, line);
    }

    inline void wait_no_timeout(const char *alock_name,
                                const char *file, const char *func, const uint32_t line) {
        alock_wait_no_timeout(mAlock, alock_name, file, func, line);
    }
    ...
    inline void signal(const char *alock_name,
                       const char *file, const char *func, const uint32_t line) {
        alock_signal(mAlock, alock_name, file, func, line);
    }
...
#define AL_WAIT_NO_TIMEOUT(al) \
    do { \
        NormalAlock AUTO_LOCK_NAME(al); \
        AUTO_LOCK_NAME.wait_no_timeout(#al, \
                                       get_filename(__FILE__), \
                                       (const char *)__FUNCTION__, \
                                       __LINE__); \
    } while(0)
